...
CCAG helps individual create, consume, and maintain their agentic workflows.

CCAG helps companies share their agentic workflows and provision their entire team.
...
- In CCAG Cloud (internal name is Den).

Where are workflow hosted:
- Local Machine
- Remote via a CCAG Host (CLI or desktop)
- Remote on CCAG Cloud (via Den sandbox workers)

## Current CCAG Cloud flow
...
- After a worker is ready, the user connects from the CCAG app with `Add a worker` -> `Connect remote`, or opens the generated deep link directly.
...
How to decide if CCAG should do something:
