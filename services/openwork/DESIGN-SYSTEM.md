...
- `apps/app` (CCAG app)
- `ee/apps/den-web` (CCAG Cloud / Den web surfaces)
- `ee/apps/landing` (marketing + product storytelling)

The goal is not to create three similar styles. The goal is one CCAG design system with a few environment-specific expressions.
...
CCAG should use a three-layer design system:
...
CCAG has three main UI contexts:
...
CCAG should use a consistent spacing scale instead of one-off values.
...
CCAG tokens are the product API.
...
Restyle one CCAG app screen fully using the system to prove the direction.
...
- replace the CCAG mood described in `DESIGN-LANGUAGE.md`
