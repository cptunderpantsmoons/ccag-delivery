...
async fn main() -> Result<()> {
    let image = env::var("OPENWORK_MICROSANDBOX_IMAGE")
        .unwrap_or_else(|_| "ccag-microsandbox:dev".to_string());
    let name = env::var("OPENWORK_MICROSANDBOX_NAME")
        .unwrap_or_else(|_| "ccag-microsandbox-rust".to_string());
    ...
    let server = sandbox
        ...
        .await
        .context("failed to start the ccag microsandbox entrypoint inside the VM")?;
...
mod tests {
    ...
    async fn rust_example_smoke_test_checks_health_and_session_endpoints() -> Result<()> {
        let image = env::var("OPENWORK_MICROSANDBOX_IMAGE")
            .unwrap_or_else(|_| "ttl.sh/ccag-microsandbox-11559:1d".to_string());
