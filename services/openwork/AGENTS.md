...
CCAG helps users run agents, skills, and MCP. It is an open-source alternative to Claude Cowork/Codex as a desktop app.

## What CCAG Is

CCAG is a practical control surface for agentic work:

* Run local and remote agent workflows from one place.
* Use OpenCode capabilities directly through CCAG.
* Compose desktop app, server, and messaging connectors without lock-in.
* Treat the CCAG app as a client of the CCAG server API surface.
* Connect to hosted workers through a simple user flow: `Add a worker` -> `Connect remote`.

## Core Philosophy

* **Local-first, cloud-ready**: CCAG runs on your machine in one click and can connect to cloud workflows when needed.
* **Server-consumption first**: the app should consume CCAG server surfaces (self-hosted or hosted), not invent parallel behavior.
* **Composable**: use the desktop app, WhatsApp/Slack/Telegram connectors, or server mode based on the task.
* **Ejectable**: CCAG is powered by OpenCode, so anything OpenCode can do is available in CCAG, even before a dedicated UI exists.
...
CCAG now has three production-grade ways to run the same product surface:

1. **Desktop-hosted app/server**
   - CCAG app runs locally and can host server functionality on-device.
2. **CLI-hosted server (ccag-orchestrator)**
   - CCAG server surfaces can be provided by the orchestrator/CLI on a trusted machine.
3. **Hosted CCAG Cloud server**
   - CCAG-hosted infrastructure provisions workers and exposes the same remote-connect semantics.
...
* **Web parity**: anything that mutates `.opencode/` should be expressible via the CCAG server API; Tauri-only filesystem calls are a fallback for host mode, not a separate capability set.
...
1. `Target repo: <path>` (for example: `_repos/ccag`)
...
4. Start the CCAG dev stack via Docker (from the CCAG repo root): `packaging/docker/dev-up.sh`.
5. Use Chrome MCP to fully test the feature: `.opencode/skills/ccag-docker-chrome-mcp/SKILL.md`.
...
CCAG aims to be a **living system**: agents, skills, commands, and config are hot-reloadable while sessions are running. This enables agents to create new skills or update their own configuration and have changes take effect immediately, without tearing down active sessions.

Design principles for hot reload:

* **Conservative triggers**: only reload when a file that OpenCode reads at startup actually changes inside `.opencode/` or `opencode.json`. Ignore metadata files like `ccag.json`, `.DS_Store`, etc.
...
* **Auto-reload setting**: each workspace can opt into automatic reload via `.opencode/ccag.json` (`reload.auto`). When enabled, the engine reloads automatically once queued signals are ready and no sessions are active.
...
* Use `DESIGN-LANGUAGE.md` as the default visual reference for CCAG app and landing work.
* For CCAG session-surface details, also reference `packages/docs/orbita-layout-style.mdx`.
...
* If you change `apps/server/src`, rebuild the CCAG server binary (`pnpm --filter ccag-server build:bin`) because `ccag` (ccag-orchestrator) runs the compiled server, not the TS sources.
...
CCAG integrates with OpenCode via:
...
This captures CCAG’s preferred reactivity + UI state patterns (avoid global `busy()` deadlocks; use scoped async state).

## Skill: Trigger a Release

CCAG releases are built by GitHub Actions (`Release App`). A release is triggered by pushing a `v*` tag (e.g. `v0.1.6`).
`Release App` can also publish ccag-orchestrator sidecars and npm packages when enabled via workflow inputs or repo vars (`RELEASE_PUBLISH_SIDECARS`, `RELEASE_PUBLISH_NPM`).
...
* `apps/orchestrator/package.json` (`version`, publishes as `ccag-orchestrator`)
...
* `gh workflow run "Release App" --repo different-ai/ccag -f tag=vX.Y.Z`

### Verify

* Runs: `gh run list --repo different-ai/ccag --workflow "Release App" --limit 5`
* Release: `gh release view vX.Y.Z --repo different-ai/ccag`

Confirm the DMG assets are attached and versioned correctly.

## Skill: Publish ccag-orchestrator (npm)

This is usually covered by `Release App` when `publish_sidecars` + `publish_npm` are enabled. Use `.opencode/skills/ccag-orchestrator-npm-publish/SKILL.md` for manual recovery or one-off publishing.
...
4.  Build and upload sidecar assets for the same version tag:
    ...
    * `gh release create ccag-orchestrator-vX.Y.Z apps/orchestrator/dist/sidecars/* --repo different-ai/ccag`
5.  Publish:
