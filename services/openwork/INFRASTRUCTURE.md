# CCAG Infrastructure Principles

CCAG is an experience layer. `opencode` is the engine. This document defines how infrastructure is built so every component is usable on its own, composable as a sidecar, and easy to automate.
...
* The CCAG UI may wrap these, but never replace or lock them out.
...
* OpenCode remains the engine; CCAG adds a thin config + UX layer.
...
* CCAG never replaces the CLI; it only connects to the engine.

### CCAG Server

* Runs standalone via `ccag-server` CLI.
* Provides filesystem-backed config surfaces (skills, plugins, MCP, commands).
* Sidecar lifecycle is described in `packages/app/pr/ccag-server.md`.
* Can also be consumed as a hosted CCAG Cloud control surface for remote worker lifecycle.

### CCAG Cloud Control Plane

* Hosted deployment of CCAG server capabilities for worker provisioning and remote connect.
...
* Must be able to use CCAG server for config and approvals.
...
* `PRINCIPLES.md`
* `ARCHITECTURE.md`
* `packages/app/pr/ccag-server.md`
