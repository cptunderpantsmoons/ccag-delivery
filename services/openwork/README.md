> CCAG is the open source alternative to Claude Cowork/Codex (desktop app).


## Core Philosophy

- Local-first, cloud-ready: CCAG runs on your machine in one click. Send a message instantly.
- Composable: desktop app, Slack/Telegram connector, or server. Use what fits, no lock-in.
- Ejectable: CCAG is powered by OpenCode, so everything OpenCode can do works in CCAG, even without a UI yet.
- Sharing is caring: start solo on localhost, then explicitly opt into remote sharing when you need it.

<p align="center">
  <img src="./app-demo.gif" alt="CCAG demo" width="800" />
</p>

CCAG is designed around the idea that you can easily ship your agentic workflows for your team as a repeatable, productized process.

> [!TIP]
> **Looking for an [Enterprise Plan](https://openworklabs.com/enterprise)?** [Speak with our Sales Team today](https://cal.com/team/ccag/enterprise)
>
> Get enhanced capabilities including feature prioritization, SSO, SLA support, LTS versions, and more.

## Alternate UIs
- **CCAG Orchestrator (CLI host)**: run OpenCode + CCAG server without the desktop UI.
  - install: `npm install -g ccag-orchestrator`
  - run: `ccag start --workspace /path/to/workspace --approval auto`
  - docs: [apps/orchestrator/README.md](./apps/orchestrator/README.md)

## Quick start

Download the desktop app from [openworklabs.com/download](https://openworklabs.com/download), grab the latest [GitHub release](https://github.com/different-ai/ccag/releases), or install from source below.
...
- Hosted CCAG Cloud workers are launched from the web app after checkout, then connected from the desktop app via `Add a worker` -> `Connect remote`.
...
- **Local/Remote**: CCAG works locally as well as can connect to remote servers.
...
which bun
bun --version
pnpm --filter @ccag/desktop exec tauri --version
```
...
CCAG now lives in `apps/app` (UI) and `apps/desktop` (desktop shell).
...
`pnpm dev` now enables `CCAG_DEV_MODE=1` automatically, so desktop dev uses an isolated OpenCode state instead of your personal global config/auth/data.
...
All repo `dev` entrypoints now opt into the same dev-mode isolation so local testing uses the CCAG-managed OpenCode state consistently.
...
- In **Host mode**, CCAG runs a local host stack and connects the UI to it.
  - Default runtime: `ccag` (installed from `ccag-orchestrator`), which orchestrates `opencode`, `ccag-server`, and optionally `opencode-router`.
  - Fallback runtime: `direct`, where the desktop app spawns `opencode serve --hostname 127.0.0.1 --port <free-port>` directly.

When you select a project folder, CCAG runs the host stack locally using that folder and connects the desktop UI.
...
Plugins are the **native** way to extend OpenCode. CCAG now manages them from the Skills tab by
...
You can still edit `opencode.json` manually; CCAG uses the same format as the OpenCode CLI:
...
If CCAG crashes on launch with WebKitGTK errors like `Failed to create GBM buffer`, disable dmabuf or compositing before launch. Try one of the following environment flags.

```bash
WEBKIT_DISABLE_DMABUF_RENDERER=1 ccag
```

```bash
WEBKIT_DISABLE_COMPOSITING_MODE=1 ccag
```

## Security Notes

- CCAG hides model reasoning and sensitive tool metadata by default.
...
Interested in using CCAG in your organization? We'd love to hear from you — reach out at [ben@openworklabs.com](mailto:ben@openworklabs.com) to chat about your use case.
