...
This document captures the desired end-state runtime model for CCAG Server V2.
...
-> talks to remote CCAG servers for remote workspaces
```

## Server And Workspace Registry Model

The local CCAG server should maintain the canonical registry of servers and workspaces.
...
The local CCAG server becomes the primary owner of:
...
- remote CCAG workspace connections
...
This database is the source of truth for CCAG-managed metadata and relationships.
...
- `skills`
  - skill metadata and CCAG-managed ownership
- `plugins`
...
The sqlite DB is for CCAG-owned metadata.
...
- belongs to the local CCAG server
- has a stable CCAG workspace ID
- maps to exactly one OpenCode project
- has its own CCAG-managed config directory
...
- belongs to a remote CCAG server
- maps to exactly one workspace on that remote CCAG server
...
- CCAG owns the workspace record
- CCAG workspace ID remains separate from the OpenCode project ID
- OpenCode owns the underlying session state for that project
- CCAG server is responsible for translating between workspace IDs and OpenCode project/session identifiers
...
Each remote workspace should map to a single workspace on a remote CCAG server.

That means:

- the local UI does not treat a remote workspace as a direct OpenCode connection
- the local server talks to the remote CCAG server
- the remote CCAG server is responsible for its own OpenCode integration
...
- one local CCAG server
- one exposed workspace access surface per workspace
- remote consumers connect to the CCAG server and are scoped to the workspace they were granted
...
2. Desktop app starts the local CCAG server.
...
6. The server resolves the workspace to its backend:
   ...
   - remote CCAG server workspace for remote workspaces
...
`opencode-router` should be started by the new CCAG server, not by the desktop app.
...
1. Desktop app launches the local CCAG server.
...
- one router process per local CCAG server
...
That makes `opencode-router` just another runtime dependency of the CCAG server, not a separate app-owned or orchestrator-owned control surface.
...
The CCAG server should query OpenCode via the SDK to get those.

CCAG should not duplicate the source of truth for those fields in its own sqlite DB unless it needs a cache or index for performance.

## Workspace Config Ownership Model

The config of a workspace should be managed by CCAG.
...
- plugins
- skills
- any CCAG-owned workspace settings
...
This lets CCAG own the config model while still projecting it into the OpenCode format OpenCode needs.
...
- CCAG-managed config directory
- OpenCode-managed runtime state

When a user adds a new local workspace:

1. CCAG prompts for the directory the user wants to work with.
2. That chosen directory becomes the workspace data directory.
3. CCAG creates a separate workspace config directory under an CCAG-controlled path.
4. OpenCode is pointed at the CCAG-controlled config directory.
5. The user’s chosen data directory is added as an authorized path in config.

This gives a cleaner split between:

- files owned by the user
- files controlled by CCAG
...
- parsing those items into CCAG-owned models
...
For anything related to a session, the client should always talk to the CCAG server with the workspace ID.

Example shape:

```text
client action
-> CCAG server request with workspaceId (+ sessionId if needed)
-> server resolves workspace backend
-> local OpenCode project/session OR remote CCAG workspace
-> response comes back through CCAG server
```
...
- the server forwards to the remote CCAG server
...
- CCAG workspace ID
- backend type
- OpenCode project ID or remote CCAG workspace ID

## OpenCode Primitive Exposure

To make this work, the CCAG server will need to expose the OpenCode primitives it depends on.
...
The CCAG server becomes the adapter and policy layer over those upstream primitives.

The public API exposed to clients should not use those raw session-first routes directly.

Instead, the public API should be normalized around workspace-first CCAG routes, for example:
...
- projection of CCAG config into OpenCode workspace config
...
- remote workspace support should normalize around "one workspace on one remote CCAG server"
...
9. Local workspaces keep a stable CCAG workspace ID that is separate from the OpenCode project ID, and the server stores the mapping.
10. Session and message primitives should be exposed through workspace-first CCAG-shaped endpoints, not raw OpenCode endpoint shapes.
...
4. What exact workspace-first CCAG route naming should wrap the required OpenCode session/message primitives?
