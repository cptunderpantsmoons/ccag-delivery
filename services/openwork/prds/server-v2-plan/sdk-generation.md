...
- Reusable client package: `packages/ccag-server-sdk`
...
- `packages/ccag-server-sdk/src/streams/**`
...
- `packages/ccag-server-sdk`: generated Server V2 client plus small handwritten SSE helpers for Server V2
...
### `packages/ccag-server-sdk/package.json`
...
```json
{
  "scripts": {
    ...
    "sdk:generate": "pnpm --filter ccag-server-v2 openapi:generate && pnpm --filter @ccag/server-sdk generate"
  }
}
```
...
- `packages/ccag-server-sdk/scripts/watch.mjs`
...
```bash
pnpm --filter ccag-server-v2 openapi:generate && pnpm --filter @ccag/server-sdk generate && git diff --exit-code
```
...
- SSE helpers: handwritten in `packages/ccag-server-sdk/src/streams/**`
