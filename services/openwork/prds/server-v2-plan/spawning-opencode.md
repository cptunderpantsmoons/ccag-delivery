...
- our distribution plan wants the CCAG server to ship its own bundled OpenCode binary
...
- accepts a custom config object generated by the CCAG server
...
- `ccag-server-v2` is the primary executable
- it embeds `opencode`
...
That config blob should come from the CCAG server's own database-backed config model, not from ad hoc client state.
...
The config object passed into this helper should be generated by the CCAG server.
...
- `ccag-server-v2` embeds `opencode`
