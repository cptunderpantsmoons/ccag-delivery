...
4. The desktop layer starts or reconnects to the local CCAG server host path.
...
- Ends up calling: child-process kill paths for the engine, orchestrator, CCAG server, and router.
...
- What it does: renames the macOS process to `CCAG - Dev` in dev mode.
...
Reasoning: the desktop app should keep the ability to launch the CCAG server, but OpenCode runtime ownership and coordination should move behind the server boundary over time.
...
- Ends up calling: local project-directory setup, `opencode.json` seeding, port selection, credential generation, engine spawn, CCAG server startup, and OpenCodeRouter startup.
...
- Ends up calling: child-process shutdown/restart across engine, router, orchestrator, and hosted CCAG server.
...
- What it does: launches the CCAG orchestrator daemon sidecar.
...
- Ends up calling: `ccag-orchestrator start --detach`, token generation, health polling, optional owner-token issuance, and progress event emission.

### `sandbox_doctor()`, `sandbox_stop()`, `sandbox_cleanup_ccag_containers()`, `sandbox_debug_probe()`
...
## Hosted CCAG Server Lifecycle

Disposition guidance:

- `start_ccag_server()` -> `Split`
- `spawn_ccag_server()` -> `Stay`
- `resolve_ccag_port()` -> `Stay`
- token and state helpers in `ccag_server/mod.rs` -> `Split`
- `build_urls()` -> `Stay`
- `ccag_server_info()` / `ccag_server_restart()` -> `Split`
- `CcagServerManager` -> `Split`

Reasoning: the desktop shell should keep launch/supervision of the local CCAG server process, but token semantics, runtime info, and restart/control behavior should keep shrinking into the server over time.

### `start_ccag_server()`

- What it does: starts the local desktop-hosted CCAG server and tracks its tokens, URLs, and health.
- Called from and when: called by engine startup and explicit CCAG server restart flows.
...
- What it does: launches the `ccag-server` sidecar.
- Called from and when: called by `start_ccag_server()`.
- Ends up calling: child-process spawn, cwd selection, and env var injection including OpenCode base URL, OpenCode creds, and CCAG tokens.

### `resolve_ccag_port()`

- What it does: picks an CCAG server port with reuse and conflict avoidance.
- Called from and when: called on host-mode server start.
- Ends up calling: local TCP port selection logic.

### token and state helpers in `ccag_server/mod.rs`
...
### `ccag_server_info()` / `ccag_server_restart()`

- What they do: expose CCAG server runtime info and restart it with the current engine credentials/workspaces.
...
- authorized-root and `ccag.json` read/write helpers -> `Move`
...
- Ends up calling: `ccag-workspaces.json` reads/writes and path canonicalization; in the ideal model this becomes transitional reconnect/cache state rather than the canonical registry.

### `workspace_create()`

- What it does: creates a new local workspace and seeds its initial CCAG/OpenCode files.
...
### `workspace_add_authorized_root()`, `workspace_ccag_read()`, `workspace_ccag_write()`

- What they do: manage `.opencode/ccag.json` data for authorized roots and related settings.
- Called from and when: called from settings/permissions UI.
- Ends up calling: workspace-local config file reads and writes; in the ideal model this is server-side materialization into the CCAG-managed workspace config directory.
...
- What they do: register `ccag://` and `ccag-dev://`, receive deep links, and forward them into the UI.
...
- `reset_ccag_state()` -> `Split`
- `nuke_ccag_and_opencode_config_and_exit()` -> `Split`
...
- What it does: stops managed services and clears CCAG desktop state.
...
### `nuke_ccag_and_opencode_config_and_exit()`

- What it does: performs a broader local reset across CCAG and OpenCode state, then exits.
