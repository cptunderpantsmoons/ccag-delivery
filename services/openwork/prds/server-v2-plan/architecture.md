...
- one `opencode-router` child per local CCAG server
...
packages/ccag-server-sdk/generated/**
packages/ccag-server-sdk/src/index.ts
...
- `packages/ccag-server-sdk`: watch `openapi/openapi.json`, regenerate the reusable generated client package
- `apps/app`: watch the app-side `createSdk({ serverId })` adapter alongside normal app code
- `packages/ccag-server-sdk`: optional watch build if the package publishes built output
...
The local CCAG server should maintain the durable registry of servers and workspaces. The app should render or cache what the server returns.
...
- local desktop-hosted CCAG server
- remote worker-backed CCAG server
...
- `id` is the stable CCAG workspace identifier the UI uses
...
In the ideal steady state, normal app traffic should still flow through the local CCAG server using stable CCAG workspace IDs, with remote CCAG workspace IDs and OpenCode project IDs remaining server-owned mappings.
...
The app should consume OpenCode-related streaming only through the CCAG server.
...
- expose small handwritten streaming helpers from `packages/ccag-server-sdk`
...
- whether the working name `ccag-server-v2` survives to ship time or is renamed before release
