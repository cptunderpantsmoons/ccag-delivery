...
- Server V2 should be enabled explicitly with `CCAG_UI_USE_SERVER_V2=1` (or the Vite-exposed equivalent in the frontend build).
...
export function checkServerFlag() {
  return import.meta.env.VITE_CCAG_UI_USE_SERVER_V2 === "1"
}
```
...
- direct fetches to current CCAG server endpoints
