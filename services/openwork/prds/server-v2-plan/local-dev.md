...
```bash
CCAG_UI_USE_SERVER_V2=1 pnpm dev:server-v2
```
...
-> SDK watch regenerates packages/ccag-server-sdk/generated/**
...
To keep iteration fast, `packages/ccag-server-sdk` should ideally work like this in development:
...
In the ideal product model, most user-facing app traffic should still target the local CCAG server as the canonical adapter and registry. Direct alternate `serverId` targeting is mainly for migration, testing, and explicit server-management flows.
...
- expose small handwritten SSE helpers from `packages/ccag-server-sdk`
