...
- Called from and when: called when the `ccag-server` binary or `bun src/cli.ts` is launched.
...
- What it does: creates starter `.opencode` state, commands, skills, agent, `opencode.json`, and `ccag.json`.
- Called from and when: called when creating a local workspace.
- What it calls: CCAG/OpenCode starter file generation.
...
### `ensureWorkspaceCcagConfig`

- What it is: CCAG config seeder.
- What it does: seeds `ccag.json` with authorized roots, blueprint sessions, and workspace metadata.
- Called from and when: called during `ensureWorkspaceFiles`.
- What it calls: CCAG-specific workspace behavior and starter session metadata.

### workspace config routes

- What they are: config read/patch/raw text endpoints.
- What they do: read or patch workspace `opencode` and `ccag` config, including raw OpenCode config editor flows.
...
- What they do: manage uploadable inbox files and downloadable artifact files under `.opencode/ccag`.
...
- What they do: publish and fetch named bundle payloads via a trusted CCAG publisher.
...
- What they call: audit persistence under CCAG data directories.

### blueprint session helpers

- What they are: blueprint template normalization/materialization helpers.
- What they do: parse starter-session templates from `ccag.json`, track what was already materialized, create OpenCode sessions, and seed starter messages.
- Called from and when: called by blueprint/session materialization routes and workspace bootstrap flows.
- What they call: upstream OpenCode session creation, direct OpenCode DB seeding, and `ccag.json` updates.
