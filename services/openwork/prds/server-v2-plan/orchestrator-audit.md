...
The orchestrator is the host-side runtime manager for CCAG.
...
3. It starts and supervises OpenCode, CCAG server, and optional router.
...
- Called from and when: process entrypoint for the `ccag` CLI.
...
- Called from and when: called by `main()` for `ccag start` and `ccag serve`; also used indirectly when desktop detached mode shells out to `ccag start --detach`.
- Ends up calling: OpenCode startup, CCAG server startup, optional OpenCodeRouter startup, sandbox/container logic, health loops, owner token issuance, and runtime control server setup.
...
- What it does: launches `ccag-server` with the bootstrap information needed for host mode today.
- Called from and when: called from `runStart()` and restart paths.
- Ends up calling: the main CCAG API/control layer process; over time this bootstrap contract should shrink toward the minimum needed to bring the server up.
...
- Ends up calling: rolling restart of OpenCode, CCAG server, and OpenCodeRouter.
...
Reasoning: in the current orchestrator these are host/bootstrap concerns, but the Server V2 target changes the default distribution model. When the canonical runtime is `ccag-server-v2` bundling and extracting its own sidecars, the equivalent resolution logic should move into the server for that path. A thinner host shell may still need fallback or external-runtime resolution in some modes, so this boundary is better treated as `Split` than `Stay`.

### `resolveCcagServerBin()`

- What it does: decides which `ccag-server` binary to run.
...
- Called from and when: called by `ccag daemon run`; desktop also spawns it for desktop host mode.
...
- Called from and when: called by `ccag workspace ...`.
- Ends up calling: daemon HTTP API.

### `runInstanceCommand()`

- What it does: thin CLI wrapper for instance disposal commands.
- Called from and when: called by `ccag instance dispose ...`.
- Ends up calling: daemon HTTP API.
...
- What they do: read and write `ccag-orchestrator-state.json` and related state snapshots.
...
- What it does: writes the in-container boot script that starts OpenCode, optional router, then CCAG server.
...
- Called from and when: called by `ccag start --check` and related flags.
- Ends up calling: broad end-to-end runtime verification.

### `runStatus()`

- What it does: reports current CCAG/OpenCode URLs and related status in human-readable form.
- Called from and when: called by `ccag status`.
...
Reasoning: these are not orchestrator responsibilities. They are convenience clients over `ccag-server` APIs.

### `runFiles()`

- What it does: exposes CLI wrappers for file session creation, read/write, catalog, events, mkdir, delete, and rename.
- Called from and when: called by `ccag files ...`.
- Ends up calling: CCAG server file-session APIs.

### `runApprovals()`

- What it does: exposes CLI wrappers for approvals listing and replies.
- Called from and when: called by `ccag approvals ...`.
- Ends up calling: CCAG server approvals APIs.
...
```text
desktop app
-> launches or connects to CCAG server
