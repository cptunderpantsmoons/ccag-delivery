...
This document defines the preferred distribution model for the new CCAG server.
...
- `ccag-server-v2-darwin-arm64`
- `ccag-server-v2-darwin-x64`
- `ccag-server-v2-linux-x64`
- `ccag-server-v2-linux-arm64`
- `ccag-server-v2-windows-x64.exe`
...
- ship with the matching `ccag-server-v2` runtime embedded or bundled as an app resource
- launch only that server
- never directly launch `opencode` or `opencode-router`

At runtime:

1. Desktop app launches `ccag-server-v2`.
2. `ccag-server-v2` checks its managed runtime directory.
...
- standalone users download `ccag-server-v2` for their platform
...
```bash
bun build --compile --minify --bytecode --target=bun-darwin-arm64 ./src/cli.ts --outfile dist/ccag-server-v2
```
...
- `pnpm --filter ccag-server-v2 build:bin` builds the plain compiled executable.
- `pnpm --filter ccag-server-v2 build:bin:embedded --bundle-dir <runtime-bundle-dir>` builds the compiled executable with embedded runtime assets from a prepared bundle directory.
- `pnpm --filter ccag-server-v2 build:bin:embedded:all` drives the same embedding flow across the supported Bun targets when target-specific runtime bundle files are staged.
...
- desktop app contains the matching `ccag-server-v2` runtime
- user launches app
- app launches server

### Standalone server release

- user downloads `ccag-server-v2` for their platform
...
- release: sidecars are embedded into `ccag-server-v2` and extracted on first run
...
- `ccag-server`, `opencode`, and `opencode-router` are not yet one canonical server runtime bundle

Target behavior becomes:

- desktop app starts one thing: `ccag-server-v2`
- standalone users start that same `ccag-server-v2`
- `ccag-server-v2` starts and supervises its own runtime dependencies
...
- the main `ccag-server-v2` executable will need a clean signing and notarization path
...
- if `ccag-server-v2.exe` extracts `opencode.exe` and `opencode-router.exe`, those sidecars should ideally also be signed before embedding
...
1. Build `ccag-server-v2.exe` for Windows.
...
4. Sign the final `ccag-server-v2.exe`.
  ...
  build-windows-server-v2:
    ...
    steps:
      ...
      - name: Build server runtime
        run: pnpm --filter ccag-server-v2 build:bin:windows

      - name: Build or fetch Windows sidecars
        run: pnpm --filter ccag-server-v2 build:sidecars:windows
      ...
      - name: Build embedded runtime
        run: pnpm --filter ccag-server-v2 package:windows

      - name: Sign final server runtime
        shell: pwsh
        run: |
          signtool sign /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 /f codesign.pfx /p "${{ secrets.WINDOWS_CERT_PASSWORD }}" dist\\ccag-server-v2.exe

      - name: Verify signature
        shell: pwsh
        run: |
          signtool verify /pa /v dist\\ccag-server-v2.exe
```
