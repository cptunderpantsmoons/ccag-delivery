...
For planning purposes in this doc set, `ccag-server-v2` is the working name for that new package, binary, and runtime bundle. We can rename it later without changing the architecture direction.
...
```text
desktop app or CLI
-> starts or connects to one CCAG server process
-> CCAG server owns workspace/runtime/product behavior
-> CCAG server supervises the local runtime pieces it needs
```
...
- local bootstrap logic that only exists to support the CCAG runtime
...
The new server should expose CCAG-shaped routes directly.

Recommendation:

- use workspace-first CCAG routes as the real public API shape
...
- `packages/ccag-server-sdk` watches `openapi/openapi.json` and regenerates the reusable generated client package
- `packages/ccag-server-sdk` regenerates the reusable server-agnostic client package
- the app watches its own `createSdk({ serverId })` adapter alongside normal app code
- the app depends on `ccag-server-sdk` through the workspace so type updates are visible immediately
...
- import or normalize workspace records from current desktop state such as `ccag-workspaces.json`
...
In the ideal model, the local CCAG server owns the durable mapping between:

- CCAG workspace ID
- remote CCAG workspace ID
- OpenCode project ID
- backend server identity

The app should usually operate on stable CCAG workspace IDs returned by the local server, not on remote backend IDs directly.
...
In the ideal steady state, normal product traffic should target the local CCAG server as the canonical adapter and registry, while direct alternate `serverId` targeting is reserved for explicit server-management or migration/testing scenarios.
...
- the CCAG server should still be the only streaming surface the app talks to
