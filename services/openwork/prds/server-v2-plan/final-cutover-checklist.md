...
- App rollout default: legacy path unless `CCAG_UI_USE_SERVER_V2=1`
...
- Desktop can bundle and start Server V2 when `CCAG_UI_USE_SERVER_V2=1` is enabled.
- App can route through Server V2 when `CCAG_UI_USE_SERVER_V2=1` is enabled.
...
pnpm sdk:generate
pnpm contract:check
pnpm --filter ccag-server-v2 test
pnpm --filter ccag-server-v2 typecheck
pnpm --filter @ccag/app test:server-v2-boundary
pnpm --filter @ccag/app typecheck
cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml --locked
pnpm --filter ccag-server-v2 build:bin
pnpm --filter ccag-server-v2 build:bin:embedded --bundle-dir ../desktop/src-tauri/sidecars
```
...
pnpm --filter ccag-server-v2 build:bin:embedded --bundle-dir ../desktop/src-tauri/sidecars
codesign --deep --force -vvvv --sign "<APPLE_IDENTITY>" --entitlements apps/desktop/src-tauri/entitlements.plist apps/server-v2/dist/bin/ccag-server-v2
codesign -vvv --verify apps/server-v2/dist/bin/ccag-server-v2
xcrun notarytool submit apps/server-v2/dist/bin/ccag-server-v2 --key <KEY_ID> --key-id <KEY_ID> --issuer <ISSUER_ID> --wait
spctl --assess --type execute --verbose apps/server-v2/dist/bin/ccag-server-v2
CCAG_SERVER_V2_WORKDIR="$(mktemp -d)" apps/server-v2/dist/bin/ccag-server-v2 --port 32123
```
...
pnpm -C apps/desktop prepare:sidecar
pnpm --filter ccag-server-v2 build:bin:embedded --bundle-dir ../desktop/src-tauri/sidecars --target bun-windows-x64
signtool sign /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 /f codesign.pfx /p <PASSWORD> apps\server-v2\dist\bin\ccag-server-v2-bun-windows-x64.exe
signtool verify /pa /v apps\server-v2\dist\bin\ccag-server-v2-bun-windows-x64.exe
```

SmartScreen / AV validation:

```powershell
$env:CCAG_SERVER_V2_WORKDIR = Join-Path $env:TEMP "ccag-server-v2-smoke"
New-Item -ItemType Directory -Force -Path $env:CCAG_SERVER_V2_WORKDIR | Out-Null
apps\server-v2\dist\bin\ccag-server-v2-bun-windows-x64.exe --port 32123
...
curl -H "Authorization: Bearer $CCAG_TOKEN" http://127.0.0.1:<CCAG_PORT>/workspaces
curl -X POST -H "Authorization: Bearer $CCAG_TOKEN" -H "Content-Type: application/json" --data '{"title":"Docker E2E"}' http://127.0.0.1:<CCAG_PORT>/workspaces/<WORKSPACE_ID>/sessions
```
