...
- **Prepared seam**: `@ccag/ui` already exports both React and Solid components. `SessionViewProps` is a clean data-only interface.
...
- **No server-side session read endpoints**: The CCAG server has no `GET /sessions` or `GET /session/:id`. It proxies to OpenCode.
...
### Feature flag gate
```
localStorage.setItem('ccag:react-session', 'true')
// or
