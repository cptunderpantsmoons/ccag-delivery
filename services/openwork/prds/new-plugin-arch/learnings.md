...
- Workspace dependency installation is still a gating factor for broader den-api tests in this worktree: pure helper tests can run with Bun, but route/store tests that import `hono` or `@ccag-ee/den-db/*` still fail until the workspace dependencies are installed.
...
- den-api package tests currently work best from `ee/apps/den-api/test/` rather than `ee/apps/den-api/src/`, because the package `tsconfig.json` compiles `src/**` during `pnpm --filter @ccag-ee/den-api build` and would otherwise drag Bun-only test imports into the production build.
...
- The persistence backbone now lives in `ee/packages/den-db/src/schema/sharables/plugin-arch.ts`; den-api can query these tables directly through the existing shared `@ccag-ee/den-db/schema` export instead of adding a new repository layer.
...
- Local verification is currently dependency-blocked in this worktree: `pnpm --filter @ccag-ee/den-api build` and direct `tsc` both stop immediately because package-local dependencies like `tsup`, `zod`, and other workspace modules are not installed in this checkout.
