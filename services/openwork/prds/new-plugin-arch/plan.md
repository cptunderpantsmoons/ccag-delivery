...
- Cloud: created or edited directly in CCAG Cloud.
...
- cloud/import objects are first-party editable in CCAG;
- connector-managed objects may still be editable in CCAG by users with permission, but connector sync remains authoritative and may overwrite content on the next sync;
- CCAG must keep enough source metadata to re-ingest, reconcile, audit, and display provenance.
