...
Status: draft
Owner: src-opn
Scope: CCAG server + Den web + OpenCode integration
...
- **Installing a bundle into a workspace** means writing each member primitive to its **native `.opencode/` path** via the CCAG server's existing per-workspace endpoints. The `reload-watcher` then propagates to running sessions.
...
|---|---|---|---|
| Plugin manifest | `.claude-plugin/plugin.json` | **None** (no manifest file) | ❌ — CCAG-only DB concept |
| Marketplace catalog | `.claude-plugin/marketplace.json` | **None** | ❌ — CCAG-only DB concept |
...
Bundle
  id
  org_id
  slug                       # "ccag-release-kit"
  name                       # "CCAG Release Kit"
  description
  version                    # "2.3.1"
  icon                       # emoji or URL
...
WorkspaceInstallation
  ...
  org_id
  bundle_id
  workspace_id               # the CCAG workspace (worker + path)
...
Materialization steps (all executed server-side against the CCAG server API — not direct FS access):
...
The `reload-watcher` on the server picks up each file write and emits `ReloadEvent`s keyed by `workspaceId`. OpenCode-running sessions pick up skills/commands/agents hot; plugin code and MCP changes require a new session (reload-watcher already handles this via `ccag.json` → `reload.auto`).
...
- A tiny **CCAG skill** running inside the user's workspace maintains a **local SQLite** DB that mirrors the subset of the remote DB relevant to that workspace (primitives + installed bundles).
...
Workspace scope lives under `/v1/workspaces/:workspaceId/…` — mirrors the existing CCAG server shape (`/workspace/:id/skills`, `/workspace/:id/commands`, etc.). The installation endpoints below are **orchestrators** that internally fan out to those existing workspace-level endpoints. No new filesystem primitives required.
...
```
 User            Den web             Den API             Provider           CCAG server
  │                 │                  │                    │                    │
  │─ click Connect ─▶                  │                    │                    │
...
```
 User            Den web           Den API                         CCAG server                           Disk
  │                 │                │                                 │                                      │
  │─ click Install ▶│                │                                 │                                      │
  ...
  │                 │                │                                 │                                      │
  │                 │  OpenCode session:                                                                      │
  │                 │    auto-reload if ccag.json.reload.auto, else toast "reload available"              │
```
...
| C. Ship a universal runner | Publish one npm package `@ccag/claude-hooks-runtime` that reads JSON hooks from a well-known path (`.opencode/ccag/hooks/*.json`) and registers them once. Installing any Bundle with JSON hooks just drops files in that path. | High upfront (one-time infrastructure) but zero per-Bundle cost thereafter. | v2 — best long-term |
...
Closes the loop with the ecosystem: an authored Bundle can be pushed back to a Git repo as a Claude-compatible marketplace tree so other CCAG (or Claude Code) users can ingest it.
...
Ingesting an exported tree back into CCAG MUST produce semantically-identical Primitives (same `contentHash` per primitive, same Bundle membership). This is a tested invariant — see [Test strategy](#test-strategy).
...
```
ccag connector list
ccag connector add github --repo different-ai/ccag-plugins [--ref main]
ccag connector remove <source-id>
ccag connector sync <source-id>

ccag bundle list [--installed]
ccag bundle show <bundle-slug>
ccag bundle install <bundle-slug> [--scope workspace|user|org]
ccag bundle uninstall <bundle-slug>
ccag bundle publish <bundle-slug> --to github:owner/repo [--ref main]

ccag primitive list [--kind=skill|agent|…]
ccag primitive show <name> --kind=<kind>
ccag primitive create --kind=skill --file=./my-skill.md
```
...
An **CCAG skill** wraps the CLI and surfaces it to agents:

```yaml
---
name: ccag-plugin-manager
description: |
  Install, uninstall, and discover CCAG plugins (bundles).
...
- **Materialization**: mock the CCAG server API; assert correct endpoint + body per primitive kind; assert `appliedPrimitiveDigests` is recorded.
- **Drift detection**: given `{ primitive.contentHash, disk.hash }` permutations, assert correct classification.

### Integration

- **Real GitHub**: a test org + throwaway repo under `different-ai/ccag-test-plugins` with fixture plugins. CI authenticates with a PAT; runs a full ingest + install + uninstall against a temp worktree. Skipped in local unless `CCAG_TEST_GITHUB_PAT` is set.
- **Real CCAG server**: spins up `packaging/docker/dev-up.sh`, provisions a workspace, runs materialization, diffs `.opencode/` against expectations.
...
1. **Agents mutation endpoint**: CCAG server has `/workspace/:id/{skills,commands,plugins,mcp}` but **no dedicated agents endpoint**. Options: (a) add `POST /workspace/:id/agents`, (b) use the generic `POST /workspace/:id/files/content` for agents, (c) bundle agents under plugins. Recommend (a) for parity.
