...
4. let the user choose which discovered plugins should map into CCAG;
5. translate the selected discovery result into CCAG connector records and future ingestion work.
...
- how the result maps into CCAG internal structures.
...
It is not enough to understand the shape of the repo and convert that shape into useful CCAG mappings.

The discovery phase fills that gap.

Instead of immediately asking the user to author raw path mappings, CCAG should first inspect the repo and propose a structured interpretation of what it found.
...
3. CCAG creates the connector instance and target.
4. CCAG routes the user into a dedicated `Setup` / `Discovery` page for that connector instance.
5. CCAG reads the repository tree and shows progress steps in the UI.
6. CCAG classifies the repo shape.
7. CCAG shows discovered plugins, preselected by default.
8. User confirms or deselects discovered plugins.
9. CCAG creates the initial connector mappings and plugin records from that discovery result.
10. CCAG is ready for initial ingestion/sync.
...
That means CCAG discovery should treat marketplace repos as a first-class shape, but be explicit about what is in-scope for a connected single repo.
...
This keeps the behavior honest and still lets users understand what CCAG detected.
...
- a short explanation that CCAG is figuring out how to map this repo.
...
## Mapping discovered plugins to CCAG internal data
...
- create CCAG plugin `Sales`
- create CCAG plugin `finance`
- create mappings:
...
- create one CCAG plugin from the marketplace/plugin metadata;
...
5. CCAG creates plugin rows + connector mappings
6. CCAG enqueues initial full sync
...
- hooks-to-CCAG runtime semantics beyond discovery;
...
4. Should root-level `plugin.json` remain a metadata hint only, or do we want to formalize it as an CCAG-specific compatibility rule?
