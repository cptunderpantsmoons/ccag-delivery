...
Define the desired user experience for connecting GitHub to CCAG in den-web (cloud), independent of the current implementation state.

This flow is for the den-api plugin connector system and GitHub App based connector onboarding.

## Desired user flow

1. User is in CCAG den-web (cloud).
...
6. CCAG sends the user to the GitHub App install/authorize flow.
7. User completes the normal GitHub steps on GitHub.
8. GitHub returns the user to CCAG.
9. CCAG recognizes the completed GitHub App installation for the current org/user context.
10. CCAG shows the user the list of repositories available through that installation.
11. User selects one repository.
12. CCAG creates a new GitHub connector instance for that selected repository.
13. CCAG configures webhook-driven sync for that repository.
14. Future pushes to the connected repository trigger CCAG sync behavior through the connector pipeline.
...
- The flow should preserve enough CCAG context to return the user to the correct org and screen after GitHub finishes.
- The GitHub-side step should feel like a normal GitHub App installation flow.

### Return to CCAG

- After GitHub redirects back, CCAG should detect the installation that was just created or updated.
- If installation state is incomplete or ambiguous, CCAG should guide the user instead of silently failing.
- The user should land back in the GitHub integration flow, not on a generic page with no next step.

### Repository selection

- CCAG should list repositories available to the installation.
- The user should be able to pick one repository as the first connected source.
- Selecting a repository should create a connector instance for that repo in the current CCAG org.
...
- Once connected, CCAG should be ready to receive GitHub App webhooks for the selected repository.
...
2. Preserve CCAG return context across the redirect.
...
3. The user returns to CCAG automatically.
4. CCAG shows repositories from that installation.
5. The user selects a repo.
6. CCAG creates a connector instance for that repo.
