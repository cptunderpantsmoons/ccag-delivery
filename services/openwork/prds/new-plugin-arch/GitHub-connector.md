...
- ingest matching files into CCAG;
- keep CCAG in sync when GitHub sends webhook events.
...
- CCAG stores a connector account, connector instance, target repo, and path mappings;
- GitHub webhooks notify CCAG that the selected branch changed;
- CCAG then reconciles against the current branch head state;
- CCAG does not treat individual changed files in the webhook payload as the final source of truth.
...
- authorize CCAG to see which installations/repos they can use;
...
Inside CCAG, an admin creates a GitHub connector instance.
...
After setup, CCAG should run an initial full reconciliation against the selected branch head.
...
CCAG must verify the GitHub App webhook secret on every incoming webhook delivery.
...
CCAG should fetch repo state at the selected branch head commit.
...
- GitHub App installation gives CCAG repo access and webhooks;
...
- CCAG reads the current branch-head repo state and ingests from that state;
- CCAG compares desired current files against existing bindings to create, update, delete, and tombstone objects;
