# CCAG Desktop Bootstrap And Org Runtime Config

## Goal

Support three desktop distribution modes while still allowing all of them to update from the default app build:

1. Default build pointing at the default CCAG server with no forced sign-in.
2. Custom build pointing at the default CCAG server with forced sign-in.
...
- `_repos/ccag/_worktrees/Desktop-config-den-endpoint/apps/app/src/app/lib/den.ts`
...
- `CCAG_DESKTOP_DEN_BASE_URL`
- `CCAG_DESKTOP_DEN_API_BASE_URL`
- `CCAG_DESKTOP_DEN_REQUIRE_SIGNIN`
...
```bash
CCAG_DESKTOP_DEN_BASE_URL="https://client.example.com" \
CCAG_DESKTOP_DEN_API_BASE_URL="https://client.example.com/api/den" \
CCAG_DESKTOP_DEN_REQUIRE_SIGNIN="1" \
pnpm --filter @ccag/desktop build
```

### Accepted boolean values

`CCAG_DESKTOP_DEN_REQUIRE_SIGNIN` and `VITE_DEN_REQUIRE_SIGNIN` are treated as enabled when set to one of:
...
For the current external bootstrap-file approach, the desktop shell should look for `desktop-bootstrap.json` in the host config directory under `CCAG/`, unless an explicit override path is provided with `CCAG_DESKTOP_BOOTSTRAP_PATH`.

Expected default locations:

- macOS: `~/Library/Application Support/CCAG/desktop-bootstrap.json`
- Linux: `~/.config/CCAG/desktop-bootstrap.json`
- Windows: `%AppData%\CCAG\desktop-bootstrap.json`
...
- `server.baseUrl`: default CCAG server
- `requireSignin`: `false`
