...
- renderer access to localhost/CCAG/OpenCode HTTP surfaces
...
apps/desktop/src-tauri/src/commands/ccag_server.rs: 2
...
apps/app/src/app/lib/ccag-server.ts
...
- `server-provider.tsx`, `ccag-server.ts`, `opencode.ts`, `den.ts`, and `bundles/sources.ts` use Tauri HTTP fetch in desktop mode.
...
apps/app/src/react-app/domains/connections/ccag-server-store.ts
...
apps/desktop/src-tauri/src/ccag_server/manager.rs
apps/desktop/src-tauri/src/ccag_server/mod.rs
apps/desktop/src-tauri/src/ccag_server/spawn.rs
...
- `workspace/watch.rs` emits `ccag://reload-required` from native file-watch events.
...
- `engine/*`, `orchestrator/*`, `ccag_server/*`, and `opencode_router/*` currently spawn and supervise local child processes.
...
apps/desktop/src-tauri/src/commands/ccag_server.rs
...
- `ccag-debug.sh` kills Tauri dev processes.
...
```text
prds/ccag-desktop-bootstrap-config/desktop-bootstrap-and-org-runtime-config.md
...
1. Keep the package name `@ccag/desktop` and replace internals in place, or create a parallel Electron package and cut over later.
