...
| OS       | Path                                                     |
| -------- | -------------------------------------------------------- |
| macOS    | `~/Library/Application Support/com.differentai.ccag/`|
| Windows  | `%APPDATA%\com.differentai.ccag\`                    |
| Linux    | `~/.config/com.differentai.ccag/`                    |

Contents (observed on a real machine):
- `ccag-workspaces.json` (Tauri's name)
- `ccag-server-state.json`
- `ccag-server-tokens.json`
- `workspaces/` subtree
...
- `~/.ccag/ccag-orchestrator/` — orchestrator daemon data
...
- `apps/desktop/src-tauri/tauri.conf.json` →
  `endpoints: ["https://github.com/different-ai/ccag/releases/latest/download/latest.json"]`
...
app.setPath(
  "userData",
  path.join(app.getPath("appData"), "com.differentai.ccag"),
);
```

```yaml
# apps/desktop/electron-builder.yml
appId: com.differentai.ccag   # (currently com.differentai.ccag.electron)
```
...
- First Electron launch finds the Tauri-written `ccag-server-*.json`
...
async function readWorkspaceState() {
  const legacy = path.join(userData, "ccag-workspaces.json"); // Tauri
...
1. A single new command `migrate_to_electron()` in the Tauri shell that:
   - Downloads the matching Electron installer from the same GitHub Release
     (`CCAG-0.12.0-mac-<arch>.dmg`, `.exe`, `.AppImage`).
...
2. A one-time prompt:
   ...
   > CCAG is moving to a new desktop engine. We'll install the new
...
latest.json says 0.12.0 is out → DMG installed in-place → Tauri v0.12.0-migrate
      ...
      ▼
migrate_to_electron():
  download CCAG-0.12.0-electron-mac.dmg from same release
  codesign --verify ✓
  open installer, schedule Tauri quit
      │
      ▼
Installer replaces the .app bundle
  appId = com.differentai.ccag (same as Tauri)
  Launchpad slot + Dock pin preserved, no duplicate "CCAG" icon
      │
      ▼
CCAG Electron v0.12.0 first launch
  app.setPath("userData", .../com.differentai.ccag) points at the
  Tauri-written folder → tokens, workspaces, orchestrator state already there
  rename ccag-workspaces.json → workspace-state.json (once)
      │
      ▼
...
- `Release App` workflow:
  - Replaces `tauri build` with `pnpm --filter @ccag/desktop package:electron`.
...
- `ccag.react.activeWorkspace` — last selected workspace
- `ccag.react.sessionByWorkspace` — per-workspace last session
- `ccag.server.list` — multi-server list
- `ccag.server.active` — selected server
- `ccag.server.urlOverride` — server override
- `ccag.server.token` — server token
...
- Electron main exposes `ccag:migration:read` and
  `ccag:migration:ack` IPC handlers; preload bridges them under
  `window.__CCAG_ELECTRON__.migration`.
...
2. **This PR** — "migration engine". Unifies `appId` + `userData` path to
   Tauri's, adds `ccag-workspaces.json` → `workspace-state.json`
...
3. **Last Tauri release v0.12.0** — ships:
   ...
   - a one-time prompt ("CCAG is moving to a new engine — install?")
