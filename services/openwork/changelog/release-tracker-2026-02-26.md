...
#### Release importance
Minor release: simplifies artifact management and adds faster workspace controls without changing CCAG's overall workflow model.
...
- Reused existing CCAG tokens during sandbox restarts so reconnects keep working.
...
#### Major bug fix details
- Fixed worker recovery so sandbox restarts can reconnect without rotating existing CCAG tokens.
...
#### One-line summary
Expands cloud billing into a self-serve management flow and lets Slack and Telegram carry richer CCAG Router messages.
...
- Added first-class media transport for Slack and Telegram in CCAG Router.
...
#### Main changes
- Added Settings actions to restart orchestrator, OpenCode, CCAG server, and OpenCodeRouter.
...
#### Release importance
Major release: substantially improves how users run and monitor long CCAG sessions through rendering, status, and compaction changes across core app surfaces.
...
#### Main changes
Keeps the bundled OpenCode fallback pinned to the same version across CI, prerelease, and release artifacts so packaged builds drift less, without introducing new user-facing CCAG workflows.
...
#### Title
CCAG Share turns dropped files into worker packages

#### One-line summary
Adds a real worker-packaging flow in CCAG Share, rebuilds the share site on the Next.js App Router, and makes provider connections easier to manage.

#### Main changes
- CCAG Share now packages dropped skills, agents, commands, and MCP or CCAG config into worker bundles.
...
#### Major improvement details
- Turned CCAG Share into a worker packager.
- Replatformed CCAG Share onto the Next.js App Router.
...
#### Release importance
Minor release: fixes import targeting and worker startup clarity without materially changing CCAG's overall product shape.
...
- CCAG Share adds inline link-success feedback, and the Book a Call form gets clearer topic cards.
...
- Added inline success feedback and richer content handling on CCAG Share surfaces.
...
#### Release importance
Minor release: keeps release artifacts aligned for distribution without changing how users use CCAG.
...
#### Release importance
Minor release: adds a focused operator capability and several UX improvements without broadly reshaping the CCAG product.
...
#### Release importance
Minor release: extends sharing workflows and fixes setup friction without materially changing CCAG's overall architecture.
...
- Added a local Docker publisher flow for CCAG Share.
...
#### One-line summary
Turns Den signup into a calmer guided flow with clearer provisioning states while refocusing CCAG Share on publishing one skill well.

#### Main changes
- Rebuilt Den onboarding as a guided flow with clearer naming, intent, loading, and browser-access states.
- Simplified CCAG Share to publish a single skill, with cleaner frontmatter and fuller shared previews.
...
#### Release importance
Major release: substantially changes both Den onboarding and the CCAG Share publishing flow in ways users will immediately notice.
...
- Simplified CCAG Share to publish a single skill.
...
#### Release importance
Minor release: delivers focused session and settings polish without materially changing CCAG's broader workflows.
