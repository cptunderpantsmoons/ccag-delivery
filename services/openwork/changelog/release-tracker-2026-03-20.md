...
#### One-line summary
Gets desktop users to the right download path faster and lets `ccag-orchestrator` recover when npm skips its platform binary.
...
- `ccag-orchestrator` postinstall now downloads the matching release binary when optional platform packages are missing.
...
#### Title
Local workspaces move under CCAG server control

#### One-line summary
Shifts local workspace ownership into CCAG server so creation, reconnects, config writes, and starter bootstrap stay aligned across the app.

#### Main changes
- Local workspace create, rename, delete, config writes, and reload events now go through CCAG server first.
...
#### Major bug fix details
- Fixed local workspace reconnect and onboarding inconsistencies by moving workspace ownership into CCAG server.
...
#### Title
Team template sharing reaches CCAG Cloud
...
#### Release importance
Major release: adds substantial new Cloud collaboration and organization-management workflows that materially change how teams use CCAG.
...
#### Major improvement details
- Added Cloud team template sharing flows in the CCAG app.
...
#### Title
CCAG resumes where you left off
...
- Moved CCAG tokens off CLI args and logs, stopped auto-creating Welcome, and fixed collapsed sidebar session lists.
...
- Fixed bundle fetch routing so publish and fetch traffic stays pinned to the configured CCAG publisher.
...
#### Main changes
- Added an CCAG Cloud skills catalog to the Skills page, with install and share-to-team flows.
...
#### Major improvement details
- Added the CCAG Cloud team skills catalog on the app Skills page.
