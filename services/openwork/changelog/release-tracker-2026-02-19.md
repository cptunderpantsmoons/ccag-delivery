...
#### Main changes
Added migration repair from onboarding and Settings so broken local startup can recover without leaving CCAG.
...
#### One-line summary
Clarifies when repair is actually available in the app and resets the landing page back to broader CCAG messaging.

#### Main changes
- Added disabled-state feedback and clearer reasons when migration recovery is unavailable in onboarding and Settings.
- Reverted the homepage copy from a workers-heavy framing back to the broader CCAG message.
...
#### Release importance
Minor release: adds a meaningful sharing improvement and reliability fixes without materially reshaping how CCAG works overall.
...
#### One-line summary
Keeps automations hidden until the scheduler is installed and lets CCAG pick up domain-organized skills plus machine-level MCP servers.
...
#### One-line summary
Hardens updater metadata generation and share-service deploy behavior, with no visible CCAG app or workflow changes.
...
#### One-line summary
Republishes synchronized package and desktop version metadata, with no intended CCAG app, server, or workflow changes.
...
#### Title
CCAG Cloud worker setup and reconnect flows
...
#### Main changes
Adds the first full CCAG Cloud worker setup flow in the web app.
...
#### Release importance
Major release: introduces CCAG Cloud worker provisioning and connect flows that materially change how users can start and use remote workers.
...
#### Major improvement details
- Added the Den control plane with real Render-backed cloud workers inside CCAG.
...
#### One-line summary
Turns the hosted worker page into a simpler list-detail picker and adds one-click desktop handoff into CCAG's remote connect flow.

#### Main changes
- Reworked hosted workers into a clearer list-detail connect view with status and action panels.
- Added `ccag://connect-remote` deep links so the desktop app can open remote-connect details directly.
...
#### Main changes
Hosted CCAG now hands people straight from the web into the app for connect and import flows.
...
#### Release importance
Major release: substantially expands how users sign up, connect, share, and navigate CCAG across hosted and desktop flows.
...
- Public bundle pages now look and read like CCAG, with clearer import flows outside the app.
- Settings adds `Restart local server` so local recovery no longer means leaving CCAG.
...
- Redesigned the share modal and generated bundle page styling to match CCAG’s product identity.
