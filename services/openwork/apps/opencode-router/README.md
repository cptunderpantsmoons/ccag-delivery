...
```bash
curl -fsSL https://raw.githubusercontent.com/different-ai/ccag/dev/apps/opencode-router/install.sh | bash
```
...
- `OPENCODE_ROUTER_HEALTH_PORT` controls the port (CCAG defaults to a random free port when using `ccag`).
...
- SQLite at `~/.ccag/opencode-router/opencode-router.db` unless overridden.
- Config stored at `~/.ccag/opencode-router/opencode-router.json` (created by `opencode-router` or `pnpm -C apps/opencode-router setup`).
