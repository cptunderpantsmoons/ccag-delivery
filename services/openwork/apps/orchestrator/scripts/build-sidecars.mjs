...
if (!orchestratorVersion) {
  throw new Error(
    "ccag-orchestrator version missing in apps/orchestrator/package.json",
  );
}
...
if (!serverVersion) {
  throw new Error(
    "ccag-server version missing in apps/server/package.json",
  );
}
...
run("pnpm", ["--filter", "ccag-server", "build:bin:all"], repoRoot);
...
const entries = {
  "ccag-server": { version: serverVersion, targets: {} },
  "opencode-router": { version: routerVersion, targets: {} },
};

for (const target of targets) {
  const ext = target.id.startsWith("windows") ? ".exe" : "";
  const serverSrc = join(serverDir, `ccag-server-${target.bun}${ext}`);
  if (!existsSync(serverSrc)) {
    throw new Error(`Missing ccag-server binary at ${serverSrc}`);
  }
  const serverDest = join(outdir, `ccag-server-${target.id}${ext}`);
  ...
  entries["ccag-server"].targets[target.id] = {
...
writeFileSync(
  join(outdir, "ccag-orchestrator-sidecars.json"),
