...
const serverBin = resolve(root, "..", "server", "dist", "bin", "ccag-server");
...
await copyFile(serverBin, resolve(targetDir, "ccag-server"));
...
const versions = {
  "ccag-server": {
    ...
    sha256: await sha256(resolve(targetDir, "ccag-server")),
  },
  "opencode-router": {
