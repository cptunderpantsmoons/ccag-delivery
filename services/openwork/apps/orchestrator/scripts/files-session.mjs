...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `ccag CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "ccag-file-session-"));
...
try {
  await waitFor(`${ccagUrl}/health`);

  const workspaces = await fetchJson(`${ccagUrl}/workspaces`, {
  ...
  assert.equal(diskContent, "updated by ccag cli\n");
  ...
  console.log(JSON.stringify({ ok: true, ccagUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
