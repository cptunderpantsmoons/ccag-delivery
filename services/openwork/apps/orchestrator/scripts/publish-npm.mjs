...
if (!version) {
  throw new Error("ccag-orchestrator version missing in apps/orchestrator/package.json")
}
...
for (const target of targets) {
  ...
  const src = join(root, "dist", "bin", `ccag-${target.bun}${ext}`)
  if (!existsSync(src)) {
    throw new Error(`Missing ccag binary at ${src}. Run: pnpm --filter ccag-orchestrator build:bin:all`)
  }
  ...
  const dest = join(bindir, `ccag${ext}`)
  ...
  writeJson(join(dir, "package.json"), {
    name,
    version,
    description: "Platform binary for ccag-orchestrator",
    license: "MIT",
    os: [target.os],
    cpu: [target.cpu],
    bin: {
      ccag: `./bin/ccag${ext}`,
      "ccag-orchestrator": `./bin/ccag${ext}`,
    },
    files: ["bin"],
  })
...
const meta = join(outroot, "ccag-orchestrator")
mkdirSync(join(meta, "bin"), { recursive: true })

const wrapperSrc = join(root, "bin", "ccag")
if (!existsSync(wrapperSrc)) {
  throw new Error(`Missing wrapper at ${wrapperSrc}`)
}
copyFileSync(wrapperSrc, join(meta, "bin", "ccag"))
chmodSync(join(meta, "bin", "ccag"), 0o755)
...
writeJson(join(meta, "package.json"), {
  name: "ccag-orchestrator",
  version,
  description: "CCAG host orchestrator for opencode + CCAG server + opencode-router",
  license: "MIT",
  bin: {
    ccag: "./bin/ccag",
    "ccag-orchestrator": "./bin/ccag",
  },
  scripts: {
...
published.push({ name: "ccag-orchestrator", dir: meta })
