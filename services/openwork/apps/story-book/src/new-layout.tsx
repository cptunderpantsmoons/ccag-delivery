...
import type { CcagServerClient } from "../../app/src/app/lib/ccag-server";
...
const remoteWorkspace = storyWorkspaces[1] ?? {
  ...
  remoteType: "ccag" as const,
  baseUrl: "https://worker.openworklabs.com/opencode",
  ccagHostUrl: "https://worker.openworklabs.com",
  ccagWorkspaceName: "Ops Worker",
  sandboxBackend: "docker" as const,
  sandboxContainerName: "ccag-ops-worker",
};
...
const initialScheduledJobs: ScheduledJob[] = [
  {
    ...
    schedule: "30 9 * * 1,3,5",
    workdir: "/Users/benjaminshafii/ccag-enterprise/_repos/ccag",
  ...
  },
  {
    ...
    run: {
      command: "pnpm",
      arguments: "--filter @ccag/app test remote-onboarding",
      agent: "ccag-factory",
...
const initialHubSkills: HubSkillCard[] = [
  {
    ...
    source: { owner: "different-ai", repo: "ccag-hub", ref: "main", path: "skills/worker-smoke/SKILL.md" },
  },
  {
    name: "share-review",
    description: "Review share links, field labeling, and copy clarity.",
    trigger: "share review",
    source: { owner: "different-ai", repo: "ccag-hub", ref: "main", path: "skills/share-review/SKILL.md" },
  },
];
...
export default function NewLayoutApp() {
  ...
  const [pluginList, setPluginList] = createSignal<string[]>(["opencode-scheduler", "@ccag/browser-mcp"]);
  ...
  const tabDescription = (tab: SettingsTab) => {
    switch (tab) {
      case "den":
        return "Manage your CCAG Cloud connection, hosted workers, and workspace access.";
      case "model":
      ...
      case "appearance":
        return "Adjust how CCAG looks across desktop, system theme, and app chrome.";
      case "updates":
      ...
      default:
        return "Connect providers, authorize folders, and control the active CCAG workspace.";
    }
  };
  ...
  const buildRouterHealth = () => ({
    ...
    agent: {
      ...
      loaded: true,
      selected: "ccag-router",
    },
  });

  const mockCcagServerClient = {
    getConfig: async () => ({ ccag: { messaging: { enabled: messagingModuleEnabled() } } }),
    ...
    getOpenCodeRouterTelegram: async () => ({
      ...
      bot: { id: 1, username: "ccag_storybot", name: "CCAG Story Bot" },
    }),
    ...
    patchConfig: async (
      _workspaceId: string,
      payload: { ccag?: { messaging?: { enabled?: boolean } } },
    ) => {
      const enabled = payload.ccag?.messaging?.enabled;
  ...
  } as unknown as CcagServerClient;
  ...
  const pickMockWorkspaceFolder = async () => {
    const folders = [
      "/Users/demo/CCAG/client-foundation",
      "/Users/demo/CCAG/automation-lab",
      "/Users/demo/CCAG/starter-sandbox",
    ];
                ...
                <Show
                  ...
                  fallback={
                    <MessageList
                      ...
                      workspaceRoot="/Users/benjaminshafii/ccag-enterprise/_repos/ccag"
                    />
                  }
                >
                    ...
                    <div class="min-w-0 space-y-6">
                      ...
                      <Show when={settingsTab() === "skills"}>
                        <div class="rounded-[20px] border border-dls-border bg-dls-surface p-6 text-[14px] text-dls-secondary">
                          ...
                          Use the CCAG app shell to exercise this surface.
                        </div>
                      </Show>
                      ...
                      <Show when={settingsTab() === "messaging"}>
                        <div class="space-y-4">
                          <IdentitiesView
                            busy={false}
                            ccagServerStatus="connected"
                            ccagServerUrl="https://worker.openworklabs.com/opencode"
                            ccagServerClient={mockCcagServerClient}
                            ccagReconnectBusy={false}
                            reconnectCcagServer={async () => true}
