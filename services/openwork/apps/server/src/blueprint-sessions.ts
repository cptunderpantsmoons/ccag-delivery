...
export function normalizeBlueprintSessionTemplates(ccag: Record<string, unknown> | null | undefined): BlueprintSessionTemplate[] {
  const blueprint = readRecord(ccag?.blueprint);
...
export function readMaterializedBlueprintSessions(ccag: Record<string, unknown> | null | undefined): MaterializedBlueprintSession[] {
  const blueprint = readRecord(ccag?.blueprint);
...
export function sanitizeCcagTemplateConfig(ccag: Record<string, unknown> | null | undefined): Record<string, unknown> {
  const next = cloneRecord(ccag ?? {});
...
export function applyMaterializedBlueprintSessions(
  ccag: Record<string, unknown> | null | undefined,
  items: MaterializedBlueprintSession[],
  hydratedAt: number,
): Record<string, unknown> {
  const next = sanitizeCcagTemplateConfig(ccag);
  const blueprint = readRecord(next.blueprint) ?? {};
  const materialized = readRecord(blueprint.materialized) ?? {};
