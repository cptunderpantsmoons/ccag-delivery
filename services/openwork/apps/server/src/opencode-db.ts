...
const DEFAULT_AGENT = "ccag";
const DEFAULT_PROVIDER = "openai";
const DEFAULT_MODEL = "gpt-5.4";
const CCAG_DEV_DATA_DIRS = ["ccag-dev-data", "opencode-dev"];
...
function opencodeOrchestratorDataDirs(): string[] {
  const root = process.env.CCAG_DATA_DIR?.trim();
  if (!root) return [];
  ...
  for (const name of CCAG_DEV_DATA_DIRS) {
