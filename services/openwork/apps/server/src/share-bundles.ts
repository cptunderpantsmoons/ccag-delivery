...
const DEFAULT_PUBLISHER_BASE_URL = String(process.env.CCAG_PUBLISHER_BASE_URL ?? "").trim() || "https://share.openworklabs.com";
const DEFAULT_PUBLISHER_ORIGIN = String(process.env.CCAG_PUBLISHER_REQUEST_ORIGIN ?? "").trim() || "https://app.ccag.software";
...
export function resolveTrustedSharedBundleFetchUrl(bundleUrl: unknown): URL {
  ...
  if (inputUrl.origin !== trustedBaseUrl.origin) {
    throw new ApiError(
      ...
      `Shared bundle URLs must use the configured CCAG publisher (${trustedBaseUrl.origin}). Import only bundles from trusted sources.`,
    );
  }
  ...
  try {
    const response = await fetch(`${baseUrl}/v1/bundles`, {
      ...
      headers: {
        ...
        ...(input.name?.trim() ? { "X-CCAG-Name": input.name.trim() } : {}),
      },
