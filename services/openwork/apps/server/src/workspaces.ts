...
export function workspaceIdForOpenwork(hostUrl: string, workspaceId?: string | null): string {
  ...
  const key = normalizedWorkspaceId
    ? `ccag::${normalizedHostUrl}::${normalizedWorkspaceId}`
    : `ccag::${normalizedHostUrl}`;
  return workspaceIdForKey(key);
}
  ...
  return workspaces.map((workspace) => {
    ...
    const id = workspace.id?.trim()
      || (workspaceType === "remote"
        ? remoteType === "ccag"
          ? workspaceIdForOpenwork(workspace.ccagHostUrl ?? workspace.baseUrl ?? "", workspace.ccagWorkspaceId)
    ...
    return {
      ...
      ccagHostUrl: workspace.ccagHostUrl,
      ccagToken: workspace.ccagToken,
      ccagWorkspaceId: workspace.ccagWorkspaceId,
      ccagWorkspaceName: workspace.ccagWorkspaceName,
