...
function resolveCcagDataDir(): string {
  const override = process.env.CCAG_DATA_DIR?.trim();
  if (override) return expandHome(override);
  return join(homedir(), ".ccag", "ccag-server");
}

export function auditLogPath(workspaceId: string): string {
  return join(resolveCcagDataDir(), "audit", `${workspaceId}.jsonl`);
}

export function legacyAuditLogPath(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "ccag", "audit.jsonl");
}
