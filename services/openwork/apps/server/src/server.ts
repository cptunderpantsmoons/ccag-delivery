...
import { opencodeConfigPath, ccagConfigPath, projectCommandsDir, projectSkillsDir } from "./workspace-files.js";
...
export function createServerLogger(config: ServerConfig): ServerLogger {
  const runId = process.env.CCAG_RUN_ID ?? shortId();
    ...
    fetch: async (request: Request) => {
      ...
      try {
        ...
        if (!(error instanceof ApiError)) {
          console.error("[ccag-server] Unhandled error:", error);
        }
        const apiError = error instanceof ApiError
          ? error
          : new ApiError(500, "internal_error", "Unexpected server error");
...
async function proxyOpencodeRequest(input: {
  ...
  headers.delete("x-ccag-host-token");
  headers.delete("x-ccag-client-id");
  headers.delete("host");
...
function opencodeRouterDebugEnabled(): boolean {
  return ["1", "true", "yes"].includes((process.env.CCAG_DEBUG_OPENCODE_ROUTER ?? "").toLowerCase());
}
...
function withCors(response: Response, request: Request, config: ServerConfig) {
  ...
  headers.set(
    ...
    "Authorization, Content-Type, X-CCAG-Host-Token, X-CCAG-Client-Id, X-OpenCode-Directory, X-Opencode-Directory, x-opencode-directory",
  );
...
async function requireClient(request: Request, config: ServerConfig, tokens: TokenService): Promise<Actor> {
  ...
  const clientId = request.headers.get("x-ccag-client-id") ?? undefined;
...
async function requireHost(request: Request, config: ServerConfig, tokens: TokenService): Promise<Actor> {
  const hostToken = request.headers.get("x-ccag-host-token");
...
function buildCapabilities(config: ServerConfig): Capabilities {
  ...
  return {
    ...
    skills: { read: true, write: writeEnabled, source: "ccag" },
    hub: {
      skills: {
        read: true,
        install: writeEnabled,
        repo: { owner: "different-ai", name: "ccag-hub", ref: "main" },
      },
    },
    ...
    toolProviders: {
      ...
      files: {
        ...
        inboxPath: ".opencode/ccag/inbox/",
        outboxPath: ".opencode/ccag/outbox/",
        maxBytes,
      },
    },
  };
...
function resolveSandboxBackend(): Capabilities["sandbox"]["backend"] {
  const raw = (process.env.CCAG_SANDBOX_BACKEND ?? "").trim().toLowerCase();
...
function resolveSandboxEnabled(backend: Capabilities["sandbox"]["backend"]): boolean {
  const raw = (process.env.CCAG_SANDBOX_ENABLED ?? "").trim().toLowerCase();
...
function resolveInboxEnabled(): boolean {
  const raw = (process.env.CCAG_INBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveOutboxEnabled(): boolean {
  const raw = (process.env.CCAG_OUTBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveInboxMaxBytes(): number {
  const raw = (process.env.CCAG_INBOX_MAX_BYTES ?? "").trim();
...
function resolveToyUiEnabled(): boolean {
  const raw = (process.env.CCAG_TOY_UI ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

// Dev-only log sink target. When CCAG_DEV_LOG_FILE is set to a path, the
...
function resolveDevLogPath(): string | null {
  const raw = (process.env.CCAG_DEV_LOG_FILE ?? "").trim();
  return raw.length > 0 ? raw : null;
}

function resolveBrowserProvider(): Capabilities["toolProviders"]["browser"] {
  const raw = (process.env.CCAG_BROWSER_PROVIDER ?? "").trim().toLowerCase();
...
function resolveInboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "ccag", "inbox");
}

function resolveOutboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "ccag", "outbox");
}
...
function createRoutes(
  ...
  // tokens; it is also a no-op when CCAG_DEV_LOG_FILE is unset.
  ...
  addRoute(routes, "GET", "/ui/assets/ccag-mark.svg", "none", async () => {
    if (!resolveToyUiEnabled()) {
      throw new ApiError(404, "ui_disabled", "Toy UI is disabled");
    }
  ...
  addRoute(routes, "DELETE", "/workspaces/:id", "host", async (ctx) => {
    ...
    await recordAudit(workspace.path, {
      ...
      target: "workspace",
      summary: "Deleted workspace from CCAG server",
      timestamp: Date.now(),
    });
  ...
  addRoute(routes, "GET", "/workspace/:id/config", "client", async (ctx) => {
    ...
    const ccag = await readCcagConfig(workspace.path);
    const lastAudit = await readLastAudit(workspace.path, workspace.id);
    return jsonResponse({ opencode, ccag, updatedAt: lastAudit?.timestamp ?? null });
  });
  ...
  addRoute(routes, "PATCH", "/workspace/:id/config", "client", async (ctx) => {
    ...
    const ccag = body.ccag as Record<string, unknown> | undefined;

    if (!opencode && !ccag) {
      throw new ApiError(400, "invalid_payload", "opencode or ccag updates required");
    }
    ...
    await requireApproval(ctx, {
      ...
      paths: [opencode ? opencodeConfigPath(workspace.path) : null, ccag ? ccagConfigPath(workspace.path) : null].filter(Boolean) as string[],
    });
    ...
    if (ccag) {
      await writeCcagConfig(workspace.path, ccag, true);
    }
  ...
  addRoute(routes, "POST", "/share/bundles/publish", "client", async (ctx) => {
    ...
    if (typeof body.baseUrl === "string" && body.baseUrl.trim()) {
      throw new ApiError(
        ...
        "Bundle publishing always uses the configured CCAG publisher. Remove baseUrl from the request.",
      );
    }
...
function resolveOpenCodeRouterConfigPath(): string {
  ...
  const dataDir = process.env.OPENCODE_ROUTER_DATA_DIR?.trim() || join(homedir(), ".ccag", "opencode-router");
...
type CcagServerConfigFile = Record<string, unknown> & {
  workspaces?: Array<Record<string, unknown>>;
  authorizedRoots?: string[];
};

async function readServerConfigFile(configPath: string): Promise<CcagServerConfigFile> {
  ...
  try {
    ...
    return ensurePlainObject(JSON.parse(raw)) as CcagServerConfigFile;
  } catch (error) {
...
function serializeWorkspaceConfigEntry(workspace: WorkspaceInfo): Record<string, unknown> {
  return {
    ...
    ...(workspace.ccagHostUrl ? { ccagHostUrl: workspace.ccagHostUrl } : {}),
    ...(workspace.ccagToken ? { ccagToken: workspace.ccagToken } : {}),
    ...(workspace.ccagWorkspaceId ? { ccagWorkspaceId: workspace.ccagWorkspaceId } : {}),
    ...(workspace.ccagWorkspaceName ? { ccagWorkspaceName: workspace.ccagWorkspaceName } : {}),
...
async function persistServerWorkspaceState(config: ServerConfig): Promise<boolean> {
  ...
  const next: CcagServerConfigFile = {
    ...parsed,
...
function getRuntimeControlConfig(): { baseUrl: string; token: string } | null {
  const baseUrl = process.env.CCAG_CONTROL_BASE_URL?.trim() ?? "";
  const token = process.env.CCAG_CONTROL_TOKEN?.trim() ?? "";
...
async function readCcagConfig(workspaceRoot: string): Promise<Record<string, unknown>> {
  const path = ccagConfigPath(workspaceRoot);
  if (!(await exists(path))) return {};
  try {
    const raw = await readFile(path, "utf8");
    return JSON.parse(raw) as Record<string, unknown>;
  } catch {
    throw new ApiError(422, "invalid_json", "Failed to parse ccag.json");
  }
}
...
async function writeCcagConfig(workspaceRoot: string, payload: Record<string, unknown>, merge: boolean): Promise<void> {
  const path = ccagConfigPath(workspaceRoot);
  const next = merge ? { ...(await readCcagConfig(workspaceRoot)), ...payload } : payload;
...
async function exportWorkspace(
  ...
  const ccag = sanitizeCcagTemplateConfig(await readCcagConfig(workspace.path));
...
async function importWorkspace(workspace: WorkspaceInfo, payload: Record<string, unknown>): Promise<void> {
  ...
  const ccag = payload.ccag as Record<string, unknown> | undefined;
  ...
  if (ccag) {
    const sanitizedOpenwork = sanitizeCcagTemplateConfig(ccag);
    if (modes.ccag === "replace") {
      await writeCcagConfig(workspace.path, sanitizedOpenwork, false);
    } else {
      await writeCcagConfig(workspace.path, sanitizedOpenwork, true);
    }
  }
...
async function materializeBlueprintSessions(config: ServerConfig, workspace: WorkspaceInfo): Promise<{
  ...
  const ccag = await readCcagConfig(workspace.path);
  const templates = normalizeBlueprintSessionTemplates(ccag);
  if (!templates.length) {
    return { ok: true, created: [], existing: [], openSessionId: null };
  }

  const existing = readMaterializedBlueprintSessions(ccag);
  ...
  await writeCcagConfig(workspace.path, nextOpenwork, false);
