...
<html lang="en">
  <head>
    ...
    <title>CCAG Toy UI</title>
    <link rel="icon" type="image/svg+xml" href="/ui/assets/ccag-mark.svg" />
  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div class="title">
          <h1>CCAG Toy UI</h1>
          <div class="sub">Local-first host contract harness (served by ccag-server)</div>
        </div>
        <div class="row">
        ...
        <div class="card">
          ...
          <div class="body">
            ...
            <div class="panel hidden" data-panel="config">
              ...
              <div class="small">Uploads go to <span class="mono">.opencode/ccag/inbox/</span> inside the workspace.</div>

              <div class="hr"></div>

              <div class="row">
                <button class="btn" id="btn-artifacts">List artifacts</button>
                <span class="small">Downloads read from <span class="mono">.opencode/ccag/outbox/</span>.</span>
              </div>
...
const STORAGE_TOKEN = "ccag.toy.token";
const STORAGE_SESSION_PREFIX = "ccag.toy.session.";
...
async function ensureSession(workspaceId) {
  ...
  const created = await apiFetch("/w/" + encodeURIComponent(workspaceId) + "/opencode/session", {
    method: "POST",
    body: JSON.stringify({ title: "CCAG Toy UI" }),
  });
...
async function main() {
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's CCAG server config?")) return;
    try {
