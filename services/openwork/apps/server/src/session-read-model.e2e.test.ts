...
async function createWorkspaceRoot() {
  const root = await mkdtemp(join(tmpdir(), "ccag-session-read-"));
...
function startCcagServer(input: { workspaceRoot: string; opencodeBaseUrl: string }) {
...
describe("workspace session read APIs", () => {
  ...
  test("returns 404 when the upstream session is missing", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${ccag.server.port}/workspace/ws_1/sessions/ses_missing/snapshot`, {
      headers: auth(ccag.token),
    });
  ...
  test("returns 502 when OpenCode returns an invalid session list payload", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${ccag.server.port}/workspace/ws_1/sessions`, {
      headers: auth(ccag.token),
    });
