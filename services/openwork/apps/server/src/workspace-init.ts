...
import { ccagConfigPath, opencodeConfigPath, projectCommandsDir, projectSkillsDir } from "./workspace-files.js";
...
async function ensureCcagAgent(workspaceRoot: string): Promise<void> {
  const agentsDir = join(workspaceRoot, ".opencode", "agents");
  const agentPath = join(agentsDir, "ccag.md");
  if (await exists(agentPath)) return;
  await ensureDir(agentsDir);
  await writeFile(agentPath, CCAG_AGENT.endsWith("\n") ? CCAG_AGENT : `${CCAG_AGENT}\n`, "utf8");
}
...
async function ensureWorkspaceCcagConfig(workspaceRoot: string, preset: string): Promise<void> {
  const path = ccagConfigPath(workspaceRoot);
  if (await exists(path)) return;
  const now = Date.now();
  const config: WorkspaceCcagConfig = {
    version: 1,
    workspace: {
