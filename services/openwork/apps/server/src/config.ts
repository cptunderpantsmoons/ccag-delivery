...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const envConfigPath = process.env.CCAG_SERVER_CONFIG;
  const configPath = cli.configPath ?? envConfigPath ?? resolve(homedir(), ".config", "ccag", "server.json");
  const fileConfig = await loadFileConfig(configPath);
  const configDir = dirname(configPath);

  const envWorkspaces = parseList(process.env.CCAG_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.CCAG_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.CCAG_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.CCAG_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.CCAG_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.CCAG_TOKEN;
  const hostTokenFromEnv = process.env.CCAG_HOST_TOKEN;
  ...
  const approvalTimeoutMs =
    ...
    (process.env.CCAG_APPROVAL_TIMEOUT_MS ? Number(process.env.CCAG_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.CCAG_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.CCAG_READONLY;
  ...
  const envLogFormat = process.env.CCAG_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.CCAG_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.CCAG_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.CCAG_PORT ? Number(process.env.CCAG_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
