...
function readStoredList(): string[] {
  ...
  try {
    const raw = window.localStorage.getItem("ccag.server.list");
...
function readStoredActive(): string {
  if (typeof window === "undefined") return "";
  try {
    const stored = window.localStorage.getItem("ccag.server.active");
...
function readCcagToken(): string {
  if (typeof window === "undefined") return "";
  try {
    return (window.localStorage.getItem("ccag.server.token") ?? "").trim();
  } catch {
    return "";
  }
}
...
export function ServerProvider({ children, defaultUrl }: ServerProviderProps) {
  ...
  useEffect(() => {
    ...
    // Hosted web deployments served by CCAG must reuse the OpenCode proxy
    ...
    const forceProxy =
      ...
      (import.meta.env.PROD ||
        (typeof import.meta.env?.VITE_CCAG_URL === "string" &&
          import.meta.env.VITE_CCAG_URL.trim().length > 0));
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("ccag.server.list", JSON.stringify(list));
      window.localStorage.setItem("ccag.server.active", active);
    } catch {
      // ignore
    }
  }, [active, list]);
