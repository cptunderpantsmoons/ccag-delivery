...
export function createAutomationsStore(options: CreateAutomationsStoreOptions) {
  ...
  let disposed = false;
  let started = false;
  let ccagServerUnsubscribe: (() => void) | null = null;
  let lastContextKey = "";
  ...
  const getServerSnapshot = () => options.ccagServer.getSnapshot();

  const getServerBacked = () => {
    const ccagSnapshot = getServerSnapshot();
    const runtimeWorkspaceId = (options.runtimeWorkspaceId() ?? "").trim();
    return (
      ccagSnapshot.ccagServerStatus === "connected" &&
      Boolean(ccagSnapshot.ccagServerClient && runtimeWorkspaceId)
    );
  };
  ...
  const refreshScheduledJobs = async (
    ...
    if (getScheduledJobsSource() === "remote") {
      const ccagSnapshot = getServerSnapshot();
      const client = ccagSnapshot.ccagServerClient;
      const workspaceId = (options.runtimeWorkspaceId() ?? "").trim();
      if (!client || ccagSnapshot.ccagServerStatus !== "connected" || !workspaceId) {
        if (getScheduledJobsContextKey() !== requestContextKey) return "skipped";
        const status =
          ccagSnapshot.ccagServerStatus === "disconnected"
            ? t("automations.server_unavailable")
            : ccagSnapshot.ccagServerStatus === "limited"
  ...
  const start = () => {
    ...
    ccagServerUnsubscribe = options.ccagServer.subscribe(() => {
      if (disposed) return;
      emitChange();
    });
    syncFromOptions();
  ...
  const dispose = () => {
    ...
    disposed = true;
    started = false;
    ccagServerUnsubscribe?.();
    ccagServerUnsubscribe = null;
    listeners.clear();
  };
