...
export type DebugViewProps = {
  ...
  engineRuntime: "direct" | "ccag-orchestrator";
  onSetEngineRuntime: (value: "direct" | "ccag-orchestrator") => void;
  onOpenResetModal: (mode: "onboarding" | "all") => void;
  resetModalBusy: boolean;
  ccagRestartBusy: boolean;
  opencodeRestarting: boolean;
  ccagServerRestarting: boolean;
  opencodeRouterRestarting: boolean;
  ccagRestartStatus: string | null;
  serviceRestartError: string | null;
  onRestartLocalServer: () => void | Promise<void>;
  onRestartOpencode: () => void | Promise<void>;
  onRestartCcagServer: () => void | Promise<void>;
  ...
  onNukeCcagAndOpencodeConfig: () => void | Promise<void>;
};

function formatActor(entry: CcagAuditEntry) {
      ...
      <div className="space-y-4">
        <div className={settingsCardClass}>
          ...
          <div className="grid gap-2 text-xs text-gray-11 md:grid-cols-2">
            ...
            <div>
              {t("settings.debug_ccag_server_version", undefined, {
                version: props.runtimeSummary.ccagServerVersionLabel,
              })}
            </div>
            <div>
            ...
            {props.developerMode ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button
                    variant={props.engineRuntime === "ccag-orchestrator" ? "secondary" : "outline"}
                    onClick={() => props.onSetEngineRuntime("ccag-orchestrator")}
                    disabled={props.busy}
                  >
        ...
        <div className={settingsCardClass}>
          ...
          <div className={`${settingsSoftCardClass} space-y-3`}>
            ...
            <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
              <Button
                ...
                disabled={props.busy || props.ccagRestartBusy || !isDesktop}
                className="justify-center px-3 py-1.5 text-xs"
              >
                <RefreshCcw className={`mr-1.5 h-3.5 w-3.5 ${props.ccagRestartBusy ? "animate-spin" : ""}`} />
                {props.ccagRestartBusy ? t("settings.restarting") : t("settings.restart_orchestrator")}
              </Button>
              <Button
                variant="secondary"
              ...
              <Button
                variant="secondary"
                onClick={() => void props.onRestartCcagServer()}
                disabled={props.ccagServerRestarting || !isDesktop}
                className="justify-center px-3 py-1.5 text-xs"
              >
                <RefreshCcw className={`mr-1.5 h-3.5 w-3.5 ${props.ccagServerRestarting ? "animate-spin" : ""}`} />
                {props.ccagServerRestarting ? t("settings.restarting") : t("settings.restart_ccag_server")}
              </Button>
              <Button
                variant="secondary"
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div className={`${settingsSoftCardClass} space-y-3`}>
              ...
              <div className="space-y-1">{renderLines([
                ...
                t("settings.debug_ccag_server_version", undefined, { version: props.runtimeSummary.ccagServerVersionLabel }),
            ...
            <div className={`${settingsSoftCardClass} space-y-3`}>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-medium text-gray-12">{t("settings.ccag_server_label")}</div>
                  <div className="text-xs text-gray-10">{t("settings.ccag_config_sidecar_desc")}</div>
                </div>
                <div className={`rounded-full border px-2 py-1 text-xs ${props.ccagCard.className}`}>
                  {props.ccagCard.label}
                </div>
              </div>
              <div className="space-y-1">{renderLines(props.ccagCard.lines)}</div>
              <div className="grid gap-2">
                <div>
                  <div className="mb-1 text-[11px] text-gray-9">{t("settings.last_stdout")}</div>
                  <pre className={settingsMiniPreClass}>{props.ccagCard.stdout || "—"}</pre>
                </div>
                <div>
                  <div className="mb-1 text-[11px] text-gray-9">{t("settings.last_stderr")}</div>
                  <pre className={settingsMiniPreClass}>{props.ccagCard.stderr || "—"}</pre>
                </div>
              </div>
            </div>
          ...
          <div className={`${settingsSoftCardClass} space-y-3`}>
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm font-medium text-gray-12">{t("settings.ccag_diagnostics_title")}</div>
              <div className="truncate font-mono text-[11px] text-gray-8">
                {props.ccagServerDiagnostics?.version ?? "—"}
              </div>
            </div>
            {props.ccagServerDiagnostics ? (
              <div className="grid gap-2 text-xs text-gray-11 md:grid-cols-2">
                <div>{t("settings.diag_started", undefined, { time: formatUptime(props.ccagServerDiagnostics.uptimeMs) })}</div>
                <div>
                  {t("settings.diag_read_only", undefined, {
                    value: props.ccagServerDiagnostics.readOnly ? "true" : "false",
                  })}
                </div>
                <div>
                  {t("settings.diag_approval", undefined, {
                    mode: props.ccagServerDiagnostics.approval.mode,
                    ms: String(props.ccagServerDiagnostics.approval.timeoutMs),
                  })}
                </div>
                <div>{t("settings.diag_workspaces", undefined, { count: String(props.ccagServerDiagnostics.workspaceCount) })}</div>
                <div>
                  {t("settings.diag_selected_workspace", undefined, {
                    id: props.ccagServerDiagnostics.selectedWorkspaceId ?? "—",
                  })}
                </div>
                <div>
                  {t("settings.diag_runtime_workspace", undefined, {
                    id: props.ccagServerDiagnostics.activeWorkspaceId ?? "—",
                  })}
                </div>
                <div>
                  {t("settings.diag_config_path", undefined, {
                    path: props.ccagServerDiagnostics.server.configPath ?? t("settings.diag_default"),
                  })}
                </div>
                <div>
                  {t("settings.diag_token_source", undefined, {
                    source: props.ccagServerDiagnostics.tokenSource.client,
                  })}
                </div>
                <div>
                  {t("settings.diag_host_token_source", undefined, {
                    source: props.ccagServerDiagnostics.tokenSource.host,
                  })}
                </div>
              </div>
            ) : (
          ...
          <div className={`${settingsSoftCardClass} space-y-3`}>
            ...
            {props.ccagServerCapabilities ? (
              <div className="grid gap-2 text-xs text-gray-11 md:grid-cols-2">
                <div>{t("settings.cap_skills", undefined, { value: formatCapability(props.ccagServerCapabilities.skills) })}</div>
                <div>{t("settings.cap_plugins", undefined, { value: formatCapability(props.ccagServerCapabilities.plugins) })}</div>
                <div>{t("settings.cap_mcp", undefined, { value: formatCapability(props.ccagServerCapabilities.mcp) })}</div>
                <div>{t("settings.cap_commands", undefined, { value: formatCapability(props.ccagServerCapabilities.commands) })}</div>
                <div>{t("settings.cap_config", undefined, { value: formatCapability(props.ccagServerCapabilities.config) })}</div>
                <div>
                  {t("settings.cap_proxy", undefined, {
                    value: props.ccagServerCapabilities.proxy?.opencodeRouter
                      ? t("settings.enabled")
                      : t("settings.disabled"),
                  })}
                </div>
                <div>
                  {t("settings.cap_browser_tools", undefined, {
                    value: (() => {
                      const browser = props.ccagServerCapabilities.toolProviders?.browser;
                ...
                <div>
                  {t("settings.cap_file_tools", undefined, {
                    value: (() => {
                      const files = props.ccagServerCapabilities.toolProviders?.files;
                ...
                <div>
                  {t("settings.cap_sandbox", undefined, {
                    value: props.ccagServerCapabilities.sandbox
                      ? `${props.ccagServerCapabilities.sandbox.backend} (${props.ccagServerCapabilities.sandbox.enabled ? t("settings.on") : t("settings.off")})`
          ...
          <div className={`${settingsSoftCardClass} space-y-3`}>
            <div className="flex items-center justify-between gap-3">
              ...
              <div className={`rounded-full border px-2 py-1 text-xs ${props.ccagAuditStatus.className}`}>
                {props.ccagAuditStatus.label}
              </div>
            </div>
            {props.ccagAuditError ? <div className="text-xs text-red-11">{props.ccagAuditError}</div> : null}
            {props.ccagAuditEntries.length > 0 ? (
              <div className="divide-y divide-gray-6/50">
                {props.ccagAuditEntries.map((entry) => (
        ...
        {isDesktop ? (
          <div className="space-y-4 rounded-2xl border border-red-7/30 bg-red-3/10 p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-sm font-medium text-gray-12">{t("settings.reset_ccag_title")}</div>
                <div className="text-xs text-gray-10">
                  {props.opencodeDevModeEnabled
                    ? t("settings.reset_ccag_desc_dev")
                    : t("settings.reset_ccag_desc_prod")}
                </div>
              </div>
              <div
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                onClick={() => void props.onNukeCcagAndOpencodeConfig()}
