...
import type { CcagHubRepo, CcagServerClient } from "../../../../app/lib/ccag-server";
import {
  createDenClient,
...
import type { CcagServerStore } from "../../connections/ccag-server-store";
...
const HUB_REPOS_STORAGE_KEY = "ccag.skills.hubRepos.v1";
...
export function createExtensionsStore(options: {
  ...
  let disposed = false;
  let started = false;
  let stopCcagSubscription: (() => void) | null = null;
  ...
  const readWorkspaceCcagConfigRecord = async (): Promise<Record<string, unknown>> => {
    const root = options.selectedWorkspaceRoot().trim();
    const isLocalWorkspace = options.workspaceType() === "local";
    const ccagSnapshot = options.ccagServer.getSnapshot();
    const ccagClient = ccagSnapshot.ccagServerClient;
    const ccagWorkspaceId = options.runtimeWorkspaceId();
    const canUseCcagServer =
      ccagSnapshot.ccagServerStatus === "connected" &&
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.config?.read;

    if (canUseCcagServer) {
      const config = await ccagClient.getConfig(ccagWorkspaceId);
      return config.ccag ?? {};
    }

    if (isLocalWorkspace && isDesktopRuntime() && root) {
      return await workspaceCcagRead({ workspacePath: root }) as unknown as Record<string, unknown>;
    }

    return {};
  };

  const writeWorkspaceCcagConfigRecord = async (config: Record<string, unknown>) => {
    const root = options.selectedWorkspaceRoot().trim();
    const isLocalWorkspace = options.workspaceType() === "local";
    const ccagSnapshot = options.ccagServer.getSnapshot();
    const ccagClient = ccagSnapshot.ccagServerClient;
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.config?.write;

    if (canUseCcagServer) {
      await ccagClient.patchConfig(ccagWorkspaceId, { ccag: config });
      return true;
    }

    if (isLocalWorkspace && isDesktopRuntime() && root) {
      const result = await workspaceCcagWrite({
        workspacePath: root,
        config: config as never,
      });
      if (!result.ok) {
        throw new Error(result.stderr || result.stdout || "Failed to write .opencode/ccag.json");
      }
      return true;
    }
  ...
  const refreshImportedCloudSkillHubs = async () => {
    try {
      const config = await readWorkspaceCcagConfigRecord();
  ...
  const persistImportedCloudSkillHubs = async (nextSkillHubs: Record<string, CloudImportedSkillHub>) => {
    ...
    const persisted = await writeWorkspaceCcagConfigRecord(nextConfig);
    if (!persisted) {
      throw new Error("CCAG server unavailable. Connect to manage imported cloud skill hubs.");
    }
  ...
  const persistImportedCloudSkills = async (nextSkills: Record<string, CloudImportedSkill>) => {
    ...
    const persisted = await writeWorkspaceCcagConfigRecord(nextConfig);
    if (!persisted) {
      throw new Error("CCAG server unavailable. Connect to manage imported cloud skills.");
    }
  ...
  const upsertWorkspaceSkill = async (
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.skills?.write;

    if (canUseCcagServer) {
      await ccagClient.upsertSkill(ccagWorkspaceId, {
        name,
        content,
        description,
      });
    ...
    if (isRemoteWorkspace) {
      throw new Error("CCAG server unavailable. Connect to import skills.");
    }
  ...
  const deleteWorkspaceSkill = async (name: string) => {
    ...
    if (canUseCcagServer) {
      await ccagClient.deleteSkill(ccagWorkspaceId, name);
      return;
    }

    if (isRemoteWorkspace) {
      throw new Error("CCAG server unavailable. Connect to remove skills.");
    }
  ...
  async function refreshHubSkills(optionsOverride?: { force?: boolean }) {
    ...
    const canUseCcagServer =
      ccagSnapshot.ccagServerStatus === "connected" &&
      ccagClient &&
      ccagSnapshot.ccagServerCapabilities?.hub?.skills?.read;
    ...
    try {
      ...
      if (canUseCcagServer) {
        const response = await ccagClient.listHubSkills({
          repo: {
            owner: repo.owner,
            repo: repo.repo,
            ref: repo.ref,
  ...
  async function installHubSkill(name: string): Promise<{ ok: boolean; message: string }> {
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.hub?.skills?.install;

    if (!canUseCcagServer) {
      if (isRemoteWorkspace) return { ok: false, message: "CCAG server unavailable. Connect to install skills." };
      return { ok: false, message: "Hub install requires CCAG server." };
    }
    ...
    try {
      const repoOverride: CcagHubRepo = { owner: repo.owner, repo: repo.repo, ref: repo.ref };
      const result = await ccagClient.installHubSkill(ccagWorkspaceId, trimmed, { repo: repoOverride });
      await refreshSkills({ force: true });
  ...
  async function refreshSkills(optionsOverride?: { force?: boolean }) {
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.skills?.read;
    ...
    if (canUseCcagServer) {
      ...
      try {
        setStateField("skillsStatus", null);
        const response = await ccagClient.listSkills(ccagWorkspaceId, { includeGlobal: isLocalWorkspace });
    ...
    if (!client) {
      mutateState((current) => ({
        ...current,
        skills: [],
        skillsStatus: "CCAG server unavailable. Connect to load skills.",
      }));
      return;
    }
  ...
  async function refreshPlugins(scopeOverride?: PluginScope) {
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.plugins?.read;
    ...
    if (scope === "project" && canUseCcagServer) {
      mutateState((current) => ({
        ...current,
        pluginConfig: null,
        pluginConfigPath: `opencode.json (${isRemoteWorkspace ? "remote" : "ccag"} server)`,
      }));

      try {
        mutateState((current) => ({ ...current, pluginStatus: null, sidebarPluginStatus: null }));
        if (refreshPluginsAborted) return;
        const result = await ccagClient.listPlugins(ccagWorkspaceId, { includeGlobal: false });
        if (refreshPluginsAborted) return;
    ...
    if (!isLocalWorkspace && !canUseCcagServer) {
      mutateState((current) => ({
        ...current,
        pluginStatus: "CCAG server unavailable. Connect to manage plugins.",
        pluginList: [],
        sidebarPluginStatus: "Connect an CCAG server to load plugins.",
        sidebarPluginList: [],
      }));
      refreshPluginsInFlight = false;
      return;
  ...
  async function addPlugin(pluginNameOverride?: string) {
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.plugins?.write;
    ...
    if (snapshot.pluginScope === "project" && canUseCcagServer) {
      try {
        setStateField("pluginStatus", null);
        await ccagClient.addPlugin(ccagWorkspaceId, pluginName);
    ...
    if (!isLocalWorkspace && !canUseCcagServer) {
      setStateField("pluginStatus", "CCAG server unavailable. Connect to manage plugins.");
      return;
    }
  ...
  async function removePlugin(pluginName: string) {
    ...
    if (snapshot.pluginScope === "project" && canUseCcagServer) {
      try {
        setStateField("pluginStatus", null);
        await ccagClient.removePlugin(ccagWorkspaceId, name);
  ...
  async function installSkillCreator(): Promise<{ ok: boolean; message: string }> {
    ...
    if (canUseCcagServer) {
      ...
      try {
        await ccagClient.upsertSkill(ccagWorkspaceId, { name: "skill-creator", content: skillCreatorTemplate });
    ...
    if (isRemoteWorkspace) {
      const message = "CCAG server unavailable. Connect to install skills.";
      setStateField("skillsStatus", message);
      return { ok: false, message };
    }
    if (!isDesktopRuntime()) {
  ...
  async function readSkill(name: string): Promise<{ name: string; path: string; content: string } | null> {
    ...
    if (canUseCcagServer) {
      try {
        setStateField("skillsStatus", null);
        const result = await ccagClient.getSkill(ccagWorkspaceId, trimmed, { includeGlobal: isLocalWorkspace });
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "CCAG server unavailable. Connect to view skills.");
      return null;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", translate("skills.desktop_required"));
  ...
  async function saveSkill(input: { name: string; content: string; description?: string }) {
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "CCAG server unavailable. Connect to edit skills.");
      return;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", translate("skills.desktop_required"));
  ...
  const start = () => {
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("ccag-den-session-updated", onDenSessionUpdated);
      stopDenSessionListener = () => window.removeEventListener("ccag-den-session-updated", onDenSessionUpdated);
    }

    stopCcagSubscription = options.ccagServer.subscribe(() => {
      syncFromOptions();
    });
  ...
  const dispose = () => {
    ...
    started = false;
    abortRefreshes();
    stopCcagSubscription?.();
    stopCcagSubscription = null;
