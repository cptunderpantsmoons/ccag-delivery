...
export function AuthorizedFoldersPanel(props: AuthorizedFoldersPanelProps) {
  ...
  const ccagServerReady = props.ccagServerStatus === "connected";
  const ccagServerWorkspaceReady = Boolean(props.runtimeWorkspaceId);
  const canReadConfig =
    ccagServerReady &&
    ccagServerWorkspaceReady &&
    (props.ccagServerCapabilities?.config?.read ?? false);
  const canWriteConfig =
    ccagServerReady &&
    ccagServerWorkspaceReady &&
    (props.ccagServerCapabilities?.config?.write ?? false);

  const authorizedFoldersHint = useMemo(() => {
    if (!ccagServerReady) return t("context_panel.server_disconnected");
    if (!ccagServerWorkspaceReady) return t("context_panel.no_server_workspace");
    ...
    return null;
  }, [canReadConfig, canWriteConfig, ccagServerReady, ccagServerWorkspaceReady]);
  ...
  useEffect(() => {
    const ccagClient = props.ccagServerClient;
    const ccagWorkspaceId = props.runtimeWorkspaceId;

    if (!ccagClient || !ccagWorkspaceId || !canReadConfig) {
  ...
  }, [canReadConfig, props.ccagServerClient, props.runtimeWorkspaceId]);

  const persistAuthorizedFolders = useCallback(async (nextFolders: string[]) => {
    const ccagClient = props.ccagServerClient;
    const ccagWorkspaceId = props.runtimeWorkspaceId;
    if (!ccagClient || !ccagWorkspaceId || !canWriteConfig) {
    ...
    try {
      const currentConfig = await ccagClient.getConfig(ccagWorkspaceId);
      ...
      await ccagClient.patchConfig(ccagWorkspaceId, {
        opencode: {
          permission: {
