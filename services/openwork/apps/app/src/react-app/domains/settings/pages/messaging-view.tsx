...
import { t } from "../../../../i18n";
import type {
  CcagOpenCodeRouterHealthSnapshot,
  CcagOpenCodeRouterIdentityItem,
  CcagOpenCodeRouterSendResult,
  CcagServerStatus,
...
export type MessagingViewProps = {
  ...
  health: CcagOpenCodeRouterHealthSnapshot | null;
  ...
  telegram: {
    identities: CcagOpenCodeRouterIdentityItem[];
  ...
  sendTest: {
    ...
    status: string | null;
    error: string | null;
    result: CcagOpenCodeRouterSendResult | null;
  };
  modals: {
...
export function MessagingView(props: MessagingViewProps) {
  const serverReady = props.ccagServerStatus === "connected";
  const scopedWorkspaceReady = Boolean(props.workspaceId?.trim());
  const workspaceScopeLabel =
    props.scopedCcagBaseUrl?.trim() || props.ccagServerUrl.trim() || t("identities.not_set");
        ...
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              disabled={props.busy || props.ccagReconnectBusy}
            >
              <RefreshCcw size={14} className={props.ccagReconnectBusy ? "animate-spin" : ""} />
