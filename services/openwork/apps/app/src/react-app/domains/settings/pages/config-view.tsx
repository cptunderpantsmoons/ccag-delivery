...
import type { CcagServerInfo } from "../../../../app/lib/desktop";
...
export type ConfigViewProps = {
  ...
  updateCcagServerSettings: (next: CcagServerSettings) => void;
  resetCcagServerSettings: () => void;
  testCcagServerConnection: (
    next: CcagServerSettings,
  ) => Promise<boolean>;
...
type CcagTestState = "idle" | "testing" | "success" | "error";

export function ConfigView(props: ConfigViewProps) {
  const [ccagUrl, setCcagUrl] = useState("");
  const [ccagToken, setCcagToken] = useState("");
  const [ccagTokenVisible, setCcagTokenVisible] = useState(false);
  const [ccagTestState, setCcagTestState] =
    useState<CcagTestState>("idle");
  const [ccagTestMessage, setCcagTestMessage] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    setCcagUrl(props.ccagServerSettings.urlOverride ?? "");
    setCcagToken(props.ccagServerSettings.token ?? "");
  }, [props.ccagServerSettings]);

  useEffect(() => {
    setCcagTestState("idle");
    setCcagTestMessage(null);
  }, [ccagUrl, ccagToken]);
  ...
  const buildCcagSettings = (): CcagServerSettings => ({
    ...props.ccagServerSettings,
    urlOverride: ccagUrl.trim() || undefined,
    token: ccagToken.trim() || undefined,
  });

  const hasCcagChanges = (() => {
    const currentUrl = props.ccagServerSettings.urlOverride ?? "";
    const currentToken = props.ccagServerSettings.token ?? "";
    return (
      ccagUrl.trim() !== currentUrl || ccagToken.trim() !== currentToken
    );
  })();
  ...
  const resolvedWorkspaceId = (() => {
    ...
    return parseCcagWorkspaceIdFromUrl(ccagUrl) ?? "";
  })();

  const resolvedWorkspaceUrl = (() => {
    const baseUrl = ccagUrl.trim();
    if (!baseUrl) return "";
    return buildCcagWorkspaceBaseUrl(baseUrl, resolvedWorkspaceId) ?? baseUrl;
  })();

  const hostInfo = props.ccagServerHostInfo;
  ...
  const diagnosticsBundleJson = useMemo(() => {
    const urlOverride = props.ccagServerSettings.urlOverride?.trim() ?? "";
    const token = props.ccagServerSettings.token?.trim() ?? "";
    ...
    const bundle = {
      ...
      ccagServer: {
        status: props.ccagServerStatus,
        url: props.ccagServerUrl,
        settings: {
    ...
    props.developerMode,
    props.ccagServerSettings.token,
    props.ccagServerSettings.urlOverride,
    props.ccagServerStatus,
    props.ccagServerUrl,
  ...
  const handleTestConnection = async () => {
    if (ccagTestState === "testing") return;
    const next = buildCcagSettings();
    props.updateCcagServerSettings(next);
    setCcagTestState("testing");
    setCcagTestMessage(null);
    try {
      const ok = await props.testCcagServerConnection(next);
      setCcagTestState(ok ? "success" : "error");
      setCcagTestMessage(
      ...
      setCcagTestState("error");
      setCcagTestMessage(message);
    }
  };
    ...
    <section className="space-y-6">
      ...
      <div className="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          ...
          <div
            className={`text-xs px-2 py-1 rounded-full border ${ccagStatusStyle}`}
          >
            {ccagStatusLabel}
          </div>
        </div>
        ...
        <div className="grid gap-3">
          <TextInput
            ...
            value={ccagUrl}
            onChange={(event) => setCcagUrl(event.currentTarget.value)}
          ...
          <label className="block">
            ...
            <div className="flex items-center gap-2">
              <input
                type={ccagTokenVisible ? "text" : "password"}
                value={ccagToken}
                onChange={(event) => setCcagToken(event.currentTarget.value)}
              ...
              <Button
                ...
                onClick={() => setCcagTokenVisible((prev) => !prev)}
                disabled={props.busy}
              >
                {ccagTokenVisible ? t("common.hide") : t("common.show")}
              </Button>
            </div>
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            disabled={props.busy || ccagTestState === "testing"}
          >
            {ccagTestState === "testing"
              ? t("config.testing")
          ...
          <Button
            variant="outline"
            onClick={() =>
              props.updateCcagServerSettings(buildCcagSettings())
            }
            disabled={props.busy || !hasCcagChanges}
          >
            {t("common.save")}
          </Button>
          <Button
            variant="ghost"
            onClick={props.resetCcagServerSettings}
            disabled={props.busy}
          >
            {t("common.reset")}
          </Button>
        ...
        {ccagTestState !== "idle" ? (
          ...
          >
            {ccagTestState === "testing"
              ...
              : (ccagTestMessage ?? t("config.connection_status_updated"))}
          </div>
        ) : null}

        {ccagStatusLabel !== t("config.status_connected") ? (
          <div className="text-xs text-gray-9">
