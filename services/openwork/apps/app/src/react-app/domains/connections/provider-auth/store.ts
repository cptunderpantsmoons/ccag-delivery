...
export function createProviderAuthStore(options: CreateProviderAuthStoreOptions) {
  ...
  const readWorkspaceCcagConfigRecord = async (): Promise<
  ...
  > => {
    ...
    if (isLocalWorkspace && isDesktopRuntime() && root) {
      return (await workspaceCcagRead({
        workspacePath: root,
  ...
  const writeWorkspaceCcagConfigRecord = async (
    ...
    if (isLocalWorkspace && isDesktopRuntime() && root) {
      ...
      if (!result.ok) {
        throw new Error(
          result.stderr || result.stdout || "Failed to write .opencode/ccag.json",
        );
      }
      return true;
    }
  ...
  const persistImportedCloudProviders = async (
    ...
    if (!persisted) {
      throw new Error(
        "CCAG server unavailable. Connect to manage imported cloud providers.",
      );
    }
  ...
  const readProjectConfigFile = async () => {
    ...
    const canUseCcagServer =
      ...
      ccagWorkspaceId &&
      ccagCapabilities?.config?.read &&
      typeof ccagClient.readOpencodeConfigFile === "function";

    if (canUseCcagServer) {
      return await ccagClient.readOpencodeConfigFile(ccagWorkspaceId, "project");
    }
  ...
  const writeProjectConfigFile = async (content: string) => {
    ...
    const canUseCcagServer =
      ...
      ccagWorkspaceId &&
      ccagCapabilities?.config?.write &&
      typeof ccagClient.writeOpencodeConfigFile === "function";

    if (canUseCcagServer) {
      const result = await ccagClient.writeOpencodeConfigFile(
        ccagWorkspaceId,
        "project",
        content,
      );
  ...
  const cloudProviderComment = (provider: Pick<DenOrgLlmProvider, "id" | "name">) =>
    `// CCAG Cloud import: ${provider.name
      .replace(/\s+/g, " ")
  ...
  async function connectCloudProvider(cloudProviderId: string) {
    ...
    const orgId = settings.activeOrgId?.trim() ?? "";
    if (!token || !orgId) {
      throw new Error("Sign in to CCAG Cloud and choose an organization first.");
    }
