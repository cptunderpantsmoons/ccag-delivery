...
export function createConnectionsStore(options: {
  ...
  const getCcagSnapshot = () => options.ccagServer.getSnapshot();
  ...
  const readMcpConfigFile = async (scope: "project" | "global"): Promise<OpencodeConfigFile | null> => {
    ...
    const ccagSnapshot = getCcagSnapshot();
    ...
    if (canUseCcagServer && ccagClient && ccagWorkspaceId) {
      return ccagClient.readOpencodeConfigFile(ccagWorkspaceId, scope);
    }
  ...
  const ensureActiveClient = async () => {
    ...
    const ccagSnapshot = getCcagSnapshot();
    const ccagBaseUrl = ccagSnapshot.ccagServerBaseUrl.trim();
    const token = ccagSnapshot.ccagServerAuth.token?.trim();
    if (!ccagBaseUrl || !token) {
      return null;
    }

    activeClient = createClient(`${ccagBaseUrl.replace(/\/+$/, "")}/opencode`, undefined, {
      token,
      mode: "ccag",
    });
  ...
  const resolveWritableCcagTarget = async () => {
    const ccagSnapshot = getCcagSnapshot();
    const ccagClient = ccagSnapshot.ccagServerClient;
    let ccagWorkspaceId = options.runtimeWorkspaceId();
    const ccagCapabilities = ccagSnapshot.ccagServerCapabilities;
    if (!ccagWorkspaceId && ccagClient && ccagSnapshot.ccagServerStatus === "connected") {
      ccagWorkspaceId = (await options.ensureRuntimeWorkspaceId?.()) ?? null;
    }
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagCapabilities?.mcp?.write;

    return {
      ccagClient,
      ccagWorkspaceId,
      canUseCcagServer: Boolean(canUseCcagServer),
    };
  };
  ...
  async function refreshMcpServers() {
    ...
    const canUseCcagServer =
      ...
      ccagClient &&
      ccagWorkspaceId &&
      ccagSnapshot.ccagServerCapabilities?.mcp?.read;

    if (isRemoteWorkspace) {
      if (!canUseCcagServer) {
        mutateState((current) => ({
          ...current,
          mcpStatus: "CCAG server unavailable. MCP config is read-only.",
          mcpServers: [],
          mcpStatuses: {},
        }));
        return;
      ...
      try {
        ...
        const response = await ccagClient.listMcp(ccagWorkspaceId);
  ...
  async function connectMcp(entry: McpDirectoryInfo) {
    ...
    const isRemoteWorkspace =
      ...
      (!isDesktopRuntime() && ccagSnapshot.ccagServerStatus === "connected");
    const projectDir = options.projectDir().trim();
    ...
    const { ccagClient, ccagWorkspaceId, canUseCcagServer } =
      await resolveWritableCcagTarget();

    if (isRemoteWorkspace && !canUseCcagServer) {
      setStateField("mcpStatus", "CCAG server unavailable. MCP config is read-only.");
      finishPerf(options.developerMode(), "mcp.connect", "blocked", startedAt, {
        reason: "ccag-server-unavailable",
      });
      return;
    }

    if (!canUseCcagServer && !isDesktopRuntime()) {
    ...
    try {
      ...
      if (canUseCcagServer && ccagClient && ccagWorkspaceId) {
        await ccagClient.addMcp(ccagWorkspaceId, {
          name: slug,
  ...
  async function logoutMcpAuth(name: string) {
    ...
    if (isRemoteWorkspace && !canUseCcagServer) {
      setStateField("mcpStatus", "CCAG server unavailable. MCP auth is read-only.");
      return;
    }
    ...
    try {
      if (canUseCcagServer && ccagClient && ccagWorkspaceId) {
        await ccagClient.logoutMcpAuth(ccagWorkspaceId, safeName);
      } else {
        try {
  ...
  async function removeMcp(name: string) {
    try {
      ...
      const canUseCcagServer =
        ...
        ccagClient &&
        ccagWorkspaceId &&
        ccagSnapshot.ccagServerCapabilities?.mcp?.write;

      if (canUseCcagServer && ccagClient && ccagWorkspaceId) {
        await ccagClient.removeMcp(ccagWorkspaceId, name);
      } else {
        const projectDir = options.projectDir().trim();
        if (!projectDir) {
          setStateField("mcpStatus", translate("mcp.pick_workspace_first"));
