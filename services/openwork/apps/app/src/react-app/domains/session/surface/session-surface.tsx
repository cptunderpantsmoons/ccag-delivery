...
function transcriptToText(messages: UIMessage[]) {
  return messages
    .map((message) => {
      const header = message.role === "user" ? "You" : message.role === "assistant" ? "CCAG" : message.role;
...
function statusLabel(snapshot: CcagSessionSnapshot | undefined, busy: boolean) {
...
export function SessionSurface(props: SessionSurfaceProps) {
  ...
  const [rendered, setRendered] = useState<{ sessionId: string; snapshot: CcagSessionSnapshot } | null>(null);
  ...
  const opencodeClient = useMemo(
    () => createClient(props.opencodeBaseUrl, undefined, { token: props.ccagToken, mode: "ccag" }),
    [props.opencodeBaseUrl, props.ccagToken],
  );
  ...
  const snapshotQuery = useQuery<CcagSessionSnapshot>({
