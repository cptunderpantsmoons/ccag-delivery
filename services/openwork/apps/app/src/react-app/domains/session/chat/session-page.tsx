...
import { t } from "../../../../i18n";
import { buildCcagWorkspaceBaseUrl, type CcagServerClient, type CcagServerStatus } from "../../../../app/lib/ccag-server";
...
export type SessionPageSurfaceProps = Omit<
  SessionSurfaceProps,
  "client" | "workspaceId" | "sessionId" | "opencodeBaseUrl" | "ccagToken"
>;
...
export function SessionPage(props: SessionPageProps) {
  ...
  const reactSessionBaseUrl = useMemo(() => {
    ...
    const baseUrl = props.ccagServerClient?.baseUrl?.trim() ?? "";
    if (!workspaceId || !baseUrl) return "";
    const mounted = buildCcagWorkspaceBaseUrl(baseUrl, workspaceId) ?? baseUrl;
    return `${mounted.replace(/\/+$/, "")}/opencode`;
  }, [props.ccagServerClient?.baseUrl, props.runtimeWorkspaceId]);

  const reactSessionToken = props.ccagServerClient?.token?.trim() || props.ccagServerToken?.trim() || "";
  const canRenderReactSurface = Boolean(
    props.selectedSessionId &&
      props.runtimeWorkspaceId &&
      props.ccagServerClient &&
      reactSessionBaseUrl &&
      reactSessionToken &&
      props.surface,
  );
      ...
      <div className="flex min-h-0 flex-1 gap-4 p-3 md:p-4">
        ...
        <main className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-[24px] border border-dls-border bg-dls-surface shadow-[var(--dls-shell-shadow)]">
          ...
          <div className="flex min-h-0 flex-1 overflow-hidden">
            <div className="relative min-w-0 flex-1 overflow-hidden bg-dls-surface">
              ...
              {!showDelayedSessionLoadingState && canRenderReactSurface ? (
                <SessionSurface
                  client={props.ccagServerClient!}
                  workspaceId={props.runtimeWorkspaceId!}
                  sessionId={props.selectedSessionId!}
                  opencodeBaseUrl={reactSessionBaseUrl}
                  ccagToken={reactSessionToken}
                  {...props.surface!}
                />
              ) : null}
          ...
          <StatusBar
            ...
            ccagServerStatus={props.ccagServerStatus}
