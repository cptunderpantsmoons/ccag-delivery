...
import type { CcagSessionSnapshot } from "../../../../app/lib/ccag-server";
...
function syncKey(input: SyncOptions) {
  return `${input.workspaceId}:${input.baseUrl}:${input.ccagToken}`;
}
...
function startSync(input: SyncOptions) {
  const client = createClient(input.baseUrl, undefined, { token: input.ccagToken, mode: "ccag" });
...
export function seedSessionState(workspaceId: string, snapshot: CcagSessionSnapshot) {
  const queryClient = getReactQueryClient();
