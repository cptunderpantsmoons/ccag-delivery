...
export function CreateWorkspaceModal(props: CreateWorkspaceModalProps) {
  ...
  const handleRemoteSubmit = async () => {
    ...
    await Promise.resolve(
      props.onConfirmRemote({
        ccagHostUrl: remoteUrl.trim(),
        ccagToken: remoteToken.trim() || null,
        directory: null,
  ...
  const handleCCAGer = async (worker: DenWorkerSummary) => {
    if (!props.onConfirmRemote) return;
    const orgId = activeOrgId.trim();
    if (!orgId) {
    ...
    try {
      ...
      const ccagUrl = tokens.ccagUrl?.trim() ?? "";
      const accessToken =
        tokens.ownerToken?.trim() || tokens.clientToken?.trim() || "";
      if (!ccagUrl || !accessToken) {
        throw new Error(translate("dashboard.error_workspace_not_ready"));
      }
      const ok = await Promise.resolve(
        props.onConfirmRemote({
          ccagHostUrl: ccagUrl,
          ccagToken: accessToken,
          ccagClientToken: tokens.clientToken?.trim() || null,
          ccagHostToken: tokens.hostToken?.trim() || null,
          directory: null,
    ...
    <div className={`${modalShellClass} ${modalWidthClass}`}>
      ...
      {screen === "shared" ? (
        <CreateWorkspaceSharedPanel
          ...
          onCCAGer={(worker) => void handleCCAGer(worker)}
