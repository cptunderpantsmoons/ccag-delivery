...
export function CreateRemoteWorkspaceModal(
  ...
  const [ccagHostUrl, setCcagHostUrl] = useState("");
  ...
  const canSubmit = useMemo(() => {
    ...
    return ccagHostUrl.trim().length > 0;
  }, [ccagHostUrl, submitting]);
  ...
  useEffect(() => {
    ...
    setCcagHostUrl(defaults.ccagHostUrl?.trim() ?? "");
    setCcagToken(defaults.ccagToken?.trim() ?? "");
    setCcagTokenVisible(false);
    ...
    <div className={`${modalShellClass} max-w-[560px]`}>
      ...
      <div className={modalBodyClass}>
        <RemoteWorkspaceFields
          hostUrl={ccagHostUrl}
          onHostUrlInput={setCcagHostUrl}
          token={ccagToken}
          tokenVisible={ccagTokenVisible}
          onTokenInput={setCcagToken}
          onToggleTokenVisible={() =>
            setCcagTokenVisible((prev) => !prev)
          }
          ...
          description="Use the URL your CCAG server shared with you. Add a token only if the server needs one."
        />
      </div>
      ...
      <div className="space-y-3 border-t border-dls-border px-6 py-5">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            onClick={() =>
              props.onConfirm({
                ccagHostUrl: ccagHostUrl.trim(),
                ccagToken: ccagToken.trim(),
