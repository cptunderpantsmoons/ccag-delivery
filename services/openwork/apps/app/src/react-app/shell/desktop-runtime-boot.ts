...
import { hydrateCcagServerSettingsFromEnv, writeCcagServerSettings } from "../../app/lib/ccag-server";
 ...
 *   2) if a local workspace is selected, restart the embedded CCAG server
 *   3) start the OpenCode engine pointed at the workspace
 *   4) activate the workspace in the orchestrator
 *   5) persist the resulting base URL + token into local CCAG settings so the
 *      React routes (session-route / settings-route) see a live `readCcagServerSettings()`
 *
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // current ccag-server base URL + token and finish in <1s.
        ...
        try {
          ...
          if (engine?.running && engine.baseUrl) {
            ...
            const fresh = await ccagServerInfo().catch(() => null);
            if (fresh?.baseUrl) {
              ...
              try {
                window.dispatchEvent(
                  new CustomEvent("ccag-server-settings-changed"),
                );
              } catch {
                /* ignore */
              }
        ...
        // and ccag-server (re)start with --opencode-base-url attached,
        // so we don't need a separate ccagServerRestart step.
        ...
        if (engineStartResult) {
          ...
          try {
            const freshInfo = await ccagServerInfo();
            ...
            console.warn("[desktop-boot] post-engineStart ccagServerInfo failed:", error);
          }
        }
