...
async function resolveRouteCcagConnection() {
  ...
  let hostInfo: CcagServerInfo | null = null;
...
export function SessionRoute() {
  ...
  const [client, setClient] = useState<CcagServerClient | null>(null);
  ...
  const [ccagServerHostInfoState, setCcagServerHostInfoState] = useState<CcagServerInfo | null>(null);
  const [ccagServerSettingsVersion, setCcagServerSettingsVersion] = useState(0);
  ...
  const ccagServerSettings = useMemo(
    () => readCcagServerSettings(),
    [ccagServerSettingsVersion],
  );

  const shareWorkspaceState = useShareWorkspaceState({
    workspaces,
    ccagServerHostInfo: ccagServerHostInfoState,
    ccagServerSettings,
  ...
  const refreshRouteState = useCallback(async () => {
    ...
    try {
      ...
      const { normalizedBaseUrl, resolvedToken, hostInfo } = await resolveRouteCcagConnection();
      setCcagServerHostInfoState(hostInfo);
      ...
      const ccagClient = createCcagServerClient({
        baseUrl: normalizedBaseUrl,
        token: resolvedToken,
      });
      const list = await ccagClient.listWorkspaces();
      const nextWorkspaces = mergeRouteWorkspaces(list.items, desktopWorkspaces);

      const sessionEntries = await Promise.all(
        nextWorkspaces.map(async (workspace) => {
          try {
            const response = await ccagClient.listSessions(workspace.id, { limit: 200 });
            // The underlying opencode instance is shared across all local
            // workspaces attached to the same ccag-server, so `listSessions`
  ...
  useEffect(() => {
    ...
    const handleSettingsChange = () => {
      setCcagServerSettingsVersion((value) => value + 1);
  ...
  const handleSaveShareRemoteAccess = useCallback(
    async (enabled: boolean) => {
      ...
      const previous = readCcagServerSettings();
      ...
      setCcagServerSettingsVersion((value) => value + 1);
      try {
        const info = await ccagServerRestart({ remoteAccessEnabled: enabled });
        setCcagServerHostInfoState(info);
        await refreshRouteState();
      } catch (error) {
        writeCcagServerSettings(previous);
        setCcagServerSettingsVersion((value) => value + 1);
  ...
  const handleCreateTaskInWorkspace = useCallback(async (workspaceId: string) => {
    ...
    const workspaceOpencodeBaseUrl = `${(buildCcagWorkspaceBaseUrl(baseUrl, workspace.id) ?? baseUrl).replace(/\/+$|\/+$/g, "")}/opencode`;
  ...
  return (
    ...
    <SessionPage
      ...
      ccagServerStatus={client ? "connected" : "disconnected"}
      ccagServerClient={client}
      ccagServerToken={token}
      developerMode={false}
          ...
          ? {
              ...
              remoteAccess:
                isDesktopRuntime() && shareWorkspaceState.shareWorkspace?.workspaceType === "local"
                  ? {
                      enabled: ccagServerSettings.remoteAccessEnabled === true,
              ...
              templateContentSummary: {
                skillNames: [],
                commandNames: [],
                configFiles: ["opencode.json", "ccag.json"],
              },
