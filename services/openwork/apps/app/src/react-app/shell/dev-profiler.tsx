 ...
 * localStorage.ccag.debug.profilerOverlay = "1" / "0".
 *
...
//
// Explicit opt-ins:
//   - VITE_CCAG_PROFILER=1 at `pnpm dev`
//   - window.localStorage.setItem("ccag.debug.profiler", "1")
...
const PROFILER_ENABLED = (() => {
  ...
  try {
    ...
    const flag = env.VITE_CCAG_PROFILER;
    if (flag === "1" || flag === "true" || flag === true) return true;
  } catch {
    // ignore
  }
  try {
    if (window.localStorage.getItem("ccag.debug.profiler") === "1") return true;
  } catch {
    // ignore
  }
  return false;
...
function readOverlayStoredPreference(): boolean | null {
  ...
  try {
    const raw = window.localStorage.getItem("ccag.debug.profilerOverlay");
...
function writeOverlayStoredPreference(value: boolean) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem("ccag.debug.profilerOverlay", value ? "1" : "0");
  } catch {
    // ignore
  }
}
