...
import { createCcagServerStore, useCcagServerStoreSnapshot } from "../domains/connections/ccag-server-store";
...
import { ensureDesktopLocalCcagConnection } from "./desktop-local-ccag";

type RouteWorkspace = CcagWorkspaceInfo & {
...
function mergeRouteWorkspaces(
  serverWorkspaces: CcagWorkspaceInfo[],
...
async function resolveRouteCcagConnection() {
  const settings = readCcagServerSettings();
  let normalizedBaseUrl = normalizeCcagServerUrl(settings.urlOverride ?? "") ?? "";
  ...
  if ((!normalizedBaseUrl || !resolvedToken) && isDesktopRuntime()) {
    try {
      ...
      normalizedBaseUrl =
        normalizeCcagServerUrl(info.connectUrl ?? info.baseUrl ?? info.lanUrl ?? info.mdnsUrl ?? "") ??
        normalizedBaseUrl;
...
const SETTINGS_THEME_KEY = "ccag.react.settings.theme-mode";
const SETTINGS_HIDE_TITLEBAR_KEY = "ccag.react.settings.hide-titlebar";
const SETTINGS_UPDATE_AUTO_CHECK_KEY = "ccag.react.settings.update-auto-check";
const SETTINGS_UPDATE_AUTO_DOWNLOAD_KEY = "ccag.react.settings.update-auto-download";

function workspaceLabel(workspace: CcagWorkspaceInfo) {
  return (
    workspace.displayName?.trim() ||
    workspace.ccagWorkspaceName?.trim() ||
    workspace.name?.trim() ||
    workspace.path?.trim() ||
    t("session.workspace_fallback")
  );
...
export function SettingsRoute() {
  ...
  const [ccagClient, setCcagClient] = useState<CcagServerClient | null>(null);
    ...
    () =>
      selectedWorkspace
        ? {
            ...
            ccagWorkspaceName: selectedWorkspace.ccagWorkspaceName,
          }
  ...
  const ccagServerStore = useMemo(
    () =>
      createCcagServerStore({
        startupPreference: () => {
          ...
          const stored = readCcagServerSettings();
  ...
  const ccagServerSnapshot = useCcagServerStoreSnapshot(ccagServerStore);
  ...
  const opencodeBaseUrl = useMemo(() => {
    ...
    const mounted = buildCcagWorkspaceBaseUrl(baseUrl, selectedWorkspaceId) ?? baseUrl;
  ...
  const refreshRouteState = useMemo(() => async () => {
    ...
    try {
      ...
      const { normalizedBaseUrl, resolvedToken } = await resolveRouteCcagConnection();
      ...
      const client = createCcagServerClient({ baseUrl: normalizedBaseUrl, token: resolvedToken });
  ...
  useEffect(() => {
    ...
    void ensureDesktopLocalCcagConnection({
      route: "settings",
  ...
  }, [loading, ccagClient, selectedWorkspace, workspaces]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("ccag-server-settings-changed", handleSettingsChange);
    return () => {
      window.removeEventListener("ccag-server-settings-changed", handleSettingsChange);
    };
  }, [refreshRouteState]);

  useEffect(() => {
    ccagServerStore.start();
    ...
    return () => {
      ...
      ccagServerStore.dispose();
    };
  }, [automationsStore, connectionsStore, extensionsStore, ccagServerStore, providerAuthStore]);
  ...
  useEffect(() => {
    ccagServerStore.syncFromOptions();
  ...
  const routeCcagStatus = ccagClient ? "connected" : "disconnected";
  const routeCcagCapabilities: CcagServerCapabilities | null = ccagClient
    ? {
        skills: { read: true, write: true, source: "ccag" },
  ...
  const handleCreateWorkspace = async (preset: WorkspacePreset, folder: string | null) => {
    ...
    try {
      ...
      // Register the workspace with the running ccag-server so
  ...
  const handleCreateRemoteWorkspace = async (input: {
    ccagHostUrl?: string | null;
    ccagToken?: string | null;
    directory?: string | null;
    displayName?: string | null;
  }) => {
    const baseUrlValue = input.ccagHostUrl?.trim() ?? "";
    ...
    try {
      const list = await workspaceCreateRemote({
        ...
        ccagHostUrl: baseUrlValue,
        ccagToken: input.ccagToken?.trim() || null,
        ...
        return (
          <GeneralSettingsView
            authorizedFoldersPanel={{
              ccagServerClient: ccagClient,
              ccagServerStatus: routeCcagStatus,
              ccagServerCapabilities: routeCcagCapabilities,
              runtimeWorkspaceId: selectedWorkspace?.id ?? null,
              selectedWorkspaceRoot,
            ...
            onReportIssue={() => platform.openLink("https://github.com/different-ai/ccag/issues/new?template=bug.yml")}
          />
        );
      case "automations":
        return (
      ...
      case "advanced":
        return (
          <AdvancedView
            ...
            headerStatus={ccagServerSnapshot.ccagServerStatus}
            clientConnected={Boolean(opencodeClient)}
            opencodeConnectStatus={null}
            ccagServerStatus={ccagServerSnapshot.ccagServerStatus}
            ccagServerUrl={ccagServerSnapshot.ccagServerUrl}
            ccagReconnectBusy={ccagServerSnapshot.ccagReconnectBusy}
            reconnectCcagServer={ccagServerStore.reconnectCcagServer}
            engineInfo={null}
            restartLocalServer={async () => false}
            stopHost={() => {}}
            developerMode={developerMode}
            ...
            configView={{
              ...
              ccagServerStatus: ccagServerSnapshot.ccagServerStatus,
              ccagServerUrl: ccagServerSnapshot.ccagServerUrl,
              ccagServerSettings: ccagServerSnapshot.ccagServerSettings,
              ccagServerHostInfo: ccagServerSnapshot.ccagServerHostInfo,
              runtimeWorkspaceId: selectedWorkspace?.id ?? null,
              updateCcagServerSettings: ccagServerStore.updateCcagServerSettings,
              resetCcagServerSettings: ccagServerStore.resetCcagServerSettings,
              testCcagServerConnection: ccagServerStore.testCcagServerConnection,
              canReloadWorkspace: false,
              reloadWorkspaceEngine: async () => {
                setRouteError("Workspace reload is not wired into the React settings route yet.");
              },
