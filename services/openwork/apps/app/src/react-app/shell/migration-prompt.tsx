 ...
 * `VITE_CCAG_MIGRATION_RELEASE=1`, so landing this code on dev has
...
function readBuildTimeConfig(): MigrationConfig | null {
  ...
  const enabled = env.VITE_CCAG_MIGRATION_RELEASE === "1";
  if (!enabled) return null;
  return {
    macUrl: env.VITE_CCAG_MIGRATION_MAC_URL,
    macSha256: env.VITE_CCAG_MIGRATION_MAC_SHA256,
    windowsUrl: env.VITE_CCAG_MIGRATION_WINDOWS_URL,
    linuxUrl: env.VITE_CCAG_MIGRATION_LINUX_URL,
  };
}
    ...
    >
      <div className="w-[min(440px,90vw)] rounded-[24px] border border-dls-border bg-dls-surface p-6 shadow-xl">
        ...
        <p className="mt-3 text-sm leading-relaxed text-gray-11">
          ...
          are. You'll be back in CCAG automatically when it's done.
        </p>
        {error ? (
