...
import { writeCcagServerSettings } from "../../app/lib/ccag-server";
...
export async function ensureDesktopLocalCcagConnection(
  options: EnsureDesktopLocalCcagOptions,
) {
  ...
  try {
    ...
    if (!info?.baseUrl) {
      throw new Error("CCAG server did not report a base URL after activation.");
    }
