...
import { parseCcagWorkspaceIdFromUrl } from "../lib/ccag-server";
...
export function buildImportPayloadFromBundle(bundle: BundleV1): {
  ...
  if (workspace.ccag && typeof workspace.ccag === "object") {
    payload.ccag = workspace.ccag;
  }
...
export function isBundleImportWorkspace(workspace: WorkspaceDisplay | WorkspaceInfo | null): boolean {
  ...
  return Boolean(workspace.remoteType === "ccag" || workspace.ccagHostUrl?.trim() || workspace.ccagWorkspaceId?.trim());
}
