...
import type { CcagWorkspaceExport } from "../lib/ccag-server";
...
export function defaultPresetFromWorkspaceProfileBundle(bundle: WorkspaceProfileBundleV1): WorkspacePreset {
  const ccag = bundle.workspace?.ccag;
  if (!ccag || typeof ccag !== "object") return "starter";
  const workspace = (ccag as Record<string, unknown>).workspace;
...
function describeWorkspaceProfileItems(bundle: WorkspaceProfileBundleV1): string[] {
  ...
  const hasCCAGConfig = Boolean(workspace.ccag && typeof workspace.ccag === "object");

  return [
    ...skills,
    ...(commands > 0 ? [`${commands} command${commands === 1 ? "" : "s"}`] : []),
    ...(hasOpenCodeConfig ? ["OpenCode config"] : []),
    ...(hasCCAGConfig ? ["CCAG config"] : []),
...
export function parseBundlePayload(value: unknown): BundleV1 {
  ...
  if (type === "workspace-profile") {
    ...
    return {
      ...
      description: typeof record.description === "string" ? record.description : undefined,
      workspace: {
        ...(workspace as CcagWorkspaceExport),
        ...(files.length ? { files } : {}),
      },
    };
  }
