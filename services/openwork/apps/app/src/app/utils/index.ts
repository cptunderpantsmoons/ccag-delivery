...
export function isElectronRuntime() {
  return typeof window !== "undefined" && (window as Window).__CCAG_ELECTRON__ != null;
}
...
const STARTUP_PREF_KEY = "ccag.startupPref";
const LEGACY_PREF_KEY = "ccag.modePref";
const LEGACY_PREF_KEY_ALT = "ccag_mode_pref";
...
export function getWorkspaceTaskLoadErrorDisplay(workspace: WorkspaceInfo, error?: string | null) {
  ...
  const host = `${workspace.baseUrl ?? ""} ${workspace.ccagHostUrl ?? ""}`.toLowerCase();
...
const TRUNCATED_SYSTEM_PATH_PATTERNS = [
  /com\.[^/]+\.(ccag|opencode)/i, // macOS app bundle identifiers
  /\.ccag\.dev\//i, // CCAG dev paths
