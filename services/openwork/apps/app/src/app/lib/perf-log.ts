...
type PerfRoot = typeof globalThis & {
  __ccagPerfSeq?: number;
  __ccagPerfLogs?: PerfLogRecord[];
  __ccagPerfConsoleAt?: Record<string, number>;
  __ccagPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__ccagPerfSeq ?? 0) + 1;
  root.__ccagPerfSeq = id;
  ...
  try {
    ...
    const now = Date.now();
    const lastByKey = root.__ccagPerfConsoleAt ?? (root.__ccagPerfConsoleAt = {});
    const suppressedByKey =
      root.__ccagPerfConsoleSuppressed ?? (root.__ccagPerfConsoleSuppressed = {});
