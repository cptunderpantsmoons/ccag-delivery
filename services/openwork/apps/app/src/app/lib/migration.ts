...
export const MIGRATION_KEY_PATTERNS: Array<RegExp> = [
  /^ccag\.react\.activeWorkspace$/,
  /^ccag\.react\.sessionByWorkspace$/,
  /^ccag\.server\.list$/,
  /^ccag\.server\.active$/,
  /^ccag\.server\.urlOverride$/,
  /^ccag\.server\.token$/,
];
...
function electronMigrationBridge(): ElectronMigrationBridge | null {
  ...
  const bridge = (window as unknown as {
    __CCAG_ELECTRON__?: { migration?: ElectronMigrationBridge };
  }).__CCAG_ELECTRON__;
...
export const MIGRATION_DEFER_KEY = "ccag.migration.deferredUntil";
