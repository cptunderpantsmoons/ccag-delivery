...
import { normalizeCcagServerUrl } from "./ccag-server";
...
function isSupportedDeepLinkProtocol(protocol: string): boolean {
  ...
  return normalized === "ccag:" || normalized === "ccag-dev:" || normalized === "https:" || normalized === "http:";
}
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("ccagHostUrl") ?? url.searchParams.get("ccagUrl") ?? "";
  const tokenRaw = url.searchParams.get("ccagToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeCcagServerUrl(hostUrlRaw);
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:ccag-dev|ccag|https?):\/\/[^\s"'<>]+/i);
  if (directMatch) return directMatch[0];

  const bareShareMatch = trimmed.match(/share\.ccag(?:labs\.com|\.software)\/b\/[^\s"'<>]+/i);
...
export function parseDebugDeepLinkInput(rawValue: string):
  ...
  const shareIdMatch = normalized.match(/share\.ccag(?:labs\.com|\.software)\/b\/([^\s/?#"'<>]+)/i);
  if (shareIdMatch?.[1]) {
    return {
      kind: "bundle",
      link: {
