...
} from "@ccag/types/den/desktop-app-restrictions";
...
import { isDesktopDeployment } from "./ccag-deployment";
import {
...
const STORAGE_BASE_URL = "ccag.den.baseUrl";
const STORAGE_API_BASE_URL = "ccag.den.apiBaseUrl";
const STORAGE_AUTH_TOKEN = "ccag.den.authToken";
const STORAGE_ACTIVE_ORG_ID = "ccag.den.activeOrgId";
const STORAGE_ACTIVE_ORG_SLUG = "ccag.den.activeOrgSlug";
const STORAGE_ACTIVE_ORG_NAME = "ccag.den.activeOrgName";
const DEFAULT_DEN_TIMEOUT_MS = 12_000;

export const DEFAULT_DEN_AUTH_NAME = "CCAG User";
...
function isWebAppHost(hostname: string): boolean {
  ...
  return normalized === "app.openworklabs.com" || normalized === "app.ccag.software" || normalized.startsWith("app.");
}
...
export function buildDenAuthUrl(baseUrl: string, mode: "sign-in" | "sign-up"): string {
  ...
  if (isDesktopDeployment()) {
    ...
    target.searchParams.set("desktopScheme", "ccag");
  }
...
function getWorkerTokens(payload: unknown): DenWorkerTokens | null {
  ...
  return {
    ...
    ccagUrl: connect && typeof connect.ccagUrl === "string" ? connect.ccagUrl : null,
