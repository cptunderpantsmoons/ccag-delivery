...
export type EngineInfo = {
  running: boolean;
  runtime: "direct" | "ccag-orchestrator";
  baseUrl: string | null;
...
export type WorkspaceInfo = {
  ...
  remoteType?: "ccag" | "opencode" | null;
  baseUrl?: string | null;
  directory?: string | null;
  displayName?: string | null;
  ccagHostUrl?: string | null;
  ccagToken?: string | null;
  ccagClientToken?: string | null;
  ccagHostToken?: string | null;
  ccagWorkspaceId?: string | null;
  ccagWorkspaceName?: string | null;
...
export async function engineStart(
  ...
  return invoke<EngineInfo>("engine_start", {
    ...
    ccagRemoteAccess: options?.ccagRemoteAccess ?? null,
...
export type WorkspaceCcagConfig = {
  version: number;
  workspace?: {
    name?: string | null;
    createdAt?: number | null;
...
export async function workspaceCcagRead(input: {
  workspacePath: string;
}): Promise<WorkspaceCcagConfig> {
  return invoke<WorkspaceCcagConfig>("workspace_ccag_read", {
    workspacePath: input.workspacePath,
  });
}

export async function workspaceCcagWrite(input: {
  workspacePath: string;
  config: WorkspaceCcagConfig;
}): Promise<ExecResult> {
  return invoke<ExecResult>("workspace_ccag_write", {
    workspacePath: input.workspacePath,
    config: input.config,
  });
}
...
export async function nukeCcagAndOpencodeConfigAndExit(): Promise<void> {
  return invoke<void>("nuke_ccag_and_opencode_config_and_exit");
}
...
export type CcagDockerCleanupResult = {
  candidates: string[];
  removed: string[];
  errors: string[];
};

export async function sandboxCleanupCcagContainers(): Promise<CcagDockerCleanupResult> {
  return invoke<CcagDockerCleanupResult>("sandbox_cleanup_ccag_containers");
}
...
export async function ccagServerInfo(): Promise<CcagServerInfo> {
  return invoke<CcagServerInfo>("ccag_server_info");
}

export async function ccagServerRestart(options?: {
  remoteAccessEnabled?: boolean;
}): Promise<CcagServerInfo> {
  return invoke<CcagServerInfo>("ccag_server_restart", {
...
export async function resetCcagState(mode: "onboarding" | "all"): Promise<void> {
  return invoke<void>("reset_ccag_state", { mode });
}
