...
import { createCcagServerClient, CcagServerError } from "./ccag-server";
...
function resolveCcagWorkspaceMount(baseUrl: string): { baseUrl: string; workspaceId: string } | null {
  try {
...
async function wrapCcagRead<T>(
  ...
  try {
    ...
    return createSyntheticResult(url, "GET", {
      ok: false,
      error,
      status: error instanceof CcagServerError ? error.status : 500,
    });
  }
}

function shouldFallbackToLegacySessionRead(error: unknown): boolean {
  if (!(error instanceof CcagServerError)) return false;
  return error.status === 404 || error.status === 405 || error.status === 501;
}

async function wrapCcagReadWithFallback<T>(
  url: string,
  read: () => Promise<T>,
 ...
 * CORS is already wide open on the ccag/opencode stack, so there's no
...
export function createClient(baseUrl: string, directory?: string, auth?: OpencodeAuth) {
  ...
  const ccagMount = auth?.mode === "ccag" ? resolveCcagWorkspaceMount(baseUrl) : null;
  const ccagSessionClient =
    ccagMount && auth?.token
      ? createCcagServerClient({ baseUrl: ccagMount.baseUrl, token: auth.token })
      : null;
  // TODO(2026-04-12): remove the old-server compatibility path here once all
  // CCAG servers expose the workspace-scoped session read APIs.
  ...
  sessionOverrides.list = (parameters?: SessionListParameters, options?: { throwOnError?: boolean }) => {
    if (!ccagMount || !ccagSessionClient) {
    ...
    const url = `${ccagMount.baseUrl}/workspace/${encodeURIComponent(ccagMount.workspaceId)}/sessions${query.size ? `?${query.toString()}` : ""}`;
    return wrapCcagReadWithFallback(
      url,
      async () => (await ccagSessionClient.listSessions(ccagMount.workspaceId, parameters)).items,
  ...
  sessionOverrides.get = (parameters: SessionLookupParameters, options?: { throwOnError?: boolean }) => {
    ...
    const url = `${ccagMount.baseUrl}/workspace/${encodeURIComponent(ccagMount.workspaceId)}/sessions/${encodeURIComponent(parameters.sessionID)}`;
    return wrapCcagReadWithFallback(
      url,
      async () => (await ccagSessionClient.getSession(ccagMount.workspaceId, parameters.sessionID)).item,
  ...
  sessionOverrides.messages = (parameters: SessionMessagesParameters, options?: { throwOnError?: boolean }) => {
    ...
    const query = new URLSearchParams();
    if (typeof parameters.limit === "number") query.set("limit", String(parameters.limit));
    const url = `${ccagMount.baseUrl}/workspace/${encodeURIComponent(ccagMount.workspaceId)}/sessions/${encodeURIComponent(parameters.sessionID)}/messages${query.size ? `?${query.toString()}` : ""}`;
    return wrapCcagReadWithFallback(
      url,
      async () =>
        (await ccagSessionClient.getSessionMessages(ccagMount.workspaceId, parameters.sessionID, {
  ...
  sessionOverrides.todo = (parameters: SessionLookupParameters, options?: { throwOnError?: boolean }) => {
    ...
    const url = `${ccagMount.baseUrl}/workspace/${encodeURIComponent(ccagMount.workspaceId)}/sessions/${encodeURIComponent(parameters.sessionID)}/snapshot`;
    return wrapCcagReadWithFallback(
      url,
      async () => (await ccagSessionClient.getSessionSnapshot(ccagMount.workspaceId, parameters.sessionID)).item.todos,
