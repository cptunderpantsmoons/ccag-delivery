...
function isElectronDesktopRuntime() {
  return typeof window !== "undefined" && window.__CCAG_ELECTRON__ != null;
}
...
async function invokeElectronHelper<T>(command: string, ...args: unknown[]): Promise<T> {
  const invokeDesktop = window.__CCAG_ELECTRON__?.invokeDesktop;
  if (!invokeDesktop) {
...
function resolveElectronBridge(): DesktopBridge {
  const exposed = window.__CCAG_ELECTRON__?.bridge ?? {};
  const invokeDesktop = window.__CCAG_ELECTRON__?.invokeDesktop;
...
function resolveDesktopBridge(): DesktopBridge {
  if (
    ...
    (window.__CCAG_ELECTRON__?.bridge || window.__CCAG_ELECTRON__?.invokeDesktop)
  ) {
...
export async function openDesktopUrl(url: string): Promise<void> {
  if (isElectronDesktopRuntime()) {
    await window.__CCAG_ELECTRON__?.shell?.openExternal?.(url);
    return;
  }
...
export async function relaunchDesktopApp(): Promise<void> {
  if (isElectronDesktopRuntime()) {
    await window.__CCAG_ELECTRON__?.shell?.relaunch?.();
    return;
  }
...
export async function subscribeDesktopDeepLinks(
  ...
  if (isElectronDesktopRuntime()) {
    ...
    const initialUrls = window.__CCAG_ELECTRON__?.meta?.initialDeepLinks;
