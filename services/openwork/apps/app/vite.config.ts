...
addHost(process.env.CCAG_PUBLIC_HOST ?? null);
...
const isElectronPackagedBuild = process.env.CCAG_ELECTRON_BUILD === "1";
...
export default defineConfig({
  ...
  plugins: [
    {
      name: "ccag-dev-server-id",
      configureServer(server) {
        server.middlewares.use("/__ccag_dev_server_id", (_req, res) => {
