...
function readArgs(argv: string[]): BuildOptions {
  const options: BuildOptions = {
    bundleDir: process.env.CCAG_SERVER_V2_BUNDLE_DIR?.trim() ? resolve(process.env.CCAG_SERVER_V2_BUNDLE_DIR.trim()) : null,
    embedRuntime: false,
    filename: "ccag-server-v2",
...
function createEmbeddedEntrypoint(assets: RuntimeAssetPaths) {
  const buildDir = mkdtempSync(join(os.tmpdir(), "ccag-server-v2-build-"));
...
function buildOnce(options: BuildOptions, target?: string) {
  ...
  const args = [
    ...
    outfile,
    "--define",
    `__CCAG_SERVER_V2_VERSION__=${JSON.stringify(version)}`,
  ];
  if (target) {
