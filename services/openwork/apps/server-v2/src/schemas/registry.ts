...
export const authSummarySchema = z.object({
  ...
  headers: z.object({
    ...
    hostToken: z.literal("X-CCAG-Host-Token"),
  }),
  required: z.boolean(),
  scopes: z.object({
...
}).meta({ ref: "CCAGServerV2AuthSummary" });
...
}).meta({ ref: "CCAGServerV2ServerInventoryItem" });
...
}).meta({ ref: "CCAGServerV2RegistrySummary" });
...
}).meta({ ref: "CCAGServerV2CapabilitiesData" });
...
const workspaceBackendSchema = z.object({
  ...
  remote: z.object({
    ...
    remoteType: z.enum(["ccag", "opencode"]),
    remoteWorkspaceId: z.string().nullable(),
    workspaceName: z.string().nullable(),
  }).nullable(),
  serverId: identifierSchema,
}).meta({ ref: "CCAGServerV2WorkspaceBackend" });
...
}).meta({ ref: "CCAGServerV2WorkspaceRuntimeSummary" });
...
}).meta({ ref: "CCAGServerV2RemoteServerConnectRequest" });

export const remoteServerSyncRequestSchema = z.object({
  directory: z.string().nullable().optional(),
  workspaceId: z.string().nullable().optional(),
}).meta({ ref: "CCAGServerV2RemoteServerSyncRequest" });
...
}).meta({ ref: "CCAGServerV2RemoteServerConnectData" });
...
export const systemStatusDataSchema = z.object({
  ...
  version: z.string(),
}).meta({ ref: "CCAGServerV2SystemStatusData" });

export const capabilitiesResponseSchema = successResponseSchema("CCAGServerV2CapabilitiesResponse", capabilitiesDataSchema);
export const serverInventoryListResponseSchema = successResponseSchema(
  "CCAGServerV2ServerInventoryListResponse",
  serverInventoryListDataSchema,
);
export const remoteServerConnectResponseSchema = successResponseSchema(
  "CCAGServerV2RemoteServerConnectResponse",
  remoteServerConnectDataSchema,
);
export const systemStatusResponseSchema = successResponseSchema("CCAGServerV2SystemStatusResponse", systemStatusDataSchema);
export const workspaceDetailResponseSchema = successResponseSchema("CCAGServerV2WorkspaceDetailResponse", workspaceDetailDataSchema);
export const workspaceListResponseSchema = successResponseSchema("CCAGServerV2WorkspaceListResponse", workspaceListDataSchema);
