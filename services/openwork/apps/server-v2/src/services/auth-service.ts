...
export function createAuthService() {
  const clientToken = trimToken(
    process.env.CCAG_SERVER_V2_CLIENT_TOKEN
      ?? process.env.CCAG_CLIENT_TOKEN
      ?? process.env.CCAG_TOKEN,
  );
  const hostToken = trimToken(
    process.env.CCAG_SERVER_V2_HOST_TOKEN
  ...
  function resolveActor(headers: Headers): RequestActor {
    const hostHeader = headers.get("x-ccag-host-token")?.trim() ?? "";
