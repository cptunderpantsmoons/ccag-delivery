...
import { requestRemoteOpenwork, resolveRemoteWorkspaceTarget } from "../adapters/remote-ccag.js";

const MANAGED_SKILL_DOMAIN = "ccag-managed";
const CCAG_CONFIG_VERSION = 1;
...
type WorkspaceConfigSnapshot = {
  ...
  materialized: {
    ...
    compatibilityCcagPath: string | null;
    configDir: string | null;
    configOpencodePath: string | null;
    configCcagPath: string | null;
  };
  stored: {
    opencode: JsonObject;
    ccag: JsonObject;
...
function canonicalizeWorkspaceConfigState(workspace: WorkspaceRecord, config: { ccag: JsonObject; opencode: JsonObject }) {
  const nextOpenwork = asObject(config.ccag);
...
export function createConfigMaterializationService(input: {
  ...
  function workspaceCcagConfigPath(workspace: WorkspaceRecord) {
    const configDir = workspace.configDir?.trim();
    if (!configDir) {
      throw new RouteError(500, "internal_error", `Workspace ${workspace.id} is missing its config directory.`);
    }
    return path.join(configDir, ".opencode", "ccag.json");
  }
  ...
  function compatibilityCcagConfigPath(workspace: WorkspaceRecord) {
    const dataDir = workspace.dataDir?.trim();
    return dataDir ? path.join(dataDir, ".opencode", "ccag.json") : null;
  }
  ...
  function readLegacyWorkspaceState(workspace: WorkspaceRecord) {
    const ccag =
      readJsonFile(workspaceCcagConfigPath(workspace))
      ?? (compatibilityCcagConfigPath(workspace) ? readJsonFile(compatibilityCcagConfigPath(workspace)!) : null)
  ...
  function absorbWorkspaceConfigState(workspace: WorkspaceRecord) {
    ...
    const canonical = canonicalizeWorkspaceConfigState(workspace, {
      ccag: mergeObjects(buildDefaultOpenwork(workspace), legacy.ccag),
  ...
  function computeSnapshot(workspace: WorkspaceRecord): WorkspaceConfigSnapshot {
    ...
    const canonicalState = canonicalizeWorkspaceConfigState(workspace, {
      ccag: workspaceState.ccag,
      opencode: workspaceState.opencode,
    });
    const storedOpenwork = mergeObjects(buildDefaultOpenwork(workspace), canonicalState.ccag);
    ...
    return {
      effective: {
        ...
        ccag: effectiveOpenwork,
      },
      materialized: {
        compatibilityOpencodePath: compatibilityOpencodeConfigPath(workspace),
        compatibilityCcagPath: compatibilityCcagConfigPath(workspace),
        configDir: workspace.configDir,
        configOpencodePath: workspaceOpencodeConfigPath(workspace),
        configCcagPath: workspaceCcagConfigPath(workspace),
      },
      stored: {
  ...
  function materializeWorkspaceSnapshot(workspaceId: string) {
    ...
    writeJsonFile(snapshot.materialized.configCcagPath!, snapshot.effective.ccag);
    ...
    if (snapshot.materialized.compatibilityCcagPath) {
      writeJsonFile(snapshot.materialized.compatibilityCcagPath, snapshot.effective.ccag);
    }
  ...
  return {
    ...
    async patchWorkspaceConfig(workspaceId: string, patch: { ccag?: JsonObject; opencode?: JsonObject }) {
      const workspace = getWorkspaceOrThrow(workspaceId);
      if (workspace.kind === "remote") {
        const server = getRemoteServerOrThrow(workspace);
        const target = resolveRemoteWorkspaceTarget(server, workspace);
      ...
      const nextOpenwork = patch.ccag ? mergeObjects(current.ccag, asObject(patch.ccag)) : current.ccag;
    ...
    async writeWorkspaceRawOpencodeConfig(workspaceId: string, content: string) {
      ...
      upsertManagedRecords(workspace.id, "providerConfigs", recognized.providers);
      const canonical = canonicalizeWorkspaceConfigState(workspace, {
        ccag: ensureWorkspaceConfigState(workspace).ccag,
        opencode: recognized.base,
      });
      input.repositories.workspaceConfigState.upsert({
        ccag: canonical.ccag,
