...
import { requestRemoteOpenwork } from "../adapters/remote-ccag.js";
...
export function createRemoteServerService(input: {
  ...
  function buildServerRecord(payload: {
    ...
    const auth: JsonObject = {
      ...(existing?.auth ?? {}),
      ...(payload.token?.trim() ? { ccagToken: payload.token.trim(), ccagClientToken: payload.token.trim() } : {}),
      ...(payload.hostToken?.trim() ? { ccagHostToken: payload.hostToken.trim() } : {}),
    };
  ...
  return {
    async connect(inputValue: {
      ...
      if (discovered.length === 0) {
        throw new HTTPException(404, { message: "Remote CCAG server did not return any visible workspaces." });
      }
