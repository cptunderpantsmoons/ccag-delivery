...
import { requestRemoteOpenwork, requestRemoteCcagRaw, resolveRemoteWorkspaceTarget } from "../adapters/remote-ccag.js";
...
function recordAuditEntry(workspaceId: string, workspaceRoot: string, action: string, target: string, summary: string) {
  const auditRoot = process.env.CCAG_DATA_DIR?.trim()
    ? path.join(process.env.CCAG_DATA_DIR.trim(), "audit")
    : path.join(workspaceRoot, ".opencode", "ccag");
  fs.mkdirSync(auditRoot, { recursive: true });
  const filePath = process.env.CCAG_DATA_DIR?.trim()
...
function initializeWorkspaceFiles(workspaceRoot: string) {
  ...
  fs.mkdirSync(path.join(workspaceRoot, ".opencode", "ccag"), { recursive: true });
}
...
function resolveInboxDir(workspaceRoot: string) {
  return path.join(workspaceRoot, ".opencode", "ccag", "inbox");
}

function resolveOutboxDir(workspaceRoot: string) {
  return path.join(workspaceRoot, ".opencode", "ccag", "outbox");
}
  ...
  return {
    ...
    async downloadArtifact(workspaceId: string, artifactId: string) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        const response = await requestRemoteCcagRaw({
          path: remote.path,
          server: remote.server,
          timeoutMs: 30_000,
        });
