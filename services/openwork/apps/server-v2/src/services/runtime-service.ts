...
function resolveBootstrapPolicy(environment: string, explicit?: RuntimeBootstrapPolicy): RuntimeBootstrapPolicy {
  ...
  const fromEnv = process.env.CCAG_SERVER_V2_RUNTIME_BOOTSTRAP?.trim().toLowerCase();
...
function resolveRestartPolicy(overrides?: Partial<RuntimeRestartPolicy>): RuntimeRestartPolicy {
  const maxAttempts = Number.parseInt(process.env.CCAG_SERVER_V2_RUNTIME_RESTART_MAX_ATTEMPTS ?? "2", 10);
  const backoffMs = Number.parseInt(process.env.CCAG_SERVER_V2_RUNTIME_RESTART_BACKOFF_MS ?? "750", 10);
  const windowMs = Number.parseInt(process.env.CCAG_SERVER_V2_RUNTIME_RESTART_WINDOW_MS ?? "30000", 10);
...
export function createRuntimeService(options: CreateRuntimeServiceOptions): RuntimeService {
  ...
  const resolveRouterEnablement = () => {
    ...
    const forced = isTruthy(process.env.CCAG_SERVER_V2_ROUTER_FORCE) || isTruthy(process.env.CCAG_SERVER_V2_ROUTER_REQUIRED);
  ...
  const startRouter = async () => {
    ...
    const healthPort = Number.parseInt(process.env.CCAG_SERVER_V2_ROUTER_HEALTH_PORT ?? "0", 10) || await getFreePort();
    ...
    try {
      const handle = await spawnManagedBinary(
        ...
        ],
        {
          ...
          timeoutMs: Number.parseInt(process.env.CCAG_SERVER_V2_ROUTER_START_TIMEOUT_MS ?? "10000", 10) || 10_000,
        },
      );
      routerHandle = handle;
  ...
  const startOpencode = async () => {
    ...
    const configuredPort = Number.parseInt(process.env.CCAG_SERVER_V2_OPENCODE_PORT ?? "0", 10);
    const handle = await createLocalOpencode({
      ...
      config: {},
      cwd: options.workingDirectory.rootDir,
      hostname: process.env.CCAG_SERVER_V2_OPENCODE_HOST?.trim() || "127.0.0.1",
      port: configuredPort > 0 ? configuredPort : await getFreePort(),
      timeout: Number.parseInt(process.env.CCAG_SERVER_V2_OPENCODE_START_TIMEOUT_MS ?? "10000", 10) || 10_000,
    });
