...
afterEach(() => {
  delete process.env.CCAG_TOKEN;
  delete process.env.CCAG_HOST_TOKEN;
});

function createTestApp(options?: { requireAuth?: boolean; seedRegistry?: boolean }) {
  if (options?.requireAuth) {
    process.env.CCAG_TOKEN = "client-token";
    process.env.CCAG_HOST_TOKEN = "host-token";
  }
  ...
  const dependencies = createAppDependencies({
    ...
    inMemory: true,
    legacy: {
      desktopDataDir: `/tmp/ccag-server-v2-test-desktop-${Math.random().toString(16).slice(2)}`,
      orchestratorDataDir: `/tmp/ccag-server-v2-test-orchestrator-${Math.random().toString(16).slice(2)}`,
    },
    runtime: {
...
test("root info uses the shared success envelope and route conventions", async () => {
  ...
  const response = await app.request("http://ccag.local/");
...
test("system health returns a consistent envelope", async () => {
  const { app } = createTestApp();
  const response = await app.request("http://ccag.local/system/health");
  const body = await response.json();
...
test("system metadata includes phase 10 registry, runtime, and cutover state", async () => {
  const { app } = createTestApp();
  const response = await app.request("http://ccag.local/system/meta");
  const body = await response.json();
...
test("openapi route is generated from the live Hono app", async () => {
  const { app } = createTestApp();
  const response = await app.request("http://ccag.local/openapi.json");
  const document = await response.json();

  expect(response.status).toBe(200);
  expect(document.openapi).toBe("3.1.0");
  expect(document.info.title).toBe("CCAG Server V2");
...
test("runtime routes expose the initial server-owned status surfaces", async () => {
  ...
  const [opencodeResponse, routerResponse, runtimeResponse] = await Promise.all([
    app.request("http://ccag.local/system/opencode/health"),
    app.request("http://ccag.local/system/router/health"),
    app.request("http://ccag.local/system/runtime/summary"),
  ]);
...
test("not found routes use the shared error envelope", async () => {
  const { app } = createTestApp();
  const response = await app.request("http://ccag.local/nope");
  const body = await response.json();
...
test("system status reports registry summary and capabilities", async () => {
  ...
  const response = await app.request("http://ccag.local/system/status");
  const body = await response.json();
...
test("workspace list excludes hidden workspaces by default", async () => {
  const { app } = createTestApp({ seedRegistry: true });
  const response = await app.request("http://ccag.local/workspaces");
  const body = await response.json();
...
test("workspace detail hides internal workspaces from non-host readers", async () => {
  ...
  const clientResponse = await app.request(`http://ccag.local/workspaces/${hiddenWorkspaceId}`, {
    headers: {
      Authorization: "Bearer client-token",
    },
  });
  const hostResponse = await app.request(`http://ccag.local/workspaces/${hiddenWorkspaceId}`, {
    headers: {
      "X-CCAG-Host-Token": "host-token",
    },
  });
...
test("auth-protected registry reads require client or host scope", async () => {
  ...
  const anonymous = await app.request("http://ccag.local/workspaces");
  const client = await app.request("http://ccag.local/workspaces", {
    headers: {
      Authorization: "Bearer client-token",
    },
  });
  const clientHidden = await app.request("http://ccag.local/workspaces?includeHidden=true", {
    headers: {
      Authorization: "Bearer client-token",
    },
  });
  const hostInventory = await app.request("http://ccag.local/system/servers", {
    headers: {
      "X-CCAG-Host-Token": "host-token",
    },
  });
...
test("host-scoped remote server connect syncs remote workspaces into the local registry", async () => {
  ...
  try {
    ...
    const response = await app.request("http://ccag.local/system/servers/connect", {
      method: "POST",
      headers: {
