...
export function buildRemoteCcagHeaders(server: ServerRecord) {
  const auth = server.auth && typeof server.auth === "object" ? server.auth as Record<string, unknown> : null;
  const headers: Record<string, string> = {
    Accept: "application/json",
  };
  const bearer = pickString(auth, ["ccagClientToken", "ccagToken", "authToken", "token", "bearerToken"]);
  const hostToken = pickString(auth, ["ccagHostToken", "hostToken"]);
...
function unwrapEnvelope<T>(payload: unknown): T {
  if (payload && typeof payload === "object" && "ok" in (payload as Record<string, unknown>)) {
    ...
    if (record.ok === false) {
      ...
      const message = typeof error.message === "string" ? error.message : "Remote CCAG request failed.";
...
export async function requestRemoteOpenwork<T>(input: {
  ...
  const response = await fetch(`${normalizeBaseUrl(baseUrl)}${input.path}`, {
    ...
    headers: {
      ...buildRemoteCcagHeaders(input.server),
  ...
  if (response.status === 401) {
    throw new RouteError(502, "bad_gateway", "Remote CCAG server rejected the stored credentials.");
  }
  if (response.status === 403) {
    throw new RouteError(502, "bad_gateway", "Remote CCAG server rejected the stored permissions.");
  }
  if (!response.ok) {
    const message = typeof (payload as any)?.error?.message === "string"
      ...
      : typeof (payload as any)?.message === "string"
        ...
        : `Remote CCAG request failed with status ${response.status}.`;
...
export async function requestRemoteCcagRaw(input: {
  body?: BodyInit | null;
  ...
  if (!response.ok) {
    ...
    throw new RouteError(502, "bad_gateway", text.trim() || `Remote CCAG request failed with status ${response.status}.`);
  }
