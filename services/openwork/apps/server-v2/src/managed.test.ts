...
const envBackup = {
  ...
  publisherBaseUrl: process.env.CCAG_PUBLISHER_BASE_URL,
  publisherOrigin: process.env.CCAG_PUBLISHER_REQUEST_ORIGIN,
};
...
afterEach(() => {
  ...
  process.env.CCAG_PUBLISHER_BASE_URL = envBackup.publisherBaseUrl;
  process.env.CCAG_PUBLISHER_REQUEST_ORIGIN = envBackup.publisherOrigin;
...
test("managed resource routes cover MCPs, plugins, skills, shares, export/import, cloud signin, bundles, and router state", async () => {
  const { app, dependencies, root } = createTestApp("ccag-server-v2-phase8-managed");
  ...
  const mcpAdded = await app.request(`http://ccag.local/workspace/${workspaceId}/mcp`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name: "demo", config: { command: ["demo"], type: "local" } }),
  });
  const pluginsAdded = await app.request(`http://ccag.local/workspace/${workspaceId}/plugins`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ spec: "demo-plugin" }),
  });
  const skillAdded = await app.request(`http://ccag.local/workspace/${workspaceId}/skills`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ content: "## When To Use\n- Demo\n", description: "Demo skill", name: "demo-skill" }),
  });
  const systemManagedMcps = await app.request("http://ccag.local/system/managed/mcps");
  const shareExposed = await app.request(`http://ccag.local/workspaces/${workspaceId}/share`, { method: "POST" });
  const shareBody = await shareExposed.json();
  const exportConflict = await app.request(`http://ccag.local/workspaces/${workspaceId}/export?sensitive=auto`);
  const exportSafe = await app.request(`http://ccag.local/workspaces/${workspaceId}/export?sensitive=exclude`);
  ...
  const createImportWorkspace = await app.request("http://ccag.local/workspaces/local", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const importResult = await app.request(`http://ccag.local/workspaces/${importedWorkspaceId}/import`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(exportSafeBody.data),
  });
  const importedSkills = await app.request(`http://ccag.local/workspace/${importedWorkspaceId}/skills`);
  ...
  const hubInstall = await app.request(`http://ccag.local/workspace/${workspaceId}/skills/hub/hub-skill`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  try {
    const cloudPersist = await app.request("http://ccag.local/system/cloud-signin", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ auth: { authToken: "token-demo" }, cloudBaseUrl: `http://127.0.0.1:${cloudServer.port}` }),
    });
    const cloudValidated = await app.request("http://ccag.local/system/cloud-signin/validate", { method: "POST" });
    const cloudCleared = await app.request("http://ccag.local/system/cloud-signin", { method: "DELETE" });
  ...
  process.env.CCAG_PUBLISHER_BASE_URL = `http://127.0.0.1:${publisherServer.port}`;
  process.env.CCAG_PUBLISHER_REQUEST_ORIGIN = "http://127.0.0.1:3000";
  try {
    const publish = await app.request("http://ccag.local/share/bundles/publish", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ bundleType: "skills-set", payload: { ok: true } }),
    });
    const publishBody = await publish.json();
    const fetchBundle = await app.request("http://ccag.local/share/bundles/fetch", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  try {
    ...
    const telegramIdentity = await app.request(`http://ccag.local/workspace/${workspaceId}/opencode-router/identities/telegram`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ access: "private", token: "123456:demo" }),
    });
    const bindings = await app.request(`http://ccag.local/workspace/${workspaceId}/opencode-router/bindings`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ channel: "telegram", directory: workspaceRoot, peerId: "peer-1" }),
    });
    const send = await app.request(`http://ccag.local/workspace/${workspaceId}/opencode-router/send`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
test("scheduler routes list and delete jobs for a local workspace", async () => {
  const root = createTempRoot("ccag-server-v2-scheduler");
  process.env.HOME = root;
  const { app } = createTestApp("ccag-server-v2-phase8-scheduler");
  ...
  const listResponse = await app.request(`http://ccag.local/workspaces/${workspaceId}/scheduler/jobs`);
  expect(listResponse.status).toBe(200);
  expect((await listResponse.json()).data.items[0].slug).toBe("nightly-review");

  const deleteResponse = await app.request(`http://ccag.local/workspaces/${workspaceId}/scheduler/jobs/nightly-review`, {
    method: "DELETE",
  });
