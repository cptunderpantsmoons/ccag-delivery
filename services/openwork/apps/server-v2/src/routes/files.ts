...
function readActorKey(c: Context<AppBindings>) {
  ...
  const hostToken = c.req.raw.headers.get("x-ccag-host-token")?.trim() ?? "";
  return {
...
export function registerFileRoutes(app: Hono<AppBindings>) {
  ...
  app.patch(
    ...
    describeRoute({
      ...
      description: "Updates stored workspace CCAG/OpenCode config, absorbs recognized managed sections into the database, and rematerializes the effective config files.",
      responses: withCommonErrorResponses({
    ...
    async (c) => {
      ...
      if (body.ccag) {
        requestContext.services.files.emitReloadEvent(workspaceId, "config", {
          action: "updated",
          name: "ccag.json",
          path: snapshot.materialized.configCcagPath ?? undefined,
          type: "config",
        });
      }
      await requestContext.services.files.recordWorkspaceAudit(
        workspaceId,
        "config.patch",
        snapshot.materialized.configOpencodePath ?? snapshot.materialized.configCcagPath ?? workspaceId,
