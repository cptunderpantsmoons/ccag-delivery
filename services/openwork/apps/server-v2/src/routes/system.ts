...
function createOpenApiDocumentation(version: string) {
  return {
    openapi: "3.1.0",
    info: {
        title: "CCAG Server V2",
        version,
        description: [
          "OpenAPI contract for the standalone CCAG Server V2 runtime and durable registry state.",
          "",
    ...
    tags: [
      ...
      {
        name: "Sessions",
        description: "Workspace-first session and streaming primitives backed by OpenCode or remote CCAG servers.",
      },
      {
        name: "Messages",
...
export function registerSystemRoutes(app: ServerV2App, dependencies: AppDependencies) {
  ...
  app.post(
    ...
    describeRoute({
      tags: ["System"],
      summary: "Connect a remote CCAG server",
      description: "Validates a remote CCAG server through the local Server V2 process, stores the remote connection metadata, and syncs the discovered remote workspaces into the local canonical registry.",
      responses: withCommonErrorResponses({
        200: jsonResponse("Remote CCAG server connected successfully.", remoteServerConnectResponseSchema),
  ...
  app.post(
    ...
    describeRoute({
      tags: ["System"],
      summary: "Sync a remote CCAG server",
      description: "Refreshes the remote workspace inventory for a stored remote CCAG server and updates the local canonical registry mapping.",
      responses: withCommonErrorResponses({
        200: jsonResponse("Remote CCAG server synced successfully.", remoteServerConnectResponseSchema),
