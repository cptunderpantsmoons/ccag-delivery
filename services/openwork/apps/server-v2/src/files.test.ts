...
function createTestApp() {
  const root = createTempRoot("ccag-server-v2-phase7");
...
test("local workspace creation and config routes use server-owned config directories", async () => {
  ...
  const createResponse = await app.request("http://ccag.local/workspaces/local", {
    method: "POST",
    headers: {
  ...
  const configResponse = await app.request(`http://ccag.local/workspaces/${workspaceId}/config`);
  const configBody = await configResponse.json();
  expect(configResponse.status).toBe(200);
  expect(configBody.data.stored.ccag.authorizedRoots).toEqual([]);
  expect(configBody.data.effective.ccag.authorizedRoots).toEqual([]);
  expect(configBody.data.effective.opencode.permission?.external_directory).toBeUndefined();

  const patchResponse = await app.request(`http://ccag.local/workspaces/${workspaceId}/config`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ccag: { reload: { auto: true } },
      opencode: { permission: { external_directory: { [`${path.join(root, "shared-data")}/*`]: "allow" } } },
    }),
  });
  const patched = await patchResponse.json();
  expect(patchResponse.status).toBe(200);
  expect(patched.data.stored.ccag.reload.auto).toBe(true);
  expect(patched.data.stored.ccag.authorizedRoots).toEqual([]);
  expect(patched.data.effective.ccag.authorizedRoots).toEqual([path.join(root, "shared-data")]);
  ...
  const rawResponse = await app.request(`http://ccag.local/workspaces/${workspaceId}/config/opencode-raw?scope=project`);
...
test("file routes cover simple content, file sessions, inbox, artifacts, and reload events", async () => {
  ...
  const contentWrite = await app.request(`http://ccag.local/workspaces/${workspaceId}/files/content`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const contentRead = await app.request(`http://ccag.local/workspaces/${workspaceId}/files/content?path=notes/today.md`);
  ...
  const sessionCreate = await app.request(`http://ccag.local/workspaces/${workspaceId}/file-sessions`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const writeBatch = await app.request(`http://ccag.local/workspaces/${workspaceId}/file-sessions/${fileSessionId}/write-batch`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const staleBatch = await app.request(`http://ccag.local/workspaces/${workspaceId}/file-sessions/${fileSessionId}/write-batch`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      writes: [{
  ...
  const readBatch = await app.request(`http://ccag.local/workspaces/${workspaceId}/file-sessions/${fileSessionId}/read-batch`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const ops = await app.request(`http://ccag.local/workspaces/${workspaceId}/file-sessions/${fileSessionId}/operations`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const catalog = await app.request(`http://ccag.local/workspaces/${workspaceId}/file-sessions/${fileSessionId}/catalog/snapshot?prefix=docs`);
  const catalogBody = await catalog.json();
  expect(catalog.status).toBe(200);
  expect(catalogBody.data.items.some((item: any) => item.path === "docs/renamed.txt")).toBe(true);

  const upload = await app.request(`http://ccag.local/workspaces/${workspaceId}/inbox`, {
    method: "POST",
    body: (() => {
  ...
  const inboxList = await app.request(`http://ccag.local/workspaces/${workspaceId}/inbox`);
  ...
  const outboxDir = path.join(workspaceRoot, ".opencode", "ccag", "outbox");
  fs.mkdirSync(outboxDir, { recursive: true });
  fs.writeFileSync(path.join(outboxDir, "artifact.bin"), "artifact", "utf8");

  const artifacts = await app.request(`http://ccag.local/workspaces/${workspaceId}/artifacts`);
  const artifactsBody = await artifacts.json();
  expect(artifacts.status).toBe(200);
  expect(artifactsBody.data.items[0].path).toBe("artifact.bin");

  const reloads = await app.request(`http://ccag.local/workspaces/${workspaceId}/reload-events`);
  ...
  const disposed = await app.request(`http://ccag.local/workspaces/${workspaceId}/dispose`, {
    method: "POST",
  });
...
test("remote workspace config and file routes proxy through the local server", async () => {
    ...
    fetch(request) {
      ...
      if (url.pathname === "/workspaces/remote-alpha/config" && request.method === "GET") {
        return Response.json({
          ok: true,
          data: {
            effective: { opencode: { permission: { external_directory: { "/srv/alpha/*": "allow" } } }, ccag: {} },
            materialized: { compatibilityOpencodePath: null, compatibilityCcagPath: null, configDir: "/srv/config", configCcagPath: "/srv/config/.opencode/ccag.json", configOpencodePath: "/srv/config/opencode.jsonc" },
            stored: { ccag: { reload: { auto: true } }, opencode: {} },
      ...
      if (url.pathname === "/workspaces/remote-alpha/config" && request.method === "PATCH") {
        return Response.json({
          ok: true,
          data: {
            effective: { opencode: { permission: { external_directory: { "/srv/alpha/*": "allow", "/srv/shared/*": "allow" } } }, ccag: {} },
            materialized: { compatibilityOpencodePath: null, compatibilityCcagPath: null, configDir: "/srv/config", configCcagPath: "/srv/config/.opencode/ccag.json", configOpencodePath: "/srv/config/opencode.jsonc" },
            stored: { ccag: { reload: { auto: true } }, opencode: {} },
            updatedAt: new Date().toISOString(),
            workspaceId: "remote-alpha",
  ...
  try {
    ...
    const workspace = dependencies.persistence.registry.importRemoteWorkspace({
      ...
      serverAuth: { ccagToken: "remote-token" },
    ...
    const config = await app.request(`http://ccag.local/workspaces/${workspace.id}/config`);
    const configBody = await config.json();
    expect(config.status).toBe(200);
    expect(configBody.data.stored.ccag.reload.auto).toBe(true);

    const patched = await app.request(`http://ccag.local/workspaces/${workspace.id}/config`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    const contentRead = await app.request(`http://ccag.local/workspaces/${workspace.id}/files/content?path=notes.md`);
    const contentBody = await contentRead.json();
    expect(contentRead.status).toBe(200);
    expect(contentBody.data.content).toBe("remote hello");

    const contentWrite = await app.request(`http://ccag.local/workspaces/${workspace.id}/files/content`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const reloads = await app.request(`http://ccag.local/workspaces/${workspace.id}/reload-events`);
    const reloadBody = await reloads.json();
    expect(reloads.status).toBe(200);
