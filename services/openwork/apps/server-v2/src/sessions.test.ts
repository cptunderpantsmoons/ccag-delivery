...
function createTestApp() {
  const dependencies = createAppDependencies({
    ...
    inMemory: true,
    legacy: {
      desktopDataDir: `/tmp/ccag-server-v2-phase6-desktop-${Math.random().toString(16).slice(2)}`,
      orchestratorDataDir: `/tmp/ccag-server-v2-phase6-orchestrator-${Math.random().toString(16).slice(2)}`,
    },
    runtime: {
...
describe("workspace session routes", () => {
  test("serves local workspace session reads, writes, and streaming", async () => {
    ...
    const listResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions?roots=true&limit=1`);
    expect(listResponse.status).toBe(200);
    expect((await listResponse.json()).data.items[0].id).toBe("ses_1");

    const snapshotResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions/ses_1/snapshot?limit=5`);
    ...
    const createResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions`, {
      method: "POST",
    ...
    const updateResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions/ses_1`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    const promptResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions/ses_1/prompt_async`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const revertResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions/ses_1/revert`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const eventsResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/events`);
  ...
  test("routes remote workspace sessions through the mounted remote backend", async () => {
    ...
    const listResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions`);
    expect(listResponse.status).toBe(200);
    expect((await listResponse.json()).data.items[0].id).toBe("ses_1");

    const commandResponse = await app.request(`http://ccag.local/workspaces/${workspace.id}/sessions/ses_1/command`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
