...
function resolvePlatformDataRoot() {
  ...
  const devMode = isTruthy(process.env.CCAG_DEV_MODE);
  const folderName = devMode ? "com.differentai.ccag.dev" : "com.differentai.ccag";
...
function resolveRootDir(options: ResolveServerWorkingDirectoryOptions) {
  ...
  const override = process.env.CCAG_SERVER_V2_WORKDIR?.trim();
  if (override) {
    return path.resolve(override);
  }

  const sharedDataDir = process.env.CCAG_DATA_DIR?.trim();
  if (sharedDataDir) {
    return path.join(path.resolve(sharedDataDir), "server-v2");
  }

  if (options.environment === "test") {
    return path.join(process.cwd(), ".ccag-server-v2-test");
  }
...
export function resolveServerWorkingDirectory(options: ResolveServerWorkingDirectoryOptions): ServerWorkingDirectory {
  ...
  return {
    databaseDir,
    databasePath: path.join(databaseDir, "ccag-server-v2.sqlite"),
