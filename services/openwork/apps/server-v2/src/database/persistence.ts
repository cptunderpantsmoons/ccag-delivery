...
const legacyWorkspaceSchema = z.object({
  ...
  ccagClientToken: z.string().optional().nullable(),
  ccagHostToken: z.string().optional().nullable(),
  ccagHostUrl: z.string().optional().nullable(),
  ccagToken: z.string().optional().nullable(),
  ccagWorkspaceId: z.string().optional().nullable(),
  ccagWorkspaceName: z.string().optional().nullable(),
  path: z.string().default(""),
  preset: z.string().optional().nullable(),
  remoteType: z.enum(["ccag", "opencode"]).optional().nullable(),
...
function detectRemoteHostingKind(value: string) {
  ...
  if (
    ...
    hostname === "app.ccag.software" ||
    hostname.endsWith(".openworklabs.com") ||
    hostname.endsWith(".ccag.software")
  ) {
...
function legacyDesktopDataDirCandidates(explicitDir?: string) {
  ...
  const names = ["com.differentai.ccag.dev", "com.differentai.ccag", "CCAG Dev", "CCAG"];
...
function legacyOrchestratorDirCandidates(explicitDir?: string) {
  ...
  const candidates: string[] = [];
  const fromEnv = process.env.CCAG_DATA_DIR?.trim();
  if (fromEnv) {
    candidates.push(path.resolve(fromEnv));
  }

  const home = os.homedir();
  for (const name of ["ccag-orchestrator-dev-react", "ccag-orchestrator-dev", "ccag-orchestrator"]) {
    candidates.push(path.join(home, ".ccag", name));
  }
...
export function createServerPersistence(options: CreateServerPersistenceOptions): ServerPersistence {
  const inMemory = options.inMemory ?? (isTruthy(process.env.CCAG_SERVER_V2_IN_MEMORY) || options.environment === "test");
  ...
  if (shouldImportLegacyWorkspaceState && desktopWorkspaceFile) {
    try {
      ...
      for (const workspace of parsed.workspaces) {
        ...
        const remoteType = workspace.remoteType === "ccag" ? "ccag" : "opencode";
        const ccagServerBaseUrl = stripWorkspaceMount(workspace.ccagHostUrl ?? workspace.baseUrl ?? "");
        const remoteServerBaseUrl = ccagServerBaseUrl ?? normalizeUrl(workspace.baseUrl) ?? "";
        ...
        if (workspace.ccagToken?.trim()) auth.ccagToken = workspace.ccagToken.trim();
        if (workspace.ccagClientToken?.trim()) auth.ccagClientToken = workspace.ccagClientToken.trim();
        if (workspace.ccagHostToken?.trim()) auth.ccagHostToken = workspace.ccagHostToken.trim();
        ...
        const record = registry.importRemoteWorkspace({
          ...
          legacyNotes: {
            legacyDesktop: {
              ...
              displayName: workspace.displayName ?? null,
              legacyId: workspace.id,
              ccagHostUrl: workspace.ccagHostUrl ?? null,
          ...
          remoteWorkspaceId:
            ...
            parseWorkspaceIdFromUrl(workspace.ccagHostUrl ?? null) ||
  ...
  const cloudSigninFile =
    ...
    resolveExistingFile(legacyDesktopDataDirCandidates(options.legacy?.desktopDataDir), "ccag-cloud-signin.json");
  const cloudSigninReport = options.legacy?.cloudSigninJson?.trim() || cloudSigninFile
    ? createEmptyReport("imported", cloudSigninFile ?? "env:CCAG_SERVER_V2_CLOUD_SIGNIN_JSON")
    : createEmptyReport(
        "unavailable",
        null,
        {
