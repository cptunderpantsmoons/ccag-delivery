...
test("fresh bootstrap seeds the local server and hidden workspaces", () => {
  const workingDirectory = makeTempDir("ccag-server-v2-phase2-fresh");
...
test("migration runner upgrades an existing database from the first migration", () => {
  const rootDir = makeTempDir("ccag-server-v2-phase2-upgrade");
...
test("legacy workspace import only runs once across repeated boots", () => {
  const rootDir = makeTempDir("ccag-server-v2-phase2-idempotent");
  const desktopDataDir = makeTempDir("ccag-server-v2-phase2-desktop");
  const orchestratorDataDir = makeTempDir("ccag-server-v2-phase2-orchestrator");
  const localWorkspaceDir = makeTempDir("ccag-server-v2-phase2-local-workspace");
  const orchestratorOnlyWorkspaceDir = makeTempDir("ccag-server-v2-phase2-orch-only-workspace");

  fs.writeFileSync(
    path.join(desktopDataDir, "ccag-workspaces.json"),
    JSON.stringify(
      {
        ...
        workspaces: [
          ...
          },
          {
            ...
            ccagHostUrl: "https://remote.example.com",
            ccagWorkspaceId: "remote-one",
            ccagToken: "client-token",
          },
        ],
      },
      null,
  ...
  fs.writeFileSync(
    path.join(orchestratorDataDir, "ccag-orchestrator-state.json"),
    JSON.stringify(
      {
        activeId: "orch-1",
        cliVersion: "0.11.206",
...
test("deleted legacy-imported workspace stays deleted after restart", () => {
  const rootDir = makeTempDir("ccag-server-v2-phase2-delete-persist");
  const desktopDataDir = makeTempDir("ccag-server-v2-phase2-delete-desktop");
  const localWorkspaceDir = makeTempDir("ccag-server-v2-phase2-delete-workspace");
...
test("corrupt legacy workspace state is surfaced without blocking bootstrap", () => {
  const rootDir = makeTempDir("ccag-server-v2-phase2-corrupt");
  const desktopDataDir = makeTempDir("ccag-server-v2-phase2-corrupt-desktop");
  const orchestratorDataDir = makeTempDir("ccag-server-v2-phase2-corrupt-orchestrator");
  fs.writeFileSync(path.join(desktopDataDir, "ccag-workspaces.json"), "{not-json");
