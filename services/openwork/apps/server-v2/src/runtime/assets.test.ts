...
const ENV_KEYS = [
  "CCAG_SERVER_V2_RUNTIME_BUNDLE_DIR",
  "CCAG_SERVER_V2_RUNTIME_SOURCE",
  "CCAG_SERVER_V2_RUNTIME_RELEASE_DIR",
  "CCAG_SERVER_V2_RUNTIME_MANIFEST_PATH",
];
...
test("release runtime assets use manifest versions without reading repo metadata", async () => {
  ...
  const releaseRoot = makeTempDir("ccag-server-v2-release-assets");
  ...
  process.env.CCAG_SERVER_V2_RUNTIME_SOURCE = "release";
  process.env.CCAG_SERVER_V2_RUNTIME_RELEASE_DIR = releaseRoot;
  process.env.CCAG_SERVER_V2_RUNTIME_MANIFEST_PATH = manifestPath;
...
test("release runtime assets extract into the managed runtime directory and survive source bundle removal", async () => {
  ...
  const bundleRoot = makeTempDir("ccag-server-v2-release-bundle");
  const runtimeRoot = makeTempDir("ccag-server-v2-runtime-root");
  ...
  process.env.CCAG_SERVER_V2_RUNTIME_SOURCE = "release";
  process.env.CCAG_SERVER_V2_RUNTIME_BUNDLE_DIR = bundleRoot;
...
test("release runtime assets can extract from an embedded runtime bundle", async () => {
  ...
  const bundleRoot = makeTempDir("ccag-server-v2-embedded-bundle");
  const runtimeRoot = makeTempDir("ccag-server-v2-embedded-runtime-root");
  const runtimeDir = path.join(runtimeRoot, "runtime");
  fs.mkdirSync(runtimeDir, { recursive: true });
  ...
  process.env.CCAG_SERVER_V2_RUNTIME_SOURCE = "release";
  delete process.env.CCAG_SERVER_V2_RUNTIME_BUNDLE_DIR;
