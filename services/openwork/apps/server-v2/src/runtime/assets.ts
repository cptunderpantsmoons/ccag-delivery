...
export function createRuntimeAssetService(options: RuntimeAssetServiceOptions) {
  ...
  const runtimeSourcePreference = process.env.CCAG_SERVER_V2_RUNTIME_SOURCE?.trim().toLowerCase();
  const bundleRootOverride = process.env.CCAG_SERVER_V2_RUNTIME_BUNDLE_DIR?.trim();
  const releaseRootOverride = process.env.CCAG_SERVER_V2_RUNTIME_RELEASE_DIR?.trim();
  const manifestPathOverride = process.env.CCAG_SERVER_V2_RUNTIME_MANIFEST_PATH?.trim();
  ...
  const materializeManifest = async (source: RuntimeAssetSource, opencode: ResolvedRuntimeBinary, router: ResolvedRuntimeBinary) => {
    ...
    const manifestPath =
      source === "release"
        ...
        : path.join(rootDir, "manifests", runtimeTarget, `ccag-server-v2-${serverVersion}.json`);
  ...
  const ensureDevelopmentOpencodeBinary = async (version: string) => {
    ...
    const archivePath = path.join(os.tmpdir(), `ccag-server-v2-opencode-${Date.now()}-${asset}`);
    const extractDir = await mkdtemp(path.join(os.tmpdir(), "ccag-server-v2-opencode-"));
