...
export function startServer(options: StartServerOptions = {}): StartedServer {
  const host = options.host ?? process.env.CCAG_SERVER_V2_HOST ?? "127.0.0.1";
  const port = resolvePort(options.port ?? Number.parseInt(process.env.CCAG_SERVER_V2_PORT ?? "3100", 10));
  ...
  const dependencies = options.dependencies ?? createAppDependencies({
    localServer: {
      ...
      hostingKind: process.env.CCAG_SERVER_V2_HOSTING_KIND === "desktop" ? "desktop" : "self_hosted",
