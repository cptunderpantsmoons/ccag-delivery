...
const TAURI_APP_IDENTIFIER = "com.differentai.ccag";
...
//
// Override via CCAG_ELECTRON_USERDATA so dogfooders can isolate their
// Electron install from the real Tauri app.
app.setName("CCAG");
const userDataOverride = process.env.CCAG_ELECTRON_USERDATA?.trim();
if (userDataOverride) {
...
// Enable by setting CCAG_ELECTRON_REMOTE_DEBUG_PORT=<port> before launch.
const remoteDebugPort = Number.parseInt(
  process.env.CCAG_ELECTRON_REMOTE_DEBUG_PORT?.trim() ?? "",
  10,
);
...
const IDLE_CCAG_SERVER_INFO = Object.freeze({
  running: false,
...
function forwardedDeepLinks(argv) {
  return argv
    ...
    .filter(
      (entry) =>
        entry.startsWith("ccag://") ||
        entry.startsWith("ccag-dev://") ||
...
// Tauri shell writes the same data to ccag-workspaces.json. Electron
...
function legacyTauriWorkspaceStatePath() {
  return path.join(app.getPath("userData"), "ccag-workspaces.json");
}
...
async function migrateLegacyWorkspaceStateIfNeeded() {
  ...
  try {
    ...
    console.info(
      "[migration] copied ccag-workspaces.json → workspace-state.json",
    );
    return true;
  } catch (error) {
...
function defaultWorkspaceCcagConfig(workspacePath) {
  return {
    version: 1,
    workspace: null,
...
async function readWorkspaceCcagConfig(workspacePath) {
  const ccagPath = path.join(workspacePath, ".opencode", "ccag.json");
  if (!(await pathExists(ccagPath))) {
    return defaultWorkspaceCcagConfig(workspacePath);
  }
  const raw = await readFile(ccagPath, "utf8");
  return JSON.parse(raw);
}

async function writeWorkspaceCcagConfig(workspacePath, config) {
  const ccagPath = path.join(workspacePath, ".opencode", "ccag.json");
  await mkdir(path.dirname(ccagPath), { recursive: true });
  await writeFile(ccagPath, `${JSON.stringify(config, null, 2)}\n`, "utf8");
  return execResult(true, `Wrote ${ccagPath}`);
}
...
function normalizeWorkspaceEntry(input) {
  return {
    ...
    ccagHostUrl: input.ccagHostUrl ?? null,
    ccagToken: input.ccagToken ?? null,
    ccagClientToken: input.ccagClientToken ?? null,
    ccagHostToken: input.ccagHostToken ?? null,
    ccagWorkspaceId: input.ccagWorkspaceId ?? null,
    ccagWorkspaceName: input.ccagWorkspaceName ?? null,
...
async function handleDesktopInvoke(event, command, ...args) {
  switch (command) {
    ...
    case "workspaceCreate": {
      ...
      await writeWorkspaceCcagConfig(folderPath, defaultWorkspaceCcagConfig(folderPath));
      return mutateWorkspaceState((state) => {
    ...
    case "workspaceCreateRemote": {
      ...
      const workspace = normalizeWorkspaceEntry({
        ...
        name: String(input.displayName ?? input.ccagWorkspaceName ?? "Remote workspace"),
        displayName: input.displayName ?? null,
        path: String(input.directory ?? baseUrl),
        preset: "remote",
        workspaceType: "remote",
        remoteType: input.remoteType ?? "ccag",
        baseUrl,
        directory: input.directory ?? null,
        ccagHostUrl: input.ccagHostUrl ?? null,
        ccagToken: input.ccagToken ?? null,
    ...
    case "workspaceAddAuthorizedRoot": {
      ...
      const config = await readWorkspaceCcagConfig(workspacePath);
      ...
      return writeWorkspaceCcagConfig(workspacePath, config);
    }
    case "workspaceCcagRead":
      return readWorkspaceCcagConfig(String(args[0]?.workspacePath ?? "").trim());
    case "workspaceCcagWrite":
      return writeWorkspaceCcagConfig(
        String(args[0]?.workspacePath ?? "").trim(),
        args[0]?.config ?? defaultWorkspaceCcagConfig(""),
      );
    ...
    case "appBuildInfo":
      return {
        ...
        gitSha: process.env.CCAG_GIT_SHA ?? null,
        buildEpoch: process.env.CCAG_BUILD_EPOCH ?? null,
        ccagDevMode: process.env.CCAG_DEV_MODE === "1",
      };
    ...
    case "nukeCcagAndOpencodeConfigAndExit": {
    ...
    case "sandboxCleanupCcagContainers":
      return runtimeManager.sandboxCleanupCcagContainers();
    case "sandboxDebugProbe":
      return runtimeManager.sandboxDebugProbe();
    case "ccagServerInfo":
      return runtimeManager.ccagServerInfo();
    case "ccagServerRestart":
      return runtimeManager.ccagServerRestart(args[0] ?? {});
    case "pickDirectory": {
...
async function createMainWindow() {
  ...
  const startUrl = process.env.CCAG_ELECTRON_START_URL?.trim() || process.env.ELECTRON_START_URL?.trim();
  if (startUrl) {
...
ipcMain.handle("ccag:desktop", handleDesktopInvoke);
ipcMain.handle("ccag:shell:openExternal", async (_event, url) => {
  if (typeof url === "string" && url.trim().length > 0) {
    await shell.openExternal(url);
  }
});
ipcMain.handle("ccag:shell:relaunch", async () => {
  app.relaunch();
  app.exit(0);
});
...
ipcMain.handle("ccag:migration:read", async () => {
...
ipcMain.handle("ccag:migration:ack", async () => {
  const snapshotPath = migrationSnapshotPath();
...
ipcMain.handle("ccag:updater:check", async () => {
...
ipcMain.handle("ccag:updater:download", async () => {
  const updater = await ensureAutoUpdater();
...
ipcMain.handle("ccag:updater:installAndRestart", async () => {
  const updater = await ensureAutoUpdater();
  if (!updater) return { ok: false, reason: "unavailable" };
  try {
