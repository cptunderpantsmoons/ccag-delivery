...
const ORCHESTRATOR_RUNTIME = "ccag-orchestrator";
const CCAG_SERVER_PORT_RANGE_START = 48_000;
const CCAG_SERVER_PORT_RANGE_END = 51_000;
...
function createCcagServerState() {
  return {
    child: null,
    childExited: true,
...
function snapshotCcagServerState(state) {
  const child = state.childExited ? null : state.child;
  return {
    running: Boolean(child && child.exitCode === null && !child.killed),
...
export function createRuntimeManager({ app, desktopRoot, listLocalWorkspacePaths }) {
  ...
  const ccagServerState = createCcagServerState();
  ...
  function ccagServerTokenStorePath() {
    return path.join(userDataDir, "ccag-server-tokens.json");
  }

  function ccagServerStatePath() {
    return path.join(userDataDir, "ccag-server-state.json");
  }

  function orchestratorDataDir() {
    const envDir = process.env.CCAG_DATA_DIR?.trim();
    if (envDir) return envDir;
    return path.join(app.getPath("home"), ".ccag", "ccag-orchestrator");
  }

  function orchestratorStatePath(dataDir) {
    return path.join(dataDir, "ccag-orchestrator-state.json");
  }

  function orchestratorAuthPath(dataDir) {
    return path.join(dataDir, "ccag-orchestrator-auth.json");
  }
  ...
  async function loadTokenStore() {
    return readJsonFile(ccagServerTokenStorePath(), { version: 1, workspaces: {} });
  }

  async function saveTokenStore(store) {
    const filePath = ccagServerTokenStorePath();
    await mkdir(path.dirname(filePath), { recursive: true });
    await writeFile(filePath, `${JSON.stringify(store, null, 2)}\n`, "utf8");
  }

  async function loadPortState() {
    return readJsonFile(ccagServerStatePath(), {
      version: 3,
      workspacePorts: {},
      preferredPort: null,
    });
  ...
  async function savePortState(state) {
    const filePath = ccagServerStatePath();
    await mkdir(path.dirname(filePath), { recursive: true });
  ...
  async function readPreferredCcagPort(workspaceKey) {
  ...
  async function persistPreferredCcagPort(workspaceKey, port) {
    const state = await loadPortState();
    const normalized = normalizeWorkspaceKey(workspaceKey);
    state.version = 3;
  ...
  async function resolveCcagPort(host, workspaceKey) {
    const preferred = await readPreferredCcagPort(workspaceKey);
    if (preferred && (await portAvailable(host, preferred))) {
      return preferred;
    }

    for (let port = CCAG_SERVER_PORT_RANGE_START; port <= CCAG_SERVER_PORT_RANGE_END; port += 1) {
  ...
  async function ensureDevModePaths() {
    const root = path.join(userDataDir, "ccag-dev-data");
    const paths = {
  ...
  async function buildChildEnv(extra = {}) {
    ...
    if (process.env.CCAG_DEV_MODE === "1") {
  ...
  function resolveDockerCandidates() {
    ...
    for (const key of ["CCAG_DOCKER_BIN", "OPENWRK_DOCKER_BIN", "DOCKER_BIN"]) {
  ...
  function runDockerCommandDetailed(args, timeoutMs = 8000) {
    ...
    throw new Error(
      `Failed to run docker: ${errors.join("; ")} (Set CCAG_DOCKER_BIN to your docker binary if needed)`,
    );
  }
  ...
  function deriveOrchestratorContainerName(runId) {
    const sanitized = String(runId ?? "")
      ...
      .slice(0, 24);
    return `ccag-orchestrator-${sanitized}`;
  }

  async function listCcagManagedContainers() {
    ...
    return result.stdout
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter((name) => name && (name.startsWith("ccag-orchestrator-") || name.startsWith("ccag-dev-") || name.startsWith("openwrk-")))
      .sort();
  }
  ...
  async function issueOwnerToken(baseUrl, hostToken) {
    const payload = await fetchJson(
      `${baseUrl.replace(/\/+$/, "")}/tokens`,
      {
        ...
        body: JSON.stringify({ scope: "owner", label: "CCAG desktop owner token" }),
      },
      5000,
    );
  ...
  async function startCcagServer(options) {
    await stopChild(ccagServerState);
    ...
    const port = await resolveCcagPort(host, activeWorkspace);
    const baseUrl = `http://127.0.0.1:${port}`;
    const tokens = await loadOrCreateWorkspaceTokens(activeWorkspace);
    const program = resolveBinary("ccag-server");
    if (!program) {
      throw new Error("Failed to locate ccag-server.");
    }
    ...
    const env = await buildChildEnv({
      CCAG_TOKEN: tokens.clientToken,
      CCAG_HOST_TOKEN: tokens.hostToken,
      ...(options.routerHealthPort ? { OPENCODE_ROUTER_HEALTH_PORT: String(options.routerHealthPort) } : {}),
      ...(options.opencodeUsername ? { CCAG_OPENCODE_USERNAME: options.opencodeUsername } : {}),
      ...(options.opencodePassword ? { CCAG_OPENCODE_PASSWORD: options.opencodePassword } : {}),
    });

    spawnManagedChild(ccagServerState, program, args, {
      cwd: activeWorkspace || desktopRoot,
      env,
    });

    ccagServerState.remoteAccessEnabled = options.remoteAccessEnabled;
    ccagServerState.host = host;
    ccagServerState.port = port;
    ccagServerState.baseUrl = baseUrl;
    ccagServerState.clientToken = tokens.clientToken;
    ccagServerState.hostToken = tokens.hostToken;

    const connectUrls = options.remoteAccessEnabled ? buildConnectUrls(port) : { connectUrl: null, mdnsUrl: null, lanUrl: null };
    ccagServerState.connectUrl = connectUrls.connectUrl;
    ccagServerState.mdnsUrl = connectUrls.mdnsUrl;
    ccagServerState.lanUrl = connectUrls.lanUrl;
    ...
    ccagServerState.ownerToken = ownerToken;
    if (ownerToken) {
      await persistWorkspaceOwnerToken(activeWorkspace, ownerToken);
    }
    await persistPreferredCcagPort(activeWorkspace, port);
    return snapshotCcagServerState(ccagServerState);
  }
  ...
  async function startOrchestratorRuntime(projectDir, options = {}) {
    ...
    const orchestratorProgram = resolveBinary("ccag-orchestrator") ?? resolveBinary("ccag");
    if (!orchestratorProgram) {
      throw new Error("Failed to locate ccag-orchestrator.");
    }
    ...
    const env = await buildChildEnv({
      CCAG_INTERNAL_ALLOW_OPENCODE_CREDENTIALS: "1",
      CCAG_OPENCODE_USERNAME: username,
      CCAG_OPENCODE_PASSWORD: password,
  ...
  async function stopAllRuntimeChildren() {
    ...
    Object.assign(ccagServerState, createCcagServerState());
  ...
  async function ensureRouterAndOpenwork(options) {
    ...
    try {
      ...
      appendOutput(engineState, "lastStderr", `CCAG server: ${error instanceof Error ? error.message : String(error)}\n`);
    }
  ...
  async function engineStart(projectDir, options = {}) {
    ...
    await ensureRouterAndOpenwork({
      ...
      workspacePaths,
      remoteAccessEnabled: options.ccagRemoteAccess === true,
    });
  ...
  async function engineRestart(options = {}) {
    ...
    return engineStart(projectDir, {
      ...
      ccagRemoteAccess: options.ccagRemoteAccess,
    });
  }
  ...
  async function ccagServerInfo() {
    return snapshotCcagServerState(ccagServerState);
  }

  async function ccagServerRestart(options = {}) {
    const workspacePaths = (await listLocalWorkspacePaths()).filter(Boolean);
    return startCcagServer({
      workspacePaths,
      opencodeBaseUrl: engineState.baseUrl,
      opencodeUsername: engineState.opencodeUsername,
      opencodePassword: engineState.opencodePassword,
  ...
  async function engineInstall() {
    if (process.platform === "win32") {
      return {
        ...
        stdout: "",
        stderr:
          "Guided install is not supported on Windows yet. Install the CCAG-pinned OpenCode version manually, then restart CCAG.",
      };
    }
  ...
  async function sandboxStop(containerName) {
    ...
    if (!name.startsWith("ccag-orchestrator-")) {
      throw new Error("Refusing to stop container: expected name starting with 'ccag-orchestrator-'");
    }
  ...
  async function sandboxCleanupCcagContainers() {
    const candidates = await listCcagManagedContainers().catch((error) => {
      throw error;
    });
    const removed = [];
    const errors = [];
  ...
  async function orchestratorStartDetached(options = {}) {
    ...
    const port = await findFreePort("127.0.0.1");
    const token = String(options.ccagToken ?? randomUUID()).trim();
    const hostToken = String(options.ccagHostToken ?? randomUUID()).trim();
    const ccagUrl = `http://127.0.0.1:${port}`;
    const program = resolveBinary("ccag-orchestrator") ?? resolveBinary("ccag");
    if (!program) {
      throw new Error("Failed to locate ccag orchestrator.");
    }
    ...
    const child = spawn(program, args, {
      env: { ...(await buildChildEnv()), CCAG_TOKEN: token, CCAG_HOST_TOKEN: hostToken },
      detached: true,
      stdio: "ignore",
      windowsHide: true,
    });
    ...
    await waitForHttpOk(`${ccagUrl}/health`, wantsDockerSandbox ? 90_000 : 12_000);
    const ownerToken = await issueOwnerToken(ccagUrl, hostToken).catch(() => null);
  ...
  async function sandboxDebugProbe() {
    ...
    const workspacePath = path.join(os.tmpdir(), `ccag-sandbox-probe-${randomUUID()}`);
