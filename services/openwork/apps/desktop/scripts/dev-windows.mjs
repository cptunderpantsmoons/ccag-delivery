...
const stopStaleSidecars = () => {
  ...
  const command = [
    `$targetDir = \"${targetDir}\"`,
    "$names = @('opencode.exe','opencode-router.exe','ccag-server.exe','ccag-orchestrator.exe','chrome-devtools-mcp.exe')",
...
const env = {
  ...
  CCAG_DATA_DIR:
    process.env.CCAG_DATA_DIR ||
    `${homedir()}${process.platform === "win32" ? "\\" : "/"}.ccag${process.platform === "win32" ? "\\" : "/"}ccag-orchestrator-dev`,
  CCAG_USE_COREPACK_PNPM: "1",
