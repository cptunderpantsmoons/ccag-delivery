...
const useCorepackPnpm = process.platform === "win32" && process.env.CCAG_USE_COREPACK_PNPM === "1";
...
const hostOverride = process.env.CCAG_DEV_HOST?.trim() || null;
const port = readPort();
...
const fetchCCAGDevServerId = async (baseUrl) => {
  try {
    const res = await fetchWithTimeout(`${baseUrl}/__ccag_dev_server_id`, { timeoutMs: 1200 });
...
const runPrepareSidecars = () => {
  ...
  if (process.env.CCAG_SIDECAR_FORCE_BUILD !== "0") {
    args.push("--force");
  }
...
const ensureLinuxDesktopDependencies = () => {
  ...
  console.error(
    "[ccag] Missing Linux desktop system dependencies required by Tauri:\n" +
      `  ${missing.join(", ")}\n\n` +
...
const runUiDevServer = () => {
  const uiArgs =
    process.platform === "win32"
      ? [...pnpmArgs, "--filter", "@ccag/app", "dev:windows"]
      : [...pnpmArgs, "-w", "dev:ui"];
  const child = spawn(pnpmCmd, uiArgs, {
    ...
    env: {
      ...
      PORT: String(port),
      CCAG_DEV_MODE: process.env.CCAG_DEV_MODE || "1",
    },
  });
...
const main = async () => {
  ...
  for (const candidate of baseUrls) {
    if (await looksLikeVite(candidate)) {
      ...
      detectedViteAppRoot = await fetchCCAGDevServerId(candidate);
      break;
    }
  }
  ...
  if (detectedViteUrl) {
    if (detectedViteAppRoot && detectedViteAppRoot !== expectedAppRoot) {
      console.error(
        `[ccag] Found a Vite dev server at ${detectedViteUrl}, but it serves a different checkout:\n` +
    ...
    if (!detectedViteAppRoot) {
      console.error(
        `[ccag] Found a Vite dev server at ${detectedViteUrl}, but could not verify which checkout it belongs to.\n` +
          `Stop the other dev server or run with a different PORT (for example: PORT=5174 pnpm dev).`
      );
      process.exit(1);
    }
    console.log(`[ccag] UI dev server already running at ${detectedViteUrl} (reusing).`);
    holdOpenUntilSignal();
    return;
  }
  ...
  if (portInUse) {
    console.error(
      `[ccag] Port ${port} is in use, but it does not look like a Vite dev server.\n` +
        `Set PORT to a free port (e.g. PORT=5174) or stop the process using port ${port}.`
    );
    process.exit(1);
  }

  console.log(`[ccag] Starting UI dev server on port ${port}...`);
