...
const explicitStartUrl = process.env.CCAG_ELECTRON_START_URL?.trim() || "";
...
// CCAG_ELECTRON_REMOTE_DEBUG_PORT=<port> or set to "0" to disable.
const defaultCdpPort = "9823";
const cdpPortRaw = process.env.CCAG_ELECTRON_REMOTE_DEBUG_PORT?.trim() ?? defaultCdpPort;
...
electronChild = run(pnpmCmd, ["exec", "electron", "./electron/main.mjs"], {
  ...
  env: {
    ...process.env,
    CCAG_DEV_MODE: process.env.CCAG_DEV_MODE ?? "1",
    CCAG_ELECTRON_START_URL: resolvedStartUrl,
    ...(cdpPort ? { CCAG_ELECTRON_REMOTE_DEBUG_PORT: cdpPort } : {}),
  },
});

if (cdpPort) {
  console.log(`[ccag] Electron CDP exposed at http://127.0.0.1:${cdpPort}`);
}
