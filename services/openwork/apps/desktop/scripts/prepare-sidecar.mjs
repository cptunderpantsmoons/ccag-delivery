...
const forceBuild = hasFlag("--force") || process.env.CCAG_SIDECAR_FORCE_BUILD === "1";
const sidecarOverride = process.env.CCAG_SIDECAR_DIR?.trim() || readArg("--outdir");
...
const opencodeGithubRepo = (() => {
  const raw =
    ...
    process.env.CCAG_OPENCODE_GITHUB_REPO?.trim() ||
    "anomalyco/opencode";
...
const chromeDevtoolsMcpVersion =
  ...
  process.env.CCAG_CHROME_DEVTOOLS_MCP_VERSION?.trim() ||
  "0.17.0";
...
// ccag-server paths
const ccagServerBaseName = "ccag-server";
const ccagServerName = process.platform === "win32" ? `${ccagServerBaseName}.exe` : ccagServerBaseName;
const ccagServerPath = join(sidecarDir, ccagServerName);
const ccagServerBuildName = bunTarget
  ? `${ccagServerBaseName}-${bunTarget}${bunTarget.includes("windows") ? ".exe" : ""}`
  : ccagServerName;
const ccagServerBuildPath = join(sidecarDir, ccagServerBuildName);
const ccagServerTargetTriple = resolvedTargetTriple;
const ccagServerTargetName = ccagServerTargetTriple
  ? `${ccagServerBaseName}-${ccagServerTargetTriple}${ccagServerTargetTriple.includes("windows") ? ".exe" : ""}`
  : null;
const ccagServerTargetPath = ccagServerTargetName ? join(sidecarDir, ccagServerTargetName) : null;

const ccagServerDir = resolve(__dirname, "..", "..", "server");
...
// orchestrator paths
const orchestratorBaseName = "ccag-orchestrator";
...
let didBuildCcagServer = false;
const shouldBuildCcagServer =
  forceBuild || !existsSync(ccagServerBuildPath) || isStubBinary(ccagServerBuildPath);

if (shouldBuildCcagServer) {
  mkdirSync(sidecarDir, { recursive: true });
  if (existsSync(ccagServerBuildPath)) {
    try {
      unlinkSync(ccagServerBuildPath);
    } catch {
      // ignore
    }
  }
  const ccagServerScript = resolveBuildScript(ccagServerDir);
  if (!existsSync(ccagServerScript)) {
    console.error(`CCAG server build script not found at ${ccagServerScript}`);
    process.exit(1);
  }
  const ccagServerArgs = [ccagServerScript, "--outdir", sidecarDir, "--filename", "ccag-server"];
  if (bunTarget) {
    ccagServerArgs.push("--target", bunTarget);
  }
  const buildResult = spawnSync("bun", ccagServerArgs, {
    cwd: ccagServerDir,
    stdio: "inherit",
    shell: true,
  });
  ...
  didBuildCcagServer = true;
}

if (existsSync(ccagServerBuildPath)) {
  const shouldCopyCanonical = didBuildCcagServer || !existsSync(ccagServerPath) || isStubBinary(ccagServerPath);
  if (shouldCopyCanonical && ccagServerBuildPath !== ccagServerPath) {
    try {
      if (existsSync(ccagServerPath)) {
        unlinkSync(ccagServerPath);
      }
    } catch {
      // ignore
    }
    copyFileSync(ccagServerBuildPath, ccagServerPath);
  }

  if (ccagServerTargetPath) {
    const shouldCopyTarget =
      didBuildCcagServer || !existsSync(ccagServerTargetPath) || isStubBinary(ccagServerTargetPath);
    if (shouldCopyTarget && ccagServerBuildPath !== ccagServerTargetPath) {
      try {
        if (existsSync(ccagServerTargetPath)) {
          unlinkSync(ccagServerTargetPath);
        }
      } catch {
        // ignore
      }
      copyFileSync(ccagServerBuildPath, ccagServerTargetPath);
    }
  }
}
...
const ccagServerVersion = (() => {
  try {
    const raw = readFileSync(resolve(ccagServerDir, "package.json"), "utf8");
...
const versions = {
  ...
  "ccag-server": {
    version: ccagServerVersion,
    sha256: existsSync(ccagServerPath) ? sha256File(ccagServerPath) : null,
  },
  opencodeRouter: {
