...
use commands::misc::{
    app_build_info, nuke_ccag_and_opencode_config_and_exit, opencode_mcp_auth,
    reset_opencode_cache, reset_ccag_state,
};
use commands::opencode_router::{
    opencodeRouter_config_set, opencodeRouter_info, opencodeRouter_start, opencodeRouter_status,
    opencodeRouter_stop,
};
use commands::ccag_server::{ccag_server_info, ccag_server_restart};
use commands::orchestrator::{
    orchestrator_instance_dispose, orchestrator_start_detached, orchestrator_status,
    orchestrator_workspace_activate, sandbox_cleanup_ccag_containers, sandbox_debug_probe,
...
use commands::workspace::{
    ...
    workspace_export_config, workspace_forget, workspace_import_config, workspace_ccag_read,
    workspace_ccag_write, workspace_set_active, workspace_set_runtime_active, workspace_set_selected,
    workspace_update_display_name, workspace_update_remote,
};
use engine::manager::EngineManager;
use opencode_router::manager::OpenCodeRouterManager;
use ccag_server::manager::CcagServerManager;
use orchestrator::manager::OrchestratorManager;
use tauri::{AppHandle, Emitter, Manager, RunEvent, WindowEvent};
use workspace::watch::WorkspaceWatchState;

const NATIVE_DEEP_LINK_EVENT: &str = "ccag:deep-link-native";
...
fn set_dev_app_name() {
    ...
    objc2_foundation::NSProcessInfo::processInfo()
        .setProcessName(&objc2_foundation::NSString::from_str("ccag - Dev"));
}
...
fn forwarded_deep_links(args: &[String]) -> Vec<String> {
    args.iter()
        ...
        .filter_map(|arg| {
            ...
            if trimmed.starts_with("ccag://")
                || trimmed.starts_with("ccag-dev://")
...
fn stop_managed_services(app_handle: &tauri::AppHandle) {
    ...
    if let Ok(mut ccag_server) = app_handle.state::<CcagServerManager>().inner.lock() {
        CcagServerManager::stop_locked(&mut ccag_server);
    }
...
pub fn run() {
    ...
    let app = builder
        ...
        .manage(CcagServerManager::default())
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            sandbox_debug_probe,
            sandbox_stop,
            sandbox_cleanup_ccag_containers,
            ccag_server_info,
            ccag_server_restart,
            opencodeRouter_info,
        ...
        .expect("error while building ccag");
    ...
    // orchestrator/opencode/ccag-server processes and stale ports.
