...
pub fn resolve_orchestrator_data_dir() -> String {
    ...
    if let Some(home) = home_dir() {
        return home
            .join(".ccag")
            .join("ccag-orchestrator")
            .to_string_lossy()
            .to_string();
    }

    ".ccag/ccag-orchestrator".to_string()
}

fn orchestrator_state_path(data_dir: &str) -> PathBuf {
    Path::new(data_dir).join("ccag-orchestrator-state.json")
}

fn orchestrator_auth_path(data_dir: &str) -> PathBuf {
    Path::new(data_dir).join("ccag-orchestrator-auth.json")
}
...
pub fn spawn_orchestrator_daemon(
    ...
    let command = match app.shell().sidecar("ccag-orchestrator") {
        Ok(command) => command,
        Err(_) => app.shell().command("ccag"),
    };
...
mod tests {
    ...
    fn request_shutdown_returns_false_without_state() {
        let dir = std::env::temp_dir().join(format!(
            "ccag-orchestrator-shutdown-missing-{}",
            Uuid::new_v4()
        ));
    ...
    fn request_shutdown_posts_to_daemon_endpoint() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "ccag-orchestrator-shutdown-state-{}",
            Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).expect("create state dir");
        let state_path = dir.join("ccag-orchestrator-state.json");
        fs::write(
            &state_path,
            format!(
