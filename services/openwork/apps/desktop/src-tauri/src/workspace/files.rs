...
use crate::types::{OpencodeCommand, WorkspaceCcagConfig};
...
description: Workspace guide to introduce ccag and onboard new users.
---

# Welcome to ccag

Hi, I'm Ben and this is ccag. It's an open-source alternative to Claude's cowork. It helps you work on your files with AI and automate the mundane tasks so you don't have to.
...
ccag feels like a chat app, but it can safely work with the files you allow. Put files in this workspace and I can summarize them, create new ones, or help organize them.
...
ccag is a GUI for OpenCode. Everything that works in OpenCode works here.

Most reliable setup today:
1) Install the ccag-pinned OpenCode version
2) Configure providers there (models and API keys)
3) Come back to ccag and start a session
...
End with two friendly next actions to try in ccag."#;
...
- Reply with these four lines, exactly and in order:
  1) hey there welcome this is ccag
...
fn seed_ccag_agent(agent_root: &PathBuf) -> Result<(), String> {
    let agent_path = agent_root.join("ccag.md");
    ...
    let doc = r#"---
description: ccag default agent (safe, mobile-first, self-referential)
mode: primary
temperature: 0.2
---
...
When the user refers to \"you\", they mean the ccag app and the current workspace.
...
const ENTERPRISE_ARCHIVE_URL: &str =
    "https://github.com/different-ai/ccag-enterprise/archive/refs/heads/main.zip";
const ENTERPRISE_SEED_MARKER: &str = ".ccag-enterprise-creators";
...
pub fn ensure_workspace_files(workspace_path: &str, preset: &str) -> Result<(), String> {
    ...
    if let Some(obj) = config.as_object_mut() {
        ...
        if current.is_empty() {
            obj.insert(
                ...
                serde_json::Value::String("ccag".to_string()),
            );
            config_changed = true;
        }
    }
    ...
    let ccag_path = root.join(".opencode").join("ccag.json");
    if !ccag_path.exists() {
        let ccag = WorkspaceCcagConfig::new(workspace_path, preset, now_ms());

        fs::create_dir_all(ccag_path.parent().unwrap())
            .map_err(|e| format!("Failed to create {}: {e}", ccag_path.display()))?;

        fs::write(
            &ccag_path,
            serde_json::to_string_pretty(&ccag).map_err(|e| e.to_string())?,
        )
