...
pub fn ccag_state_paths(app: &tauri::AppHandle) -> Result<(PathBuf, PathBuf), String> {
    let data_dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?;
    let file_path = data_dir.join("ccag-workspaces.json");
    ...
    for workspace in state.workspaces.iter_mut() {
        let next_id = match workspace.workspace_type {
            ...
            WorkspaceType::Remote => {
                if workspace.remote_type == Some(crate::types::RemoteType::ccag) {
                    stable_workspace_id_for_openwork(
                        workspace.ccag_host_url.as_deref().unwrap_or(""),
                        workspace.ccag_workspace_id.as_deref(),
                    )
                } else {
...
pub fn load_workspace_state(app: &tauri::AppHandle) -> Result<WorkspaceState, String> {
    let (_, path) = ccag_state_paths(app)?;
    if !path.exists() {
...
pub fn save_workspace_state(app: &tauri::AppHandle, state: &WorkspaceState) -> Result<(), String> {
    let (dir, path) = ccag_state_paths(app)?;
...
pub fn stable_workspace_id_for_openwork(host_url: &str, workspace_id: Option<&str>) -> String {
    let mut key = format!("ccag::{host_url}");
...
mod tests {
    ...
    fn normalize_local_workspace_path_expands_home_prefix() {
        ...
        let expected = home.join("ccag").join("ccag-state-test-expand");
        let actual = normalize_local_workspace_path("~/ccag/ccag-state-test-expand");
    ...
    fn repair_workspace_state_preserves_selected_and_watched_ids_independently() {
        let temp = std::env::temp_dir().join(format!(
            "ccag-workspace-state-selected-watched-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("clock")
    ...
    fn repair_workspace_state_defaults_watched_id_to_selected_when_missing() {
        let temp = std::env::temp_dir().join(format!(
            "ccag-workspace-state-default-watch-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("clock")
