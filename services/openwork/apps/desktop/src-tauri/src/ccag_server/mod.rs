...
use crate::types::CcagServerInfo;
...
use manager::CcagServerManager;
use spawn::{resolve_ccag_port, spawn_ccag_server};
...
struct PersistedCcagServerTokens {
    client_token: String,
    host_token: String,
...
struct PersistedCcagServerTokenStore {
    version: u32,
    workspaces: HashMap<String, PersistedCcagServerTokens>,
}

#[derive(Debug, Default, Serialize, Deserialize)]
struct PersistedCcagServerState {
    version: u32,
    #[serde(default)]
...
fn ccag_server_token_store_path(app: &AppHandle) -> Result<PathBuf, String> {
    let data_dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?;
    Ok(data_dir.join("ccag-server-tokens.json"))
}

fn ccag_server_state_path(app: &AppHandle) -> Result<PathBuf, String> {
    let data_dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?;
    Ok(data_dir.join("ccag-server-state.json"))
}
...
fn load_ccag_server_token_store(
    path: &Path,
) -> Result<PersistedCcagServerTokenStore, String> {
    if !path.exists() {
        return Ok(PersistedCcagServerTokenStore {
    ...
    let mut store: PersistedCcagServerTokenStore = serde_json::from_str(&raw)
...
fn save_ccag_server_token_store(
    path: &Path,
    store: &PersistedCcagServerTokenStore,
) -> Result<(), String> {
...
fn load_ccag_server_state(path: &Path) -> Result<PersistedCcagServerState, String> {
    if !path.exists() {
        return Ok(PersistedCcagServerState {
    ...
    let raw =
        fs::read_to_string(path).map_err(|e| format!("Failed to read {}: {e}", path.display()))?;
    let mut state: PersistedCcagServerState = serde_json::from_str(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", path.display()))?;
...
fn save_ccag_server_state(
    path: &Path,
    state: &PersistedCcagServerState,
) -> Result<(), String> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
...
fn read_preferred_ccag_port_at_path(
    path: &Path,
    workspace_key: &str,
) -> Result<Option<u16>, String> {
    let state = load_ccag_server_state(path)?;
...
fn read_preferred_ccag_port(app: &AppHandle, workspace_key: &str) -> Result<Option<u16>, String> {
    let path = ccag_server_state_path(app)?;
    read_preferred_ccag_port_at_path(&path, workspace_key)
}

fn reserved_ccag_ports_at_path(
    path: &Path,
...
fn reserved_ccag_ports(app: &AppHandle, exclude_workspace_key: &str) -> Result<HashSet<u16>, String> {
    let path = ccag_server_state_path(app)?;
    reserved_ccag_ports_at_path(&path, exclude_workspace_key)
}

fn persist_preferred_ccag_port_at_path(
    path: &Path,
    workspace_key: &str,
    port: u16,
) -> Result<(), String> {
    let mut state = load_ccag_server_state(path)?;
    state.version = OPENWORK_SERVER_STATE_VERSION;
    let normalized = normalize_workspace_key(workspace_key);
    if normalized.is_empty() {
    ...
    save_ccag_server_state(path, &state)
}

fn persist_preferred_ccag_port(
    app: &AppHandle,
    workspace_key: &str,
    port: u16,
) -> Result<(), String> {
    let path = ccag_server_state_path(app)?;
    persist_preferred_ccag_port_at_path(&path, workspace_key, port)
}

fn load_or_create_workspace_tokens(
    app: &AppHandle,
    workspace_key: &str,
) -> Result<PersistedCcagServerTokens, String> {
    let path = ccag_server_token_store_path(app)?;
...
fn load_or_create_workspace_tokens_at_path(
    ...
    let mut store = load_ccag_server_token_store(path)?;
    let normalized = normalize_workspace_key(workspace_key);
    if let Some(tokens) = store.workspaces.get(&normalized) {
        return Ok(tokens.clone());
    }

    let tokens = PersistedCcagServerTokens {
    ...
    save_ccag_server_token_store(path, &store)?;
    Ok(tokens)
}
...
fn wait_for_ccag_health(base_url: &str, timeout: Duration) -> Result<(), String> {
    let deadline = Instant::now() + timeout;
    let health_url = format!("{}/health", base_url.trim_end_matches('/'));
    let mut last_error = "ccag server did not become healthy".to_string();
    ...
    while Instant::now() < deadline {
        match ureq::get(&health_url).call() {
            ...
            Ok(response) => {
                last_error = format!(
                    "ccag server health check returned {}",
                    response.status()
                )
            }
...
fn issue_owner_token(base_url: &str, host_token: &str) -> Result<String, String> {
    let response = ureq::post(&format!("{}/tokens", base_url.trim_end_matches('/')))
        .set("X-ccag-Host-Token", host_token)
        .set("Content-Type", "application/json")
        .send_string(r#"{"scope":"owner","label":"ccag desktop owner token"}"#)
    ...
    payload
        ...
        .ok_or_else(|| "ccag server did not return an owner token".to_string())
}
...
pub fn start_ccag_server(
    app: &AppHandle,
    manager: &CcagServerManager,
...
) -> Result<CcagServerInfo, String> {
    let mut state = manager
        .inner
        .lock()
        .map_err(|_| "ccag server mutex poisoned".to_string())?;
    CcagServerManager::stop_locked(&mut state);
    ...
    let active_workspace = workspace_paths
        ...
        .unwrap_or("");
    let preferred_port = read_preferred_ccag_port(app, active_workspace)?;
    let reserved_ports = reserved_ccag_ports(app, active_workspace)?;
    let port = resolve_ccag_port(&host, preferred_port, &reserved_ports)?;
    ...
    let (mut rx, child) = spawn_ccag_server(
        app,
        &host,
        port,
        workspace_paths,
    ...
    if state.owner_token.is_none() {
        state.owner_token = wait_for_ccag_health(&base_url, Duration::from_secs(10))
            .ok()
    ...
    let _ = persist_preferred_ccag_port(app, active_workspace, port);
                ...
                CommandEvent::Terminated(payload) => {
                    if let Ok(mut state) = state_handle.try_lock() {
                        ...
                        if let Some(code) = payload.code {
                            let next = format!("ccag server exited (code {code}).");
                            state.last_stderr = Some(truncate_output(&next, 8000));
                        }
                    }
                }
    ...
    Ok(CcagServerManager::snapshot_locked(&mut state))
}
...
mod tests {
    ...
    fn unique_temp_path(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("ccag-server-{name}-{nonce}.json"))
    }
    ...
    fn migrates_legacy_fixed_port_to_no_preference() {
        ...
        let state = load_ccag_server_state(&path).expect("load migrated state");
    ...
    fn reuses_workspace_tokens_across_canonical_path_aliases() {
        ...
        let workspace = PathBuf::from(format!(
            "/tmp/ccag-workspace-token-alias-{}-{}",
            std::process::id(),
            now_ms()
        ));
    ...
    fn reuses_workspace_port_across_canonical_path_aliases() {
        ...
        persist_preferred_ccag_port_at_path(&path, &workspace.to_string_lossy(), 49_123)
            .expect("persist preferred port using raw path");

        let preferred = read_preferred_ccag_port_at_path(&path, &canonical.to_string_lossy())
            .expect("read preferred port using canonical path");
        let reserved = reserved_ccag_ports_at_path(&path, &canonical.to_string_lossy())
