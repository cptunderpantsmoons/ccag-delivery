...
pub fn resolve_ccag_port(
    ...
    Err("Failed to find a free ccag server port".to_string())
}

#[cfg(test)]
mod tests {
    use super::{resolve_ccag_port, OPENWORK_PORT_RANGE_END, OPENWORK_PORT_RANGE_START};
    use std::collections::HashSet;
    use std::net::TcpListener;
    ...
    fn uses_preferred_port_when_available() {
        ...
        let resolved = resolve_ccag_port("127.0.0.1", Some(port), &HashSet::new())
    ...
    fn falls_back_to_ephemeral_port_when_preferred_port_is_unavailable() {
        ...
        let resolved = resolve_ccag_port("127.0.0.1", Some(preferred_port), &HashSet::new())
    ...
    fn uses_range_port_when_no_preference_exists() {
        let resolved =
            resolve_ccag_port("127.0.0.1", None, &HashSet::new()).expect("resolve range port");
    ...
    fn skips_reserved_ports_from_other_workspaces() {
        ...
        let resolved =
            resolve_ccag_port("127.0.0.1", Some(OPENWORK_PORT_RANGE_START), &reserved)
...
pub fn build_ccag_args(
    ...
    let mut args = vec![
        ...
        "--port".to_string(),
        port.to_string(),
        // Always allow all origins since the ccag server is designed to accept
...
pub fn spawn_ccag_server(
    ...
    let command = match app.shell().sidecar("ccag-server") {
        Ok(command) => command,
        Err(_) => app.shell().command("ccag-server"),
    };

    let args = build_ccag_args(
        host,
        port,
        workspace_paths,
        opencode_base_url,
    ...
    command
        .spawn()
        .map_err(|e| format!("Failed to start ccag server: {e}"))
}
