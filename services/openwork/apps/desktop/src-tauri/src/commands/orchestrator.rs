...
const SANDBOX_PROGRESS_EVENT: &str = "ccag://sandbox-create-progress";
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CcagDockerCleanupResult {
    pub candidates: Vec<String>,
...
fn derive_orchestrator_container_name(run_id: &str) -> String {
    // Must match ccag-orchestrator's docker naming scheme:
    // `ccag-orchestrator-${runId.replace(/[^a-zA-Z0-9_.-]+/g, "-").slice(0, 24)}`
    ...
    format!("ccag-orchestrator-{sanitized}")
}

fn is_ccag_managed_container(name: &str) -> bool {
    name.starts_with("ccag-orchestrator-")
        || name.starts_with("ccag-dev-")
        || name.starts_with("openwrk-")
}

fn list_ccag_managed_containers() -> Result<Vec<String>, String> {
    ...
    let mut names: Vec<String> = stdout
        ...
        .filter(|name| !name.is_empty() && is_ccag_managed_container(name))
        .collect();
    names.sort();
    names.dedup();
    Ok(names)
...
fn format_sandbox_start_timeout_error(
    ...
    format!(
        "Timed out waiting for ccag server ({})",
        details.join(", ")
    )
}

fn issue_owner_token(ccag_url: &str, host_token: &str) -> Result<String, String> {
    let response = ureq::post(&format!("{}/tokens", ccag_url.trim_end_matches('/')))
        .set("X-ccag-Host-Token", host_token)
        .set("Content-Type", "application/json")
        .send_string(r#"{"scope":"owner","label":"ccag detached owner token"}"#)
...
pub fn orchestrator_start_detached(
    ...
    ccag_token: Option<String>,
    ccag_host_token: Option<String>,
    ...
    let token = ccag_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| Uuid::new_v4().to_string());
    let host_token = ccag_host_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| Uuid::new_v4().to_string());
    let ccag_url = format!("http://127.0.0.1:{port}");
    ...
    if wants_docker_sandbox {
        ...
        emit_sandbox_progress(
            ...
            json!({
                ...
                "hasCcagDockerBinOverride": env::var("OPENWORK_DOCKER_BIN").ok().is_some(),
    ...
    let (command, command_label) = match app.shell().sidecar("ccag-orchestrator") {
        Ok(command) => (command, "sidecar:ccag-orchestrator".to_string()),
        Err(_) => (app.shell().command("ccag"), "path:ccag".to_string()),
    };
    ...
    // We pass explicit tokens and a free port so the UI can connect deterministically.
    {
            ...
            .spawn()
        {
            ...
            return Err(format!("Failed to start ccag orchestrator: {err}"));
        }
        eprintln!(
            "[sandbox-create][at={}][runId={}][stage=spawn] launched ccag sidecar for detached sandbox host",
            now_ms(),
            sandbox_run_id
        );
    }
    ...
    emit_sandbox_progress(
        ...
        &sandbox_run_id,
        "spawned",
        "Sandbox process launched. Waiting for ccag server...",
        json!({
            "ccagUrl": ccag_url,
        }),
    );
    ...
    while start.elapsed() < Duration::from_millis(health_timeout_ms) {
        ...
        match ureq::get(&format!("{}/health", ccag_url.trim_end_matches('/'))).call() {
            Ok(response) if response.status() >= 200 && response.status() < 300 => {
                emit_sandbox_progress(
                    &app,
                    &sandbox_run_id,
                    "ccag.healthy",
                    "ccag server is ready.",
                    json!({
                        "ccagUrl": ccag_url,
                        "elapsedMs": elapsed_ms,
    ...
    let owner_token = issue_owner_token(&ccag_url, &host_token).ok();
...
pub fn sandbox_stop(container_name: String) -> Result<ExecResult, String> {
    ...
    if !name.starts_with("ccag-orchestrator-") {
        return Err(
            "Refusing to stop container: expected name starting with 'ccag-orchestrator-'"
                .to_string(),
        );
    }
    if !name
...
#[tauri::command]
pub fn sandbox_cleanup_ccag_containers() -> Result<CcagDockerCleanupResult, String> {
    let candidates = list_ccag_managed_containers()?;
    if candidates.is_empty() {
        return Ok(CcagDockerCleanupResult {
            candidates,
            removed: Vec::new(),
            errors: Vec::new(),
        });
...
pub fn sandbox_debug_probe(app: AppHandle) -> SandboxDebugProbeResult {
    ...
    let workspace_dir = env::temp_dir().join(format!("ccag-sandbox-probe-{}", Uuid::new_v4()));
...
mod tests {
    ...
    fn docker_command_falls_back_after_timeout() {
        ...
        let tmp =
            std::env::temp_dir().join(format!("ccag-docker-timeout-test-{}", Uuid::new_v4()));
...
"#,
    ...
    fn local_command_timeout_returns_when_descendant_keeps_pipe_open() {
        let tmp =
            std::env::temp_dir().join(format!("ccag-timeout-pipe-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&tmp).expect("create tmp dir");
    ...
    fn sandbox_start_timeout_error_includes_stage_diagnostics() {
        ...
        assert!(message.contains("stage=ccag.healthcheck"));
    ...
    fn sandbox_doctor_uses_override_docker_bin() {
        ...
        let tmp =
            std::env::temp_dir().join(format!("ccag-docker-doctor-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&tmp).expect("create tmp dir");
