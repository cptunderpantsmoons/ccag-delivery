...
pub struct MigrateToElectronRequest {
    ...
    /// Optional override for where the new ccag bundle should land on
...
fn current_app_bundle_path() -> Result<PathBuf, String> {
    // Tauri's running .app is <something>/Contents/MacOS/ccag. Walk up
...
fn write_macos_migration_script(
    ...
    let script_path = cache.join("ccag-migrate.sh");
...
TARGET="{target}"
URL="{url}"
WORK=$(mktemp -d /tmp/ccag-migrate-XXXXXX)
ZIP="$WORK/ccag-electron.zip"
...
NEW_APP=$(find "$WORK" -maxdepth 2 -name 'ccag.app' -type d | head -n 1)
if [ -z "$NEW_APP" ]; then
  echo "[migration] no ccag.app in zip" >&2
  exit 1
fi
