...
use crate::ccag_server::start_ccag_server;
use crate::types::{CcagServerInfo, WorkspaceType};
use crate::workspace::state::load_workspace_state;

#[tauri::command]
pub fn ccag_server_info(manager: State<CcagServerManager>) -> CcagServerInfo {
    let mut state = manager
        .inner
        .lock()
        .expect("ccag server mutex poisoned");
    CcagServerManager::snapshot_locked(&mut state)
}

#[tauri::command]
pub fn ccag_server_restart(
    app: AppHandle,
    manager: State<CcagServerManager>,
