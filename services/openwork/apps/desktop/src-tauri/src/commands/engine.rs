...
use crate::ccag_server::{manager::CcagServerManager, start_ccag_server};
...
pub fn engine_info(
    ...
    if state.runtime == EngineRuntime::Orchestrator {
        ...
        // auth snapshot next to ccag-orchestrator-state.json so the UI can reconnect.
...
pub fn engine_restart(
    ...
    ccag_remote_access: Option<bool>,
...
pub fn engine_install() -> Result<ExecResult, String> {
    #[cfg(windows)]
    {
      ...
      status: -1,
      stdout: String::new(),
      stderr: "Guided install is not supported on Windows yet. Install the ccag-pinned OpenCode version manually, then restart ccag.".to_string(),
    });
    }
...
pub fn engine_start(
    ...
    let dev_mode = ccag_dev_mode_enabled();
    let ccag_remote_access_enabled = ccag_remote_access.unwrap_or(false);
    ...
    if runtime == EngineRuntime::Orchestrator {
        ...
        // ccag-orchestrator doesn't start its daemon HTTP server until it has ensured that
        ...
        if let Err(error) = start_ccag_server(
            &app,
            &ccag_manager,
            &workspace_paths,
            ...
            ccag_remote_access_enabled,
        ) {
            if let Ok(mut state) = manager.inner.lock() {
                state.last_stderr =
                    Some(truncate_output(&format!("ccag server: {error}"), 8000));
            }
        }
    ...
    if let Err(error) = start_ccag_server(
        ...
        state.last_stderr = Some(truncate_output(&format!("ccag server: {error}"), 8000));
    }
