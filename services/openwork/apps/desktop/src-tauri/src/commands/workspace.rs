...
use crate::types::{
    ExecResult, RemoteType, WorkspaceInfo, WorkspaceList, WorkspaceCcagConfig, WorkspaceType,
};
...
pub fn workspace_create(
    ...
    state.workspaces.push(WorkspaceInfo {
        ...
        ccag_host_url: None,
        ccag_token: None,
        ccag_client_token: None,
        ccag_host_token: None,
        ccag_workspace_id: None,
        ccag_workspace_name: None,
...
pub fn workspace_create_remote(
    ...
    ccag_host_url: Option<String>,
    ccag_token: Option<String>,
    ccag_client_token: Option<String>,
    ccag_host_token: Option<String>,
    ccag_workspace_id: Option<String>,
    ccag_workspace_name: Option<String>,
    ...
    let ccag_host_url = ccag_host_url
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    let ccag_token = ccag_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    let ccag_client_token = ccag_client_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    let ccag_host_token = ccag_host_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    if remote_type == RemoteType::ccag {
        let host_url = ccag_host_url.clone().unwrap_or_default();
        if host_url.is_empty() {
            return Err("ccagHostUrl is required for ccag remote".to_string());
        }
        if !host_url.starts_with("http://") && !host_url.starts_with("https://") {
            return Err("ccagHostUrl must start with http:// or https://".to_string());
        }
    }

    let id = if remote_type == RemoteType::ccag {
        stable_workspace_id_for_openwork(
            ccag_host_url.as_deref().unwrap_or(""),
            ccag_workspace_id.as_deref(),
        )
    } else {
        stable_workspace_id_for_remote(&base_url, directory.as_deref())
    };
    let name = ccag_workspace_name
        .clone()
...
pub fn workspace_update_remote(
    ...
    if let Some(next_host_url) = ccag_host_url
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
    {
        ...
        entry.ccag_host_url = Some(next_host_url);
    }

    if ccag_token.is_some() {
        entry.ccag_token = ccag_token
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if ccag_client_token.is_some() {
        entry.ccag_client_token = ccag_client_token
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if ccag_host_token.is_some() {
        entry.ccag_host_token = ccag_host_token
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if ccag_workspace_id.is_some() {
        entry.ccag_workspace_id = ccag_workspace_id
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if let Some(next_name) = ccag_workspace_name
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
    {
        entry.ccag_workspace_name = Some(next_name.clone());
...
pub fn workspace_add_authorized_root(
    ...
    let ccag_path = PathBuf::from(&workspace_path)
        .join(".opencode")
        .join("ccag.json");

    if let Some(parent) = ccag_path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
    }

    let mut config: WorkspaceCcagConfig = if ccag_path.exists() {
        let raw = fs::read_to_string(&ccag_path)
            .map_err(|e| format!("Failed to read {}: {e}", ccag_path.display()))?;
        serde_json::from_str(&raw).unwrap_or_default()
    } else {
        let mut cfg = WorkspaceCcagConfig::default();
    ...
    .map_err(|e| format!("Failed to write {}: {e}", ccag_path.display()))?;
...
#[tauri::command]
pub fn workspace_ccag_read(
    _app: tauri::AppHandle,
    workspace_path: String,
) -> Result<WorkspaceCcagConfig, String> {
    let workspace_path = workspace_path.trim().to_string();
    if workspace_path.is_empty() {
        return Err("workspacePath is required".to_string());
    }
    ...
    serde_json::from_str::<WorkspaceCcagConfig>(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", ccag_path.display()))
}

#[tauri::command]
pub fn workspace_ccag_write(
    _app: tauri::AppHandle,
    workspace_path: String,
    config: WorkspaceCcagConfig,
) -> Result<ExecResult, String> {
    let workspace_path = workspace_path.trim().to_string();
    if workspace_path.is_empty() {
        return Err("workspacePath is required".to_string());
    ...
    Ok(ExecResult {
        ok: true,
        status: 0,
        stdout: format!("Wrote {}", ccag_path.display()),
        stderr: String::new(),
    })
}
...
pub fn workspace_import_config(
    ...
    let ccag_path = target_path.join(".opencode").join("ccag.json");
    ...
    if ccag_path.exists() {
        let raw = fs::read_to_string(&ccag_path)
            .map_err(|e| format!("Failed to read {}: {e}", ccag_path.display()))?;
        if let Ok(mut config) = serde_json::from_str::<WorkspaceCcagConfig>(&raw) {
        ...
        let config = WorkspaceCcagConfig::new(&target_dir, &preset, now_ms());
        if let Some(parent) = ccag_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
        }
