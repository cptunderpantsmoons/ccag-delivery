...
use crate::ccag_server::manager::CcagServerManager;
...
use crate::types::{ExecResult, WorkspaceCcagConfig};
...
fn current_ccag_state_paths(app: &AppHandle) -> Result<Vec<PathBuf>, String> {
    let mut paths = vec![
        app.path()
            .app_cache_dir()
...
fn stop_host_services(
    ...
    ccag_manager: &State<CcagServerManager>,
    ...
    if let Ok(mut ccag_state) = ccag_manager.inner.lock() {
        CcagServerManager::stop_locked(&mut ccag_state);
    }
...
fn read_workspace_ccag_config(
    workspace_path: &Path,
) -> Result<WorkspaceCcagConfig, String> {
    let ccag_path = workspace_path.join(".opencode").join("ccag.json");
    if !ccag_path.exists() {
...
fn load_authorized_roots(app: &AppHandle) -> Result<Vec<PathBuf>, String> {
    ...
    for workspace in state.workspaces {
        ...
        let mut config = read_workspace_ccag_config(&workspace_path)?;
...
pub fn app_build_info(app: AppHandle) -> AppBuildInfo {
    ...
    AppBuildInfo {
        ...
        git_sha,
        build_epoch,
        ccag_dev_mode: env_truthy("OPENWORK_DEV_MODE"),
    }
}

#[tauri::command]
pub fn nuke_ccag_and_opencode_config_and_exit(
    app: AppHandle,
    engine_manager: State<EngineManager>,
    orchestrator_manager: State<OrchestratorManager>,
    ccag_manager: State<CcagServerManager>,
    ...
    let mut paths = current_ccag_state_paths(&app)?;
    if dev_mode {
