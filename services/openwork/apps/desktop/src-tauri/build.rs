...
fn ensure_orchestrator_sidecar() {
    ...
    let canonical_name = if target.contains("windows") {
        "ccag-orchestrator.exe"
    } else {
        "ccag-orchestrator"
    };
    let mut target_name = format!("ccag-orchestrator-{target}");
    if target.contains("windows") {
        target_name.push_str(".exe");
    }
    ...
    let Some(source_path) = source_path else {
        println!(
            "cargo:warning=orchestrator sidecar missing at {} (set OPENWORK_ORCHESTRATOR_BIN_PATH or install ccag-orchestrator)",
            dest_path.display()
        );
        create_debug_stub(&dest_path, &sidecar_dir, &profile, &target);
        create_debug_stub(&target_dest_path, &sidecar_dir, &profile, &target);
...
fn ensure_ccag_server_sidecar() {
    ...
    let mut target_name = format!("ccag-server-{target}");
    if target.contains("windows") {
        target_name.push_str(".exe");
    }
    ...
    let Some(source_path) = source_path else {
        println!(
      "cargo:warning=ccag server sidecar missing at {} (set OPENWORK_SERVER_BIN_PATH or install ccag-server)",
      dest_path.display()
    );
    ...
    if copied {
        ...
        println!(
            "cargo:warning=Failed to copy ccag server sidecar from {} to {}",
            source_path.display(),
            dest_path.display()
        );
        create_debug_stub(&dest_path, &sidecar_dir, &profile, &target);
