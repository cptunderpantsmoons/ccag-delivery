...
test("shows a read-only shared skill page with CCAG import actions", async ({ page }) => {
  ...
  const openInAppHref = await page.getByRole("link", { name: /^open in ccag$/i }).getAttribute("href");
  expect(openInAppHref).toBeTruthy();
  expect(openInAppHref ?? "").toContain("ccag://import-bundle?");

  const openInAppLink = page.getByRole("link", { name: /^open in ccag$/i });
  ...
  const deepLinkQuery = new URL((openInAppHref ?? "").replace("ccag://import-bundle?", "https://example.test/?"));
