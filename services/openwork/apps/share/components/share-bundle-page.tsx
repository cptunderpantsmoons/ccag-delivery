...
function humanizeLabel(value: string | null | undefined): string {
  ...
  if (!normalized) return "CCAG Bundle";
...
function formatSurfaceEyebrow(selection: BundleSelection | undefined, parsedName: string, fallbackTitle: string | undefined): string {
  ...
  if (/^ccag\.json$/i.test(filename)) return "CCAG Config";
...
export default function ShareBundlePage(props: BundlePageProps & { stars?: string }) {
  ...
  const openInCCAG = (event: MouseEvent<HTMLAnchorElement>) => {
  ...
  const prepareOpenInCCAG = (event: PointerEvent<HTMLAnchorElement> | MouseEvent<HTMLAnchorElement>) => {
    refreshOpenInAppHref(event.currentTarget);
  };

  const prepareOpenInCCAGFromKeyboard = (event: KeyboardEvent<HTMLAnchorElement>) => {
  ...
  const actionButtons = (
    <>
      ...
      <a
        ...
        href={openInAppUrl}
        onPointerDown={prepareOpenInCCAG}
        onMouseDown={prepareOpenInCCAG}
        onKeyDown={prepareOpenInCCAGFromKeyboard}
        onClick={openInCCAG}
      >
        Open in CCAG
      </a>
    ...
    <main className="shell">
      ...
      {props.missing ? (
        <section className="status-card">
          <span className="eyebrow">CCAG Share</span>
