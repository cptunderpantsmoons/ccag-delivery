...
export function renderBundlePage({ id, rawJson, req }: { id: string; rawJson: string; req: RequestLike }): string {
  ...
  const title = bundle.name || `CCAG ${typeLabel}`;
  const description = bundle.description || buildBundleNarrative(bundle);
  const items = collectBundleItems(bundle, 8);
  const compactItem = bundle.type === "skill" ? "skill.md" : items[0]?.name || "CCAG bundle";
...
<head>
  ...
  <title>${escapeHtml(title)} - CCAG Share</title>
  <meta name="description" content="${escapeHtml(description)}" />
  <meta name="ccag:bundle-id" content="${escapeHtml(id)}" />
  <meta name="ccag:bundle-type" content="${escapeHtml(bundle.type || "unknown")}" />
  <meta name="ccag:schema-version" content="${escapeHtml(schemaVersion)}" />
  <meta name="ccag:open-in-app-url" content="${escapeHtml(openInAppDeepLink)}" />
...
<body
  data-ccag-share="true"
  data-ccag-bundle-id="${escapeHtml(id)}"
  data-ccag-bundle-type="${escapeHtml(bundle.type || "unknown")}"
  data-ccag-schema-version="${escapeHtml(schemaVersion)}"
>
  <main class="shell">
    <nav class="nav">
      <a class="brand" href="/" aria-label="CCAG Share home">
        <span class="brand-mark" aria-hidden="true"></span>
        <span>ccag</span>
      </a>
      <div class="nav-links">
        <a href="https://openworklabs.com/docs" target="_blank" rel="noreferrer">Docs</a>
        <a href="${escapeHtml(CCAG_DOWNLOAD_URL)}" target="_blank" rel="noreferrer">Download</a>
        <a href="https://openworklabs.com/enterprise" target="_blank" rel="noreferrer">Enterprise</a>
      </div>
      <div class="nav-actions">
        <a class="button-secondary" href="https://github.com/different-ai/ccag" target="_blank" rel="noreferrer">
    ...
    <section class="hero-layout">
      <div class="hero-copy">
        ...
        <div class="hero-actions">
          <a class="button-primary" href="${escapeHtml(openInAppDeepLink)}">Open in CCAG app</a>
          <a class="button-secondary" href="https://openworklabs.com/den" target="_blank" rel="noreferrer">Open in an CCAG den</a>
        </div>
      </div>
      ...
      <div class="hero-artifact">
        <div class="app-window">
          <div class="app-window-header">
            ...
            <div class="app-window-title">CCAG</div>
          </div>
    ...
    <section class="results-grid">
      <div class="result-card">
        <div class="step-list">
          <div class="step-row"><span class="step-bullet">01</span><span>Open the bundle in CCAG</span></div>
          <div class="step-row"><span class="step-bullet">02</span><span>Choose the destination worker</span></div>
          <div class="step-row"><span class="step-bullet">03</span><span>Happy CCAGing!</span></div>
        </div>
      </div>
    </section>
  ...
  <script id="ccag-bundle-json" type="application/json">${escapeJsonForScript(rawJson)}</script>
  <script>
  </script>
</body>
</html>`;
