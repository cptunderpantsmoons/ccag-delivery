...
test("renderBundlePage includes machine-readable metadata and escaped json script", () => {
  ...
  assert.match(html, /data-ccag-share="true"/);
  assert.match(html, /data-ccag-bundle-type="skill"/);
  assert.match(html, /meta name="ccag:bundle-id" content="01TEST"/);
  assert.match(html, /\/b\/01TEST\/data/);
  assert.match(html, /ccag:\/\/import-bundle\?/);
  ...
  assert.match(html, /id="ccag-bundle-json" type="application\/json"/);
  ...
  assert.match(html, /Open in CCAG app/);
  assert.match(html, /Open in an CCAG den/);
...
test("renderBundlePage shows workspace profile metadata", () => {
  ...
  assert.match(html, /Open the bundle in CCAG/);
  assert.match(html, /Choose the destination worker/);
  assert.match(html, /Happy CCAGing!/);
