...
function isCcagConfigFile(file: NormalizedFile): boolean {
  return /(^|\/)(?:\.opencode\/)?ccag\.jsonc?$/i.test(file.path);
}
...
function looksLikeCcagShape(parsed: unknown): boolean {
  if (!isRecord(parsed)) return false;

  const schema = maybeString(parsed.$schema).toLowerCase();
  return schema.includes("ccag") && schema.includes("config");
}
...
interface ConfigSection {
  ...
  ccag: Record<string, unknown> | null;
  config: Record<string, unknown>;
  preview: PreviewItem[];
  configCount: number;
}
...
function readConfigSection(file: NormalizedFile, warnings: string[]): ConfigSection | null {
  ...
  let ccag: Record<string, unknown> | null = null;
  const config: Record<string, unknown> = {};
  const preview: PreviewItem[] = [];
  let configCount = 0;

  if (isCcagConfigFile(file)) {
    ccag = cloneRecord(parsed);
    preview.push(buildConfigPreview(file, "CCAG config"));
    configCount += 1;
  }
  ...
  if (!opencode && !ccag) {
    if (looksLikeNamedMcpFile(file)) {
      ...
      preview.push(...collectOpencodePreviewItems(parsed));
      configCount += 1;
    } else if (looksLikeCcagShape(parsed)) {
      ccag = cloneRecord(parsed);
      preview.push(buildConfigPreview(file, "CCAG config"));
      configCount += 1;
  ...
  if (!opencode && !ccag && !Object.keys(config).length) {
    warnings.push(`Ignored unsupported config file: ${file.path}`);
    return null;
  }

  return { opencode, ccag, config, preview, configCount };
}
...
export function packageCcagFiles(input: PackageInput): PackageResult {
  ...
  if (!summary.skills && !summary.agents && !summary.mcpServers && !summary.commands && !summary.configs && !ccag) {
