...
function buildRootOgInput(): OgImageModel {
  return {
    title: "Share CCAG skills beautifully",
    fileName: "agent-creator.md",
    fileType: "SKILL.md",
    description: "Clean metadata-first social cards for shared CCAG skills and bundles.",
    category: "share",
    tag: "ccag preview",
...
function buildBundleOgInput({ rawJson }: { id: string; rawJson: string }): OgImageModel {
  ...
  const bundleName = maybeString(data.name).trim() || bundle.name || titleFromFileName(preview.filename) || "CCAG bundle";
  ...
  const title =
    bundle.type === "workspace-profile"
      ...
      : bundle.type === "skills-set" && bundle.skills.length > 1
        ...
        : humanizeTitle(bundleName) || "CCAG bundle";
...
export function computeOgImageLayout(model: OgImageModel): OgImageLayout {
  const displayTitle = truncateAtWordBoundary(humanizeTitle(model.title) || "CCAG bundle", MAX_DISPLAY_CHARS);
...
function renderCCAGLogo({ x, y, width, height }: { x: number; y: number; width: number; height: number }): string {
  return `
...
<svg width="${variantConfig.width}" height="${variantConfig.height}" viewBox="0 0 ${BASE_OG_IMAGE_WIDTH} ${BASE_OG_IMAGE_HEIGHT}" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  ${renderCCAGLogo({ x: cardX + 66, y: cardY + 34, width: 40, height: 34 })}
