...
import { packageCcagFiles } from "./package-ccag-files.ts";

test("packageCcagFiles creates a single skill bundle from skill markdown", () => {
  const result = packageCcagFiles({
    files: [
      {
...
test("packageCcagFiles accepts any single text file when frontmatter has name and description", () => {
  const result = packageCcagFiles({
    files: [
      {
        path: "AGENTS.md",
...
test("packageCcagFiles rejects content without name and description frontmatter", () => {
  assert.throws(
    () =>
      packageCcagFiles({
        files: [
...
test("packageCcagFiles rejects multiple uploaded files", () => {
  assert.throws(
    () =>
      packageCcagFiles({
        files: [
...
test("packageCcagFiles rejects config json uploads", () => {
  assert.throws(
    () =>
      packageCcagFiles({
        files: [
...
test("packageCcagFiles rejects agent and config combinations", () => {
  assert.throws(
    () =>
      packageCcagFiles({
        files: [
