...
export const CCAG_SITE_URL = "https://openworklabs.com";
export const CCAG_DOWNLOAD_URL = "https://openworklabs.com/download";
...
export function setCors(
  ...
  res.setHeader(
    ...
    options.headers ??
      "Content-Type,Accept,X-CCAG-Bundle-Type,X-CCAG-Schema-Version,X-CCAG-Name",
  );
}
...
export function buildOpenInAppUrls(shareUrl: string, options: { label?: string } = {}): OpenInAppUrls {
  ...
  const appScheme = (getEnv("PUBLIC_CCAG_APP_SCHEME", "ccag") || "ccag").trim().toLowerCase();
  return {
...
export function getBundleCounts(bundle: NormalizedBundle): BundleCounts {
  ...
  const ccag = maybeObject(bundle.workspace?.ccag);
  ...
  return {
    ...
    configCount: (ccag ? 1 : 0) + (opencodeConfigKeys.length ? 1 : 0) + Object.keys(genericConfig ?? {}).length,
    fileCount: files.length,
    hasConfig: Boolean(ccag || opencodeConfigKeys.length || genericConfig),
  };
}
...
export function collectBundleItems(bundle: NormalizedBundle, limit = 8): PreviewItem[] {
  ...
  if (bundle.type === "workspace-profile") {
    ...
    if (maybeObject(bundle.workspace?.ccag)) {
      items.push({
        name: "ccag.json",
        kind: "Config",
        meta: "CCAG config",
...
export function buildBundlePreview(bundle: NormalizedBundle): {
  ...
  if (bundle.type === "skill") {
    return buildBundlePreviewSelection({
      ...
      text: buildTextPreview(bundle.content, `# ${bundle.name || "CCAG skill"}`),
      tone: "skill",
  ...
  if (commands.length) {
    ...
    return buildBundlePreviewSelection({
      ...
      text: buildTextPreview(firstCommand.template || firstCommand.content, `# ${firstCommand.name || "CCAG command"}`),
      tone: "command",
  ...
  if (maybeObject(bundle.workspace?.ccag)) {
    return buildBundlePreviewSelection({
      filename: "ccag.json",
      text: buildJsonPreview(bundle.workspace?.ccag, '{\n  "ccag": {}\n}'),
      tone: "config",
      label: "CCAG config",
    });
  }
  ...
  if (files.length) {
    ...
    return buildBundlePreviewSelection({
      ...
      text: buildTextPreview(firstFile.content, `# ${basename(firstFile.path) || "CCAG portable file"}`),
      tone: preview.tone,
...
export function buildBundleNarrative(bundle: NormalizedBundle): string {
  ...
  return parts.length
    ? `${parts.join(", ")} bundled into a worker package that imports through CCAG with one step.`
    : "Worker configuration bundle prepared for CCAG import.";
}
