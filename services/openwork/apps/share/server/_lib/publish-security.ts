...
const store = globalThis as typeof globalThis & {
  __ccagShareRateLimitStore?: Map<string, FixedWindowEntry>;
};

const rateLimitStore = store.__ccagShareRateLimitStore ?? new Map<string, FixedWindowEntry>();
store.__ccagShareRateLimitStore = rateLimitStore;
...
export function getAllowedOrigins(request: Request) {
  const configured = String(process.env.CCAG_PUBLISHER_ALLOWED_ORIGINS ?? "")
    .split(",")
...
export function buildCorsHeaders(request: Request) {
  ...
  const headers: Record<string, string> = {
    ...
    "Access-Control-Allow-Headers": "Content-Type,Accept,X-CCAG-Bundle-Type,X-CCAG-Schema-Version,X-CCAG-Name",
  };
