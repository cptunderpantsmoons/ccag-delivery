# CCAG Share Service (Publisher)

This is a Next.js publisher app for CCAG "share link" bundles.
...
- `GET /`
  - Human-friendly packaging page for CCAG worker files.
  - Supports drag/drop of skills, agents, commands, `opencode.json[c]`, and `ccag.json`.
...
- `POST /v1/package`
  ...
  - Parses supported CCAG files into the smallest useful bundle shape.
...
- `GET /b/:id`
  ...
  - Includes an **Open in app** action that opens `ccag://import-bundle` with:
    - `ow_bundle=<share-url>`
    - `ow_intent=new_worker` (desktop CCAG converts single-skill bundles into a destination picker before import)
...
- `CCAG_PUBLISHER_ALLOWED_ORIGINS`
  - Optional comma-separated browser origins allowed to publish bundles.
  - Defaults include the share origin, the hosted CCAG app origin, and common local dev origins.
...
# Machine-readable payload (CCAG parser path)
