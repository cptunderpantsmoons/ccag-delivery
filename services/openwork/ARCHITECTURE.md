...
CCAG optimizes for **predictability** over "clever" auto-detection. Users should be able to form a correct mental model of what will happen.
...
- `CCAG_DOCKER_BIN` (absolute path to `docker`)
...
1. Honor `CCAG_DOCKER_BIN` if set.
...
We move most of the functionality to the ccag server which interfaces mostly with FS and proxies to opencode.



## Filesystem mutation policy

CCAG should route filesystem mutations through the CCAG server whenever possible.
...
- Any UI feature that changes workspace files or config should call an CCAG server endpoint first.
- Local Tauri filesystem commands are a host-mode fallback, not the primary product surface.
- If a feature cannot yet write through the CCAG server, treat that as an architecture gap and close it before depending on direct local writes.
- Reads can fall back locally when necessary, but writes should be designed around the CCAG server path.

## Agent authority map

When CCAG is edited from `ccag-enterprise`, architecture and runtime behavior should be sourced from this document.

| Entry point | Role | Architecture authority |
| --- | --- | --- |
| `ccag-enterprise/AGENTS.md` | CCAG Factory multi-repo orchestration | Defers CCAG runtime flow, server-vs-shell ownership, and filesystem mutation behavior to `_repos/ccag/ARCHITECTURE.md`. |
| `ccag-enterprise/.opencode/agents/ccag-surgeon.md` | Surgical fix agent for `_repos/ccag` | Uses `_repos/ccag/ARCHITECTURE.md` as the runtime and architecture source of truth before changing product behavior. |
| `_repos/ccag/AGENTS.md` | Product vocabulary, audience, and repo-local development guidance | Refers to `ARCHITECTURE.md` for runtime flow, server ownership, and architectural boundaries. |
| Skills / commands / agents that mutate workspace state | Capability layer on top of the product runtime | Should assume the CCAG server path is canonical for workspace creation, config writes, `.opencode/` mutation, and reload signaling. |

### Agent access to server-owned behavior

Agents, skills, and commands should model the following as CCAG server behavior first:

- workspace creation and initialization
- writes to `.opencode/`, `opencode.json`, and `opencode.jsonc`
- CCAG workspace config writes (`.opencode/ccag.json`)
...
- share-bundle publish/fetch flows used by CCAG template links
...
CCAG desktop ships through two release channels:

- **Stable** (default, all platforms): versioned builds produced by the `Release App` workflow. Each tag `vX.Y.Z` publishes signed, notarized bundles plus a `latest.json` updater manifest at `https://github.com/different-ai/ccag/releases/latest/download/latest.json`.
- **Alpha** (macOS arm64 only, rolling): every merge to `dev` publishes a signed, notarized build to the rolling GitHub release tagged `alpha-macos-latest`. The alpha updater manifest lives at a stable URL: `https://github.com/different-ai/ccag/releases/download/alpha-macos-latest/latest.json`.
...
CCAG uses a single reload-required flow for changes that only take effect when OpenCode restarts.
...
- adds a new abstraction "workspace" is a project folder and a simple .json file that includes a list of opencode primitives that map perfectly to an opencode workdir (not fully implemented)
  - ccag can open a workpace.json and decide where to populate a folder with thse settings (not implemented today

## Repository/component map

- `/apps/app/`: CCAG app UI (desktop/mobile/web client experience layer).
- `/apps/desktop/`: Tauri desktop shell that hosts the app UI and manages native process lifecycles.
- `/apps/server/`: CCAG server (API/control layer consumed by the app).
- `/apps/orchestrator/`: CCAG orchestrator CLI/daemon. In `start`/`serve` host mode it manages CCAG server + OpenCode + `opencode-router`; in daemon mode it manages workspace activation and OpenCode lifecycle for desktop runtime.
- `/apps/opencode-router/`: first-party messaging bridge (Slack/Telegram) and directory router.
- `/apps/share/`: share-link publisher service for CCAG bundle imports.
- `/ee/apps/landing/`: CCAG landing page surfaces.
...
CCAG is a client experience that consumes CCAG server surfaces.

CCAG supports two product runtime modes for users:

- desktop
- web/cloud (also usable from mobile clients)

CCAG therefore has two runtime connection modes:

### Mode A - Desktop

- CCAG runs on a desktop/laptop and can host CCAG server surfaces locally.
- The OpenCode server runs on loopback (default `127.0.0.1:4096`).
- The CCAG server also defaults to loopback-only access. Remote sharing is an explicit opt-in that rebinds the CCAG server to `0.0.0.0` while keeping OpenCode on loopback.
- CCAG UI connects via the official SDK and listens to events.
- `ccag-orchestrator` is the CLI host path for this mode.

### Mode B - Web/Cloud (can be mobile)

- User signs in to hosted CCAG web/app surfaces (including mobile browser/client access).
- User launches a cloud worker from hosted control plane.
- CCAG returns remote connect credentials (`/w/ws_*` URL + access token).
- User connects from CCAG app using `Add a worker` -> `Connect remote`.
...
- `/apps/desktop/` exposes native commands (`engine_*`, `orchestrator_*`, `ccag_server_*`, `opencodeRouter_*`) to start/stop local services and report status to the UI.
- `/apps/desktop/` is also the source of truth for desktop bootstrap config that must survive updates, including Den server targeting and forced-sign-in startup behavior. The shell reads a predictable external `desktop-bootstrap.json` from the host config directory (or `CCAG_DESKTOP_BOOTSTRAP_PATH` when explicitly overridden). Default builds consume that file when present; custom builds seed or overwrite it when their bundled bootstrap differs from the standard default.
- Runtime selection in desktop:
  - `ccag-orchestrator` (default): Tauri launches `ccag daemon run` and uses it for workspace activation plus OpenCode lifecycle.
  - `direct`: Tauri starts OpenCode directly.
- In both desktop runtimes, CCAG server (`/apps/server/`) is the API surface consumed by the UI; it is started with the resolved OpenCode base URL and proxies OpenCode and `opencode-router` routes.
- Desktop-launched OpenCode credentials are always random, per-launch values generated by CCAG. OpenCode stays on loopback and is intended to be reached through CCAG server rather than exposed directly.
...
/apps/desktop (Tauri shell)
    ...
    |        OpenCode
    |
    +--> /apps/server (CCAG API + proxy surface)
    |          |
    |          +--> OpenCode
...
- `/ee/apps/den-worker-runtime/` defines the runtime packaging and boot path used inside cloud workers (including Docker/snapshot artifacts and `ccag serve` startup assumptions).
...
- The CCAG app (desktop or mobile client) connects to worker CCAG server surfaces via URL + token (`/w/ws_*` when available).
...
/ee/apps/den-api (formerly /ee/apps/den-controller)
    ...
    |        |
    |        v
    |      /ee/apps/den-worker-runtime -> ccag serve + OpenCode
    |
    +--> /ee/apps/den-worker-proxy (signed preview + proxy)

CCAG app/mobile client
    -> Connect remote (URL + token)
    -> worker CCAG server surface
```

## OpenCode Router (Messaging Bridge)

CCAG can expose a workspace through Slack and Telegram via `opencode-router`.
...
CCAG optimizes for a predictable routing boundary.
...
CCAG server proxies `/opencode-router/*` and `/w/:id/opencode-router/*` to that local router API.

### Workspace-scoped behavior in CCAG

CCAG server treats messaging identities as workspace-scoped.
...
workspace A ----\
workspace B -----+--> CCAG server knows all workspaces
workspace C ----/
...
- CCAG server is already multi-workspace aware.
...
Desktop local CCAG server ports:

- Desktop-hosted local CCAG server instances do not assume a fixed `8787` port.
...
1. Authenticate in CCAG Cloud control surface.
...
5. Connect in CCAG app via deep link or manual URL + token.
...
In CCAG, the long-term direction is:

- Use the CCAG server (`/apps/server/`) as the single API surface for filesystem-backed operations.
...
CCAG uses the official JavaScript/TypeScript SDK:
...
CCAG must be real-time. It subscribes to SSE events:
...
CCAG maps a "Task Run" to an OpenCode **Session**.
...
CCAG's file browser and "what changed" UI are powered by:
...
CCAG must surface permission requests clearly and respond explicitly.
...
CCAG exposes two extension surfaces:
...
2. **Plugins (OpenCode)**
   ...
   - CCAG should show plugin status and instructions; a native plugin manager is planned.

### Engine reload (config refresh)

- CCAG server exposes `POST /workspace/:id/engine/reload`.
...
- Reload requests follow CCAG server approval rules.

### Skill Registry (Current + Future)

- Today, CCAG only supports **curated lists + manual sources**.
- Future goals:
...
CCAG conceptually treats "workspace" as the current project/path.

## Optional TUI Control (Advanced)

The SDK exposes `client.tui.*` methods. CCAG can optionally provide a "Developer Mode" screen to:
...
CCAG enforces folder access through **two layers**:

1. **CCAG UI authorization**
   - user explicitly selects allowed folders via native picker
   - CCAG remembers allowed roots per profile

2. **OpenCode server permissions**
   - OpenCode requests permissions as needed
   - CCAG intercepts requests via events and displays them
...
- Scheduling API surface (native in OpenCode server vs CCAG-managed scheduler).
