...
Use this directory to build and install the AMD64 CCAG package locally on Arch Linux without Docker.

## What this packaging does

- Targets `x86_64` Arch Linux.
- Downloads the published GitHub release asset `ccag-desktop-linux-amd64.deb`.
...
2. build an Arch package such as `ccag-<version>-1-x86_64.pkg.tar.zst`
3. install it locally with `pacman`

After install, `ccag` is available as the desktop launcher. If you also want the standalone OpenCode CLI, install `opencode-bin` separately.
...
That keeps CCAG compatible with systems that already install the OpenCode CLI from `opencode-bin`, which should remain the owner of the global `opencode` command.
...
```bash
sudo pacman -U ccag-<version>-1-x86_64.pkg.tar.zst
```
