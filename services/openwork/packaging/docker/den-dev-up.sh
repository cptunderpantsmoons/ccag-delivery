...
DEN_MYSQL_URL=mysql://root:password@127.0.0.1:$DEN_MYSQL_PORT/ccag_den
...
echo "" >&2
echo "CCAG Cloud web UI:  http://localhost:$DEN_WEB_PORT" >&2
echo "CCAG Cloud web UI (LAN/public): http://$PUBLIC_HOST:$DEN_WEB_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "CCAG Cloud web UI (LAN IP):     http://$LAN_IPV4:$DEN_WEB_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "CCAG Cloud web UI (Tailscale):  http://$TAILSCALE_DNS_NAME:$DEN_WEB_PORT" >&2
fi
...
echo "MySQL:                 mysql://root:password@127.0.0.1:$DEN_MYSQL_PORT/ccag_den" >&2
