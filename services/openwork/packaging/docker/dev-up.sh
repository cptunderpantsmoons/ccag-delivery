...
#
# Usage (from _repos/ccag repo root):
#   packaging/docker/dev-up.sh
#
# Defaults to isolated OpenCode dev state inside the container.
# Escape hatch: set CCAG_DOCKER_DEV_MOUNT_HOST_OPENCODE=1 to import host
...
MOUNT_HOST_OPENCODE="${CCAG_DOCKER_DEV_MOUNT_HOST_OPENCODE:-0}"

resolve_opencode_config_dir() {
  local override="${CCAG_OPENCODE_CONFIG_DIR:-}"
  if [ -n "$override" ]; then
    if [ -d "$override" ]; then
      printf '%s\n' "$override"
      return 0
    fi
    echo "warning: CCAG_OPENCODE_CONFIG_DIR is not a directory: $override" >&2
  fi
...
resolve_opencode_data_dir() {
  local override="${CCAG_OPENCODE_DATA_DIR:-}"
  if [ -n "$override" ]; then
    if [ -d "$override" ]; then
      printf '%s\n' "$override"
      return 0
    fi
    echo "warning: CCAG_OPENCODE_DATA_DIR is not a directory: $override" >&2
  fi
...
CCAG_PORT="$(pick_port)"
WEB_PORT="$(pick_port)"
if [ "$WEB_PORT" = "$CCAG_PORT" ]; then
  WEB_PORT="$(pick_port)"
fi
SHARE_PORT="${SHARE_PORT:-$(pick_port)}"
if [ "$SHARE_PORT" = "$CCAG_PORT" ] || [ "$SHARE_PORT" = "$WEB_PORT" ]; then
...
start_stack() {
  local config_dir="$1"
  local data_dir="$2"
  CCAG_DEV_ID="$DEV_ID" CCAG_PORT="$CCAG_PORT" WEB_PORT="$WEB_PORT" SHARE_PORT="$SHARE_PORT" \
    CCAG_DEV_MODE="1" \
    CCAG_PUBLIC_HOST="$PUBLIC_HOST" \
    CCAG_HOST_OPENCODE_CONFIG_DIR="$config_dir" \
    CCAG_HOST_OPENCODE_DATA_DIR="$data_dir" \
...
echo "- CCAG_HOST_OPENCODE_CONFIG_DIR=$ACTIVE_OPENCODE_CONFIG_DIR" >&2
echo "- CCAG_HOST_OPENCODE_DATA_DIR=$ACTIVE_OPENCODE_DATA_DIR" >&2
...
echo "" >&2
echo "CCAG web UI:     http://localhost:$WEB_PORT" >&2
echo "CCAG web UI (LAN/public): http://$PUBLIC_HOST:$WEB_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "CCAG web UI (LAN IP):     http://$LAN_IPV4:$WEB_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "CCAG web UI (Tailscale):  http://$TAILSCALE_DNS_NAME:$WEB_PORT" >&2
fi
echo "CCAG server:     http://localhost:$CCAG_PORT" >&2
echo "CCAG server (LAN/public): http://$PUBLIC_HOST:$CCAG_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "CCAG server (LAN IP):     http://$LAN_IPV4:$CCAG_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "CCAG server (Tailscale):  http://$TAILSCALE_DNS_NAME:$CCAG_PORT" >&2
fi
