...
- Treat **plugins + skills + commands + mcp** as the primary extensibility system. These are native to OpenCode and CCAG must be a thin layer on top of them. They're mostly fs based.

## Principles

- **Parity**: UI actions map to OpenCode server APIs.
- **Server-consumption first**: CCAG app consumes CCAG server surfaces (desktop-hosted, orchestrator-hosted, or cloud-hosted) instead of inventing parallel behavior.
