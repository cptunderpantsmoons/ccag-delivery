...
## Purpose
Use this skill whenever CCAG behavior is implemented directly on top of OpenCode primitives (skills, plugins, MCP servers, opencode.json config, tools/permissions). It anchors decisions to the official OpenCode documentation and keeps terminology consistent in the UI.
...
## When to Invoke
- Adding or adjusting CCAG flows that reference skills, plugins, MCP servers, or OpenCode config.
...
## Usage
Call `skill({ name: "opencode-primitives" })` before implementing or documenting any CCAG behavior that maps to OpenCode primitives.
