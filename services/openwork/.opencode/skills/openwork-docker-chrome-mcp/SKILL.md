---
name: ccag-docker-chrome-mcp
description: |
  Start the CCAG dev stack via Docker and verify real user flows via Chrome MCP.
...
- CCAG server URL (http://localhost:<CCAG_PORT>)
- Token file path (`tmp/.dev-env-<id>`) containing `CCAG_TOKEN` + `CCAG_HOST_TOKEN`
...
```bash
docker ps --format '{{.Names}}' | rg '^ccag-dev-'
```
