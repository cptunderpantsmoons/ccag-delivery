...
# ── Config ──────────────────────────────────────────────────────────────────
CCAG_ROOT="$(cd "$(dirname "$0")/.." && pwd)"

# Detect whether we're inside a factory layout (../../.. has _repos/)
FACTORY_CANDIDATE="$(cd "$CCAG_ROOT/../../.." 2>/dev/null && pwd)"
...
if [ -n "$FACTORY_ROOT" ]; then
  for d in "$FACTORY_ROOT"/_repos/*/; do
    [ -d "$d" ] || continue
    [ "$(cd "$d" && pwd)" = "$CCAG_ROOT" ] && continue
    SIBLING_REPOS+=("$d")
  done
fi

# ── Internal infra: all build/config/CI files that may reference source ────
# These are files WITHIN ccag that knip can't trace but that use source files
...
collect_infra_files() {
  ...
  for glob_pattern in "${INFRA_GLOBS[@]}"; do
    ...
    local matched
    matched=$(find "$CCAG_ROOT" -path "$CCAG_ROOT/$glob_pattern" 2>/dev/null || true)
    if [ -n "$matched" ]; then
      files="${files}${matched}"$'\n'
    fi
  done
  ...
  local pkg_jsons
  pkg_jsons=$(find "$CCAG_ROOT" -name package.json -not -path '*/node_modules/*' -not -path '*/.git/*')
  files="${files}${pkg_jsons}"$'\n'
  # And all tsconfig*.json files (for path aliases / includes)
  local tsconfigs
  tsconfigs=$(find "$CCAG_ROOT" -name 'tsconfig*.json' -not -path '*/node_modules/*' -not -path '*/.git/*')
...
# Search sibling repo CI/CD and build scripts for an ccag-relative path.
# Only checks infra files (workflows, Dockerfiles, build scripts), NOT source code,
# since no sibling repo has an npm dependency on ccag packages.
search_sibling_ci() {
  local pattern="$1"
  local hits=""
...
for filepath in "${UNUSED_FILES[@]}"; do
  ...
  # Only search by the ccag-relative path (precise) or unique filename.
  # Since no sibling repo has npm deps on ccag packages, we only check
...
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo -e "${DIM}  No imports in ccag source, no references in CI/CD, build${RESET}"
