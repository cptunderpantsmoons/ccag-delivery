...
async function main() {
  ...
  const workDir = join(tmpdir(), `ccag-migrate-validate-${args.tag}`);
  ...
  const appPath = join(unzipDir, "CCAG.app");
  if (!existsSync(appPath)) {
    console.log(`[validate] ✗ CCAG.app not found inside zip (got: ${unzipDir})`);
  ...
  const steps = [
    ...
    "Restart, see the 'CCAG is moving' modal, click 'Install now'.",
