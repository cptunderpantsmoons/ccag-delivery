...
| ---- | ---- | ------------------------ |
| 01   | Ready to ship the migration | Tauri users see "CCAG is moving" prompt on next update |
...
node scripts/migration/01-cut-migration-release.mjs --version 0.12.0 \
  --mac-url 'https://github.com/different-ai/ccag/releases/download/v0.12.0/CCAG-darwin-arm64-0.12.0-mac.zip' \
...
3. Creates a release-config fragment at
   ...
   `VITE_CCAG_MIGRATION_RELEASE=1` and the per-platform download
...
- Users still on Tauri: the migrate prompt is gated on
  `VITE_CCAG_MIGRATION_RELEASE=1` at build time. Re-cut the
...
- Users mid-migration: the Rust `migrate_to_electron` command keeps
  the previous `.app` at `CCAG.app.migrate-bak`. Instruct users
  to `mv CCAG.app.migrate-bak CCAG.app` if the Electron
  launch fails.
