...
function parseArgs(argv) {
  const options = {
    ...
    repo: process.env.GITHUB_REPOSITORY || "different-ai/ccag",
    output: "latest.json",
  };
...
function updaterPlatformKeys(assetName) {
  if (!assetName.startsWith("ccag-desktop-")) return [];

  const stem = assetName.slice("ccag-desktop-".length);
...
function authHeaders() {
  const headers = {
    ...
    "User-Agent": "ccag-release-latest-json",
  };
