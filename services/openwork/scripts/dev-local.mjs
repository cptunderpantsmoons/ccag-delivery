...
const composeProject = "ccag-den-local"
...
const databaseUrl = process.env.DATABASE_URL?.trim() || "mysql://root:password@127.0.0.1:3306/ccag_den"
...
async function main() {
  ...
  await run("bash", ["-lc", "pnpm --filter @ccag-ee/den-db build && pnpm --filter @ccag-ee/den-db exec node --import tsx ./node_modules/drizzle-kit/bin.cjs push --config drizzle.config.ts --force"], {
    env: {
      ...process.env,
  ...
  turboChild = spawn(
    ...
    [
      ...
      "dev:local",
      "--output-logs=full",
        "--filter=@ccag-ee/den-api",
        "--filter=@ccag-ee/den-worker-proxy",
        "--filter=@ccag-ee/den-web",
    ],
    {
      cwd: rootDir,
      stdio: "inherit",
      ...
      env: {
        ...process.env,
        CCAG_DEV_MODE: process.env.CCAG_DEV_MODE?.trim() || "1",
        DATABASE_URL: databaseUrl,
