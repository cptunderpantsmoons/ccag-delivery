#!/usr/bin/env bash
#
# ccag-debug.sh — one-stop observability + lifecycle control for the
# CCAG dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan ccag/opencode processes (ppid == launchd)
#   stop            full, layered teardown of the dev stack (no cache wipe)
#   start           launch pnpm dev in the background with the log sink on
#   wait-healthy    block until ccag-server reports /health = 200
...
#   3. Tauri webview     (target/debug/CCAG-Dev)  <-- never /Applications/
#   4. Vite              (node node_modules/.../vite)
#   5. orchestrator + ccag-server + opencode + opencode-router orphans
#
...
#   - ~/Library/Application Support/com.differentai.ccag.dev/** (tokens,
#     workspaces registry, prefs). Use `reset-webview` for WebKit state.
#   - /Applications/CCAG.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${CCAG_DEV_LOG_FILE:-$HOME/.ccag/debug/ccag-dev.log}"
PNPM_DEV_LOG="${CCAG_PNPM_DEV_LOG:-/tmp/ccag-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${CCAG_PNPM_DEV_PID:-/tmp/ccag-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${CCAG_WAIT_HEALTHY_SECS:-90}"

# ---------------------------------------------------------------------------
# Helpers

log() { printf '[ccag-debug] %s\n' "$*"; }
...
kill_by_pattern() {
  ...
  # webview (matched by its target/debug path, so prod CCAG.app is safe).
  local pattern="$1"
  local pids
...
discover_ccag_server_port() {
  ps -Ao command | grep "target/debug/ccag-server" | grep -v grep \
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/target\/debug\/CCAG-Dev|target\/debug\/ccag-server|target\/debug\/ccag-orchestrator|target\/debug\/opencode( |\/)|target\/debug\/opencode-router|vite|pnpm dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== ccag-server ==="
  local port
  port=$(discover_ccag_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev ccag-server running)"
  else
  ...
  local oc_port
  oc_port=$(ps -Ao command | grep "target/debug/ccag-orchestrator" | grep -v grep | grep -oE '\-\-opencode-port [0-9]+' | head -1 | awk '{print $2}')
  if [[ -z "$oc_port" ]]; then
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /ccag-server|ccag-orchestrator|opencode( |\/)|opencode-router/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with CCAG_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /ccag-server|ccag-orchestrator|opencode( |\/)|opencode-router/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
stop() {
  ...
  kill_by_pattern "target/debug/CCAG-Dev"
  ...
  # 5. ccag-server / orchestrator / opencode / opencode-router for the
  #    current dev build. These are the longest-lived children and the ones
  #    most likely to orphan after an unclean shutdown.
  kill_by_pattern "target/debug/ccag-server"
  kill_by_pattern "target/debug/ccag-orchestrator"
...
start() {
  ...
  cd "$REPO_ROOT"
  env CCAG_DEV_LOG_FILE="$DEV_LOG_FILE" \
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "ccag-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "ccag-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the ccag-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.differentai.ccag.dev"
