...
: "${DATABASE_URL:=mysql://root:password@127.0.0.1:3306/ccag_den}"
...
pnpm --filter @ccag-ee/den-db db:push <<'EOF'
y
EOF
...
(
  ...
  env \
    ...
    NEXT_PUBLIC_CCAG_AUTH_CALLBACK_URL="$DEN_WEB_ORIGIN" \
