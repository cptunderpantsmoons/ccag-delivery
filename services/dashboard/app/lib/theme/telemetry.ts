 ...
 *
 * Data is used to auto-tune animation easing curves and detect
 * experimental engine failures for graceful degradation.
 */
...
class TelemetryRing {
  ...
  recommendEngine(): "css" | "raf" | "worker" {
    // Auto-tune: if rAF shows >10% drop rate over last 5 toggles, fall back to CSS
...
/**
 * Capture memory delta if performance.memory is available (Chromium).
 */
...
/**
 * Log a transition attempt with full metrics.
 */
